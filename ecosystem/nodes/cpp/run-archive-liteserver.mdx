...
- 16-core CPU
- 128 GB RAM
- At least 16TB of NVMe Gen4+ SSD storage (Enterprise grade preferred), 20TB if you will not use ZFS with compression, or Provisioned 64k+ IOPS storage. If using non-Enterprise SSDs, you must disable Autonomous Power State Transition (APST) on the SSD and enable the `performance` PCIe ASPM policy at the system level (e.g., `echo performance | sudo tee /sys/module/pcie_aspm/parameters/policy`).
...
Ensure download and upload speeds meet the [1 Gbit/s requirement](/ecosystem/nodes/cpp/run-archive-liteserver#1-1-minimal-hardware-requirements).
...
Enable the firewall and allow only the SSH port. The node UDP port and liteserver port are added after installation in [step 2.2.1](#2-2-1-open-the-node-udp-port-and-the-liteserver-port).
...
You can monitor the progress using MyTonCtrl:
...
Update your node software and MyTonCtrl to the latest versions:
...
To verify your disk and system performance, you can run the built-in `mytonctrl` benchmark:
