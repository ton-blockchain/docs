...
<Aside type="tip">
  Uncompressed archive data requires approximately 20 TB. Using ZFS with compression reduces this to \~16 TB.
</Aside>
...
These commands will check for new versions of the TON node binaries and MyTonCtrl, download them, and apply the updates. The update process may cause temporary node downtime as the binaries are replaced and services are restarted.
...
- [Run a validator node with MyTonCtrl](/ecosystem/nodes/cpp/run-validator)
- [Setup a node with MyTonCtrl](/ecosystem/nodes/cpp/setup-mytonctrl)
