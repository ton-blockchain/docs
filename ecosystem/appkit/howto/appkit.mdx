...
- **TON Connect** compliance: integrates with the standard wallet infrastructure for TON.
...
- **DeFi**: on-chain Toncoin and jetton swaps through registered providers.

## Use cases

- **Web2 apps** that need TON for authorization, payments, NFT rewards, or app-specific TON workflows.
- **Web3 apps** that need to integrate TON wallets.
- **Telegram Mini Apps** that use the TON blockchain to provide services for Telegram users.
...
Create the `AppKit` instance once with an `AppKitConfig`. All fields are optional; pass only the configuration your app needs. See [Get started → Connectors](/ecosystem/appkit/get-started/connectors) for the constructor walkthrough.

A few configuration fields are worth knowing about up front:

- `setDefaultNetwork(appKit, { network })` defines the network connectors require when a wallet connects. Pass `undefined` to allow wallets from any network.
- `ssr: true` in `AppKitConfig` switches AppKit into a mode safe for server-rendered environments such as Next.js.
- Get a mainnet API key from [t.me/toncenter](https://t.me/toncenter) for higher rate limits. Asset reads issue many requests; without a key, expect frequent 429 responses.
