...
You need a streaming provider registered on the AppKit instance before any watcher will fire. See [Providers → Streaming providers](/ecosystem/appkit/get-started/providers#provider-types) for the registration pattern, and use `hasStreamingProvider(appKit)` from `@ton/appkit` as the runtime check before subscribing.
