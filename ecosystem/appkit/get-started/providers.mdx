...
Pass more than one swap provider in the same array — for example `new DeDustSwapProvider()` and `new OmnistonSwapProvider()` together — and the swap manager picks the better quote per request.
