...
| `parameters.query` | `UnionLooseOmit<QueryOptions<queryFnData = unknown, error = Query.DefaultError, data = queryFnData, queryKey = Query.QueryKey>, 'queryKey = Query.QueryKey' \| 'queryFn'> \| undefined` | TanStack Query options forwarded to `useQuery` (`enabled`, `staleTime`, `refetchInterval`, `select`, …). `queryKey` and `queryFn` are managed by the wrapper. |
...
| `parameters.mutation` | `LooseOmit<Query.MutationOptions<data = unknown, error = Error, Compute<variables = void>, context = unknown>, 'mutationFn' \| 'mutationKey' \| 'throwOnError'> \| undefined` | TanStack Query mutation options forwarded to `useMutation` (`onSuccess`, `onError`, `onMutate`, `retry`, …). `mutationFn`, `mutationKey` and `throwOnError` are managed by the wrapper. |
