...
<Steps>
  ...
  >
    ...
    Blueprint creates `./wrappers/FirstContract.ts` when it scaffolds the project, and the Tolk compiler emits a `contract.abi.json` describing the contract's messages and get methods. The wrapper below is written by hand to keep the tutorial self-contained, but the same skeleton can be generated from that ABI. For naming conventions and the deeper pattern, see [Wrappers in Blueprint](/contract-dev/blueprint/develop#wrappers).
