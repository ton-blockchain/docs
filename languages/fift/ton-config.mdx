...
- Familiarity with [Fift](/languages/fift/overview); installed [`fift` binary](/languages/fift/overview#installation).
...
To create this proposal, we need to pay 2.3408 Toncoin. It is advisable to add at least 1.5 Toncoin to cover the processing fees. Therefore, we will send a total of 4 Toncoin with the request — any excess Toncoin will be returned. We will then use [`wallet.fif`](https://github.com/ton-blockchain/ton/blob/05bea13375448a401d8e07c6132b7f709f5e3a32/crypto/smartcont/wallet.fif) or the appropriate Fift script for our wallet to execute a transfer from the wallet to the configuration smart contract, attaching 4 Toncoin and the body from `config-msg-body.boc`.

This process typically looks as follows:
...
- Next, the [`config-proposal-vote-signed.fif`](https://github.com/ton-blockchain/ton/blob/05bea13375448a401d8e07c6132b7f709f5e3a32/crypto/smartcont/config-proposal-vote-signed.fif) script is invoked. This script takes the same arguments as `config-proposal-vote-req.fif`, plus two more: the base64-encoded validator public key used to sign the vote request, and the base64-encoded signature.
