...
<Aside>
  Internally, word `variable` is a shorthand for `hole constant`. So, `variable x` is a shorthand for `hole constant x`, which is defining a [constant](#constants) `x` that stores an empty box.
</Aside>
...
<Aside type="note">
  For more details on blocks/execution tokens and words `' <WORD_NAME>` and `execute`, refer to the blocks section in the [control flow page](/language/fift/control#blocks).
