...
As a result, all gas consumed within the `try` block is still accounted for, and any modifications carried out by operations that change gas limits (e.g., [`accept_message`](/languages/func/stdlib#accept_message) or [`set_gas_limit`](/languages/func/stdlib#set-gas-limit)) will remain in effect.
