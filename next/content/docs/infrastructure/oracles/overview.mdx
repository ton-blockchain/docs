...
Read more: [Randomness in TON](/ecosystem/contract-dev/techniques/random)

## Oracles in TON

Since the TON execution model is asynchronous, the classic ways to interact with oracles (get methods during transaction) can't be applied here. You can learn about the differences in ["Coming from Ethereum"](/learn-more/from-ethereum) article. The best pattern to retrieve data from an oracle is the Request-Response pattern - you send an internal message to the oracle contract and verify the response, getting the needed data.
...
- [RedStone](/infrastructure/oracles/redstone)
- [Pyth](/infrastructure/oracles/pyth)
