...
Each agentic wallet is a smart contract deployed as [Soulbound Token (SBT)](/ecosystem/standard/tokens/nft/comparison#sbt) within a shared NFT collection. Internally, each agentic wallet retains full [wallet v5](/ecosystem/standard/wallets/comparison#comparison-table) functionality. It stores the user's address (owner), the agent's (operator) public key, and a nonce to protect against replay attacks.
...
The recommended way to create and use agentic wallets is through [`@ton/mcp`](/ai/mcp). The [`@ton/mcp` quick start](/ai/mcp#quick-start) guide covers the full setup flow and gives common usage examples.
