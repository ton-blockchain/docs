...
`tonConnectUi.signData(payload)` is the entry point. The wallet must be connected when you call it, or pass `options.onConnected` to defer the request until a wallet is picked from the modal — a compliant wallet then handles connect and sign in one tap. See [Connect-and-act in one tap](/ecosystem/ton-connect/how-to/embedded-request). Without either, the SDK throws.
...
3. Look up the user's Ed25519 `publicKey` — extract it from `walletStateInit` (see below), then fall back to calling `get_public_key` on-chain. Never trust `publicKey` from the wallet response directly. See [Connect a wallet § backend verification](/ecosystem/ton-connect/how-to/connect#backend-verification).
...
- [Connect a wallet § backend verification](/ecosystem/ton-connect/how-to/connect#backend-verification) — for looking up the user's public key
