---
title: "contribute/style-guide-extended"
id: contribute/style-guide-extended
description: ""
---
...
See: [TVM exit codes](/blockchain-basics/tvm/exit-codes), [TL‑B overview](/data-formats/tlb).
```
...
- Link text **SHOULD** describe the destination, not the action of navigating. One effective technique: use the title or topic of the target page as the link text and integrate it into the sentence.
  ...
  - Bad: `"To learn about fees, click [here](/blockchain-basics/primitives/fees)."`
  - Bad: `"Click [here](/blockchain-basics/primitives/fees) for fee information."`
  - Bad: `"[This page](/blockchain-basics/primitives/fees) explains fees."`
  - Good: `"The [fee structure](/blockchain-basics/primitives/fees) determines transaction costs."`
  - Good: `"Transaction costs depend on the [fee structure](/blockchain-basics/primitives/fees)."`
...
- Standalone "stub" sentences whose only job is to say "See …" or "Read here …" followed by a link (for example, `[Read here](/blockchain-basics/standard/wallets/mnemonics) to learn more.` or `See [mnemonics](/blockchain-basics/standard/wallets/mnemonics) for more details.`) **MUST NOT** appear in running prose. Instead, attach the link to the relevant term or phrase in a normal sentence (for example, `The [mnemonics guide](/blockchain-basics/standard/wallets/mnemonics) explains why.`). (Why: inline links keep the text flowing and avoid postponing useful links to trailing meta‑sentences.) \[HIGH]
...
Start the node with `--threads` (see [validator flags → `--threads`](/blockchain-basics/nodes/overview#validator-node)) to increase parallelism.
