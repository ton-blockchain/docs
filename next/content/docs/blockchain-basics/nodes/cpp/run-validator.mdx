---
id: ecosystem/nodes/cpp/run-validator
...
- Validator deposit stake [700 000 TON \~ 4 000 000 TON](/blockchain-basics/nodes/cpp/run-validator#step-4:-set-optimal-stake-for-validator).
...
- [One UDP port open for incoming connections.](/blockchain-basics/nodes/cpp/run-validator#2-4-verify-validator’s-port)
...
Ensure download and upload speeds meet the [1 Gbit/s requirement](/blockchain-basics/nodes/cpp/run-validator#1-2-minimal-hardware-requirements).
...
Enable the firewall and allow only the SSH port. The validator UDP port is added after installation in [step 2.5](/blockchain-basics/nodes/cpp/run-validator#2-5-check-validator's-port).
...
For additional protection, store validator keys and configuration on an encrypted partition. Create an encrypted volume and symlink the [backup directories](/blockchain-basics/nodes/cpp/run-validator#5-5-organize-validator-backup) from it.
...
<Aside type="tip">
  Set up [alert bot](/blockchain-basics/nodes/cpp/integrating-with-prometheus) to receive a notification when the node is synchronized.
</Aside>
...
<Aside type="tip">
  Import this secret key with [`iw`](/blockchain-basics/nodes/cpp/mytonctrl/wallet#iw) command.
</Aside>
...
Create a [single nominator pool](https://old-docs.ton.org/v3/documentation/smart-contracts/contracts-specs/single-nominator-pool) for secure stake management. As the `owner-address`, specify the beneficiary wallet address that will stake the owner's funds and receive rewards.
...
[Blockchain explorer](/overview/infrastructure/explorers/overview) should also display the now-deployed contract with `Active` status.
...
<Aside
  type="caution"
>
  After activation, verify the following in a [blockchain explorer](/overview/infrastructure/explorers/overview):
...
Test withdrawal workflow. Only the [owner's cold wallet](/blockchain-basics/nodes/cpp/run-validator#3-6-create-a-pool) can request a withdrawal.
...
In the next election cycle (odd/even), MyTonCtrl will send half of the stake from the single nominator pool. Track this with a [blockchain explorer](/overview/infrastructure/explorers/overview).

### 4.6 Track second cycle progress

In the next paired election cycle (even/odd), MyTonCtrl will send the second half of the stake from the single nominator pool. Track this with a [blockchain explorer](/overview/infrastructure/explorers/overview).
...
- Follow the [test deposit and withdrawal procedure](/blockchain-basics/nodes/cpp/run-validator#3-10-test-deposit-to-pool) before depositing the full stake.
...
Only the owner wallet can withdraw funds. Follow the [test withdrawal procedure](/blockchain-basics/nodes/cpp/run-validator#3-11-test-withdrawal-from-pool) using the same method.
...
Set up alerting in MyTonCtrl to get a notification of critical issues with the validator. For more information, see [MyTonCtrl private alerting bot](/blockchain-basics/nodes/cpp/mytonctrl/alerting) or check [MyTonCtrl public alerting bot](https://t.me/validators).
...
- For system-level metrics, [integrate Prometheus with `node_exporter` with MyTonCtrl](/blockchain-basics/nodes/cpp/integrating-with-prometheus).
...
Confirm that the validator participates in elections and recovers stakes on time by checking the pool balance in a [blockchain explorer](/overview/infrastructure/explorers/overview). For greater observability, [set up the monitoring](/blockchain-basics/nodes/cpp/run-validator#5-7-set-up-monitoring) of various validator metrics.
...
- Ensure disk IOPS meet minimum requirements. Re-run the [benchmark](/blockchain-basics/nodes/cpp/run-validator#disk-iops).
...
- Review hardware against [minimum requirements](/blockchain-basics/nodes/cpp/run-validator#1-2-minimal-hardware-requirements).
...
- Check the elector contract status using a [blockchain explorer](/overview/infrastructure/explorers/overview).
