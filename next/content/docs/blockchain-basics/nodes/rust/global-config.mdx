---
id: ecosystem/nodes/rust/global-config
title: "How to configure global JSON file"
sidebarTitle: "Global configuration"
description: ""
keywords:
  - "zero state init block hardforks global config"
  - "validator config zero_state init_block"
  - "TON network identity references"
  - "configure hardforks in global config JSON"
  - "init_block and hardforks reference points"
---
...
- `k` and `a` – DHT configuration parameters. In a [Kademlia-based DHT](/blockchain-basics/whitepapers/ton#3-2-ton-dht-kademlia-like-distributed-hash-table):
...
`liteservers` defines an array of public endpoints used by lite clients (not by the node itself) to retrieve blockchain data, including balances, transactions, blocks, and [network configuration](/blockchain-basics/primitives/config). Each server object has three properties:
