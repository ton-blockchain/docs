---
id: ecosystem/nodes/rust/node-config
...
The example shows recommended values for production deployments. These values may differ from the defaults listed in the [field reference](/blockchain-basics/nodes/rust/node-config-ref). If a field is not specified, the node uses the default value defined in the codebase.
...
Each port's service supports `type`, `externalTrafficPolicy`, `annotations`, and [`perReplica`](/blockchain-basics/nodes/rust/node-config#loadbalancer-recommended) overrides.
