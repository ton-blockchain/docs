---
id: ecosystem/nodes/rust/monitoring
title: "Monitoring"
...
1. Set `ports.metrics` in Helm values:
   ...
   The port must match the `metrics.address` port in [node config](/blockchain-basics/nodes/rust/node-config-ref).
...
`PrometheusRule` resources can be created to trigger alerts based on TON node [metrics](/blockchain-basics/nodes/rust/metrics).
