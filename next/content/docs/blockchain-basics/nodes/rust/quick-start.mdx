---
id: ecosystem/nodes/rust/quick-start
...
- If full chain history is needed, enable archival mode as described in [Archival node settings](/blockchain-basics/nodes/rust/node-config#archival-node).
...
The chart includes a mainnet [`globalConfig`](/blockchain-basics/nodes/rust/global-config) and a default [`logsConfig`](/blockchain-basics/nodes/rust/logs-config). This minimal setup requires only [`nodeConfigs`](/blockchain-basics/nodes/rust/node-config).

Other networking modes are described in the [Networking section](/blockchain-basics/nodes/rust/node-config#networking), including `NodePort`, `hostPort`, `hostNetwork`, and ingress controllers such as `ingress-nginx`.
...
- Use one [exposure mode](/blockchain-basics/nodes/rust/node-config#exposure-modes) per deployment. Combining modes is possible but uncommon.
