---
id: ecosystem/nodes/rust/logs-config
...
Logging is configured using a YAML file specified in the `log_config_name` field of the [node configuration](/blockchain-basics/nodes/rust/node-config). In the Helm chart, this file is mounted at `/main/logs.config.yml`.
