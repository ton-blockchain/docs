---
id: ecosystem/staking/stake-calculation
...
MyTonCtrl automatically calculates the stake amount before submitting an election entry. The calculation balances operator preferences, staking mode, network-imposed limits, and fee reserves to produce a single integer value (in TON) sent to the [elector contract](/blockchain-basics/primitives/system).
...
| [On-chain config #17](/blockchain-basics/primitives/config#param-17-stake-limits) | `minStake`             | Network minimum stake. Submissions below this value are rejected by the elector.                                  |
| [On-chain config #17](/blockchain-basics/primitives/config#param-17-stake-limits) | `maxStake`             | Network maximum stake. Submissions above this value are clamped.                                                  |
...
Configure `stake` and `stakePercent` with the [`set`](/blockchain-basics/nodes/cpp/mytonctrl/core#set) command:
...
- [Staking overview](/blockchain-basics/nodes/staking/overview) — comparison of staking solutions and minimum requirements.
- [Run a validator](/blockchain-basics/nodes/cpp/run-validator#step-4-set-optimal-stake-for-validator) — step-by-step guide for setting optimal stake.
- [MyTonCtrl settings](/blockchain-basics/nodes/cpp/mytonctrl/core#supported-settings) — full list of configurable parameters including `stake` and `stakePercent`.
- [Single nominator pools](/blockchain-basics/nodes/staking/single-nominator) — recommended staking contract for large holders.
