...
<Aside type="note">
  Jettons are fungible tokens on TON. Each jetton has a master contract (minter) and separate wallet contracts for every holder. Read more in [How Jettons Work](/blockchain-basics/standard/tokens/jettons/how-it-works).
</Aside>
...
For architectural patterns and deposit methods comparison, see [Toncoin processing](/blockchain-basics/payments/toncoin).
...
1. Parse the message body:
   ...
   - Extract `query_id`, `amount`, `sender`, and `forward_payload` [according to TL-B](/blockchain-basics/standard/tokens/jettons/api)
