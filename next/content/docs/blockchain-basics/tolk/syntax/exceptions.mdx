...
In [TVM](/blockchain-basics/tvm/overview), exceptions are numbers. There is no `Error` class or exception hierarchy, only [exit codes](/blockchain-basics/tvm/exit-codes).
...
>
  Lower values are [reserved](/blockchain-basics/tvm/exit-codes) by TVM. Larger ones are more gas-expensive.
</Aside>
...
An exception can carry an argument of type [`unknown`](/blockchain-basics/tolk/types/unknown):
...
The `condition` must be a [`bool`](/blockchain-basics/tolk/types/booleans). Integers are not implicitly converted: use `assert (someInt != 0)` instead of `assert (someInt)`.
...
Additionally, an [out-of-gas exception](/blockchain-basics/tvm/exit-codes#14%3A-out-of-gas-error) may occur at **any** point, and it cannot be caught using a [`catch`](#try-catch-statement) block. It can only be prevented by writing [exhaustive testing suites](/blockchain-basics/contract-dev/blueprint/testing/overview).
