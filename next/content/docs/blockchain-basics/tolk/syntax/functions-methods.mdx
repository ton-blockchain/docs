---
id: tolk/syntax/functions-methods
...
The same convention has long been allowed for [`void`-typed struct fields](/blockchain-basics/tolk/types/void-never#void-typed-fields).
...
To allow modifications, declare [`mutate self`](/blockchain-basics/tolk/syntax/mutability) explicitly.
...
The [compiler automatically inlines small functions](/blockchain-basics/tolk/features/compiler-optimizations) in-place when possible. For example:
...
Tolk supports declaring low-level [assembler functions](/blockchain-basics/tolk/features/asm-functions) with embedded Fift code:
...
Tolk supports first-class functions: [they can be passed as callbacks](/blockchain-basics/tolk/types/callables).
...
Lambdas can capture outer variables by value, becoming [closures](/blockchain-basics/tolk/types/callables#closures-lambdas-with-captures).
