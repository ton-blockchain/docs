...
Methods for [structures](/blockchain-basics/tolk/types/structures) are declared in the same way:
...
Tolk code is executed by [TVM](/blockchain-basics/tvm/overview) – a stack-based virtual machine. Mutations work by implicitly returning new values through the stack.
...
[Auto-serialization](/blockchain-basics/tolk/features/auto-serialization) with `fromSlice` follows the same mutability rules. Passing an argument without `mutate` never modifies the original variable.
