...
To make updates to `userId` visible in `demo`, declare the parameter with [`mutate`](/blockchain-basics/tolk/syntax/mutability#mutate-for-a-parameter): `mutate userId`.
...
To calculate CRC32 and similar values at compile time, use [`"...".crc32()`](/blockchain-basics/tolk/types/strings#compile-time-methods) and similar compile-time methods.

To group integer constants, use [enums](/blockchain-basics/tolk/types/enums).
...
In Tolk, avoid globals when possible. Use [auto-serialization](/blockchain-basics/tolk/features/auto-serialization) and [`lazy` loading](/blockchain-basics/tolk/features/lazy-loading).
