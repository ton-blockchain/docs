---
id: tolk/from-func/tolk-vs-func
title: "Tolk vs FunC"
description: ""
---
...
1. Review the list of [differences](/blockchain-basics/tolk/from-func/tolk-vs-func#key-differences).
1. See the [Tolk contract examples](/blockchain-basics/tolk/examples) page.
1. Use the [FunC-to-Tolk converter](/blockchain-basics/tolk/from-func/converter) to migrate existing projects.

## Gas benchmarks

The [`tolk-bench` repository](https://github.com/ton-blockchain/tolk-bench) compares FunC and Tolk across several TEP contracts. See the [Tolk contract examples](/blockchain-basics/tolk/examples) page for selected cases.
...
- Both languages run on [TVM](/blockchain-basics/tvm/overview) after compilation to bitcode.
- Both languages are supported by [IDE plugins](/blockchain-basics/contract-dev/ide/vscode).
- Both languages are available in [Blueprint](/blockchain-basics/contract-dev/blueprint/overview) and other client-side tooling. However, best support for Tolk is provided in the [Acton](https://ton-blockchain.github.io/acton/docs/welcome) development environment.
...
- [Tolk resembles](/blockchain-basics/tolk/basic-syntax) TypeScript, Rust, and Kotlin.
...
- Tolk uses named [structures](/blockchain-basics/tolk/syntax/structures-fields) with the same runtime efficiency.
...
- Tolk derives [serialization](/blockchain-basics/tolk/features/auto-serialization) from `struct` using `toCell` and `fromCell`.
...
All [integer types](/blockchain-basics/tolk/types/numbers) such as `int8`, `uint64`, and `coins` are TVM integers.

### Lazy loading

- FunC requires manual control over preloads and skips for optimization.
- Tolk uses the [`lazy` keyword](/blockchain-basics/tolk/features/lazy-loading) to load only accessed fields.
...
- Tolk provides a [`bool` type](/blockchain-basics/tolk/types/booleans) and logical operators `&&`, `||`, and `!`. Conditions of `if`, `while`, ternary, and `assert` must be `bool` — write `if (someInt != 0)` instead of `if (someInt)`.
...
- Tolk provides a [`address` type](/blockchain-basics/tolk/types/address) with methods and the `==` operator.
...
- Tolk provides [nullable types](/blockchain-basics/tolk/types/nullable) `T?`, null safety, and smart casts.
...
- Tolk provides [a type system](/blockchain-basics/tolk/types/list-of-types), including unions, generics, and enums.
...
- Tolk provides both [functions and methods](/blockchain-basics/tolk/syntax/functions-methods), applicable to structures and primitives.
...
- Tolk provides native [maps](/blockchain-basics/tolk/types/maps), for example `m.set(1, 10)`.
...
- Tolk provides [`onInternalMessage(in)`](/blockchain-basics/tolk/features/message-handling) and use `in.senderAddress`, etc.
...
- Tolk routes messages using union types and [pattern matching](/blockchain-basics/tolk/syntax/pattern-matching).
...
- Tolk handles empty or unknown messages using `else` in a [`lazy` matching](/blockchain-basics/tolk/features/lazy-loading#lazy-matching).
...
- Tolk provides [`createMessage`](/blockchain-basics/tolk/features/message-sending), which automatically chooses between inline body and body reference.
...
- Tolk allows only [alphanumeric identifiers](/blockchain-basics/tolk/syntax/variables), for example  `2+2` is `4`.
...
In FunC, `inline` modifier operates at the Fift level and may introduce extra stack permutations. In Tolk, inlining is performed at the compiler level and is combined with [constant folding](/blockchain-basics/tolk/features/compiler-optimizations).
...
There are [differences between standard libraries](/blockchain-basics/tolk/from-func/stdlib-comparison). For example, functions from `stdlib.fc` started having descriptive names:
...
Although Tolk is a high-level language, it exposes [low-level capabilities](/blockchain-basics/tolk/features/asm-functions). Code may still be written in a FunC-style with manual builders and slices, and TVM instructions are supported.
...
- Tolk emits an [ABI JSON](/blockchain-basics/tolk/features/contract-abi#contract-abi-export) directly from sources whenever a file declares a `contract { ... }`. The same metadata drives auto-generated TypeScript wrappers, source maps, and a step-by-step debugger that works on fully-optimized bytecode.
