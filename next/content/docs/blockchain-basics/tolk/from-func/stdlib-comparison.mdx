---
id: tolk/from-func/stdlib-comparison
title: "Standard libraries comparison"
sidebarTitle: "Standard libraries"
description: ""
---

Tolk provides a standard library that evolved from [FunC's standard library](/blockchain-basics/languages/func/stdlib).
Therefore, many functions from `stdlib.fc` can be mapped to Tolk, and most of these functions were converted into [methods](/blockchain-basics/tolk/syntax/functions-methods).

## Functions vs methods

Tolk supports [declaring methods](/blockchain-basics/tolk/syntax/functions-methods), including primitives. Consequently, many FunC global-scope functions are now methods, e.g., `cell.hash()`, `tuple.size()`, etc.
...
For `load_xxx`, `store_xxx`, and `skip_xxx`, use [automatic serialization](/blockchain-basics/tolk/features/auto-serialization). Methods `slice.loadXXX`, `builder.storeXXX`, and `slice.skipXXX` are available for manual cell parsing, but their use is not recommended.

For `idict_xxx`, `udict_xxx`, and `dict_xxx`, use [native maps](/blockchain-basics/tolk/types/maps). Dictionaries remain available in the file `@stdlib/tvm-dicts`, but their is not recommended.
...
In FunC, `x~method` mutates, whereas `x.method` returns a copy. In Tolk, methods are called with dot `.` and may [mutate](/blockchain-basics/tolk/syntax/mutability) the object.
...
The [standard library](/blockchain-basics/tolk/features/standard-library) is split into multiple files. Functions from `common.tolk` are always available, and most FunC analogues are included. Functions from other files must be explicitly imported:
