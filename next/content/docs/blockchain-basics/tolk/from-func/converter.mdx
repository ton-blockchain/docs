...
- Use the modern [`onInternalMessage`](/blockchain-basics/tolk/features/message-handling#oninternalmessage).
- Extract a [`Storage` struct](/blockchain-basics/tolk/features/contract-storage) with `toCell` and `fromCell`.
- Refactor incoming messages into [structs](/blockchain-basics/tolk/syntax/structures-fields#serialization-prefixes-and-opcodes) with 32-bit opcodes – incrementally, one message at a time.
- Define a union of possible messages and match it using [`lazy`](/blockchain-basics/tolk/features/lazy-loading).
- Extract outgoing messages into structs and [send](/blockchain-basics/tolk/features/message-sending) them.
...
The [Tolk contract examples](/blockchain-basics/tolk/examples) page features some contracts from the [Tolk vs. FunC benchmarks](https://github.com/ton-blockchain/tolk-bench) repository, all converted step by step using the migration tool. Check the Git history for each migration.
