---
id: tolk/features/message-handling
...
A contract file typically begins with a [`contract` declaration](/blockchain-basics/tolk/features/contract-abi) that names the contract and lists its public shapes — including the union of incoming messages.

## `onInternalMessage`

Contracts primarily handle [internal messages](/blockchain-basics/primitives/messages/internal#internal-messages). Users interact with contracts through their wallets, which send internal messages to the contract. The entrypoint is declared as follows:
...
- Reference the union from `incomingMessages:` in the [`contract` declaration](/blockchain-basics/tolk/features/contract-abi).
...
- [`struct`](/blockchain-basics/tolk/types/structures) declares business data, including messages and storage.
- `(0x12345678)` defines a message opcode, 32-bit. Unique prefixes are used to route binary data in `in.body`.
- `AllowedMessage` is a type alias for a [union type](/blockchain-basics/tolk/types/unions).
- `in: InMessage` provides access to message properties such as `in.body` and `in.senderAddress`.
- `T.fromSlice` [parses binary data into `T`](/blockchain-basics/tolk/features/auto-serialization). When combined with [`lazy`](/blockchain-basics/tolk/features/lazy-loading), parsing is performed on demand.
- [`match`](/blockchain-basics/tolk/syntax/pattern-matching) routes a union type. Within each branch, the type of `msg` is narrowed, smart cast.
- `throw 0xFFFF` is a standard reaction to an unrecognized message. Contracts typically ignore empty messages, which represent balance top-ups with an empty body. For this reason, `throw` is guarded by [`if`](/blockchain-basics/tolk/syntax/conditions-loops) or `assert`.

Bounced messages are not handled by `onInternalMessage`.

### Define and modify contract storage

[Contract storage](/blockchain-basics/tolk/features/contract-storage) is defined as a regular structure. Storage types commonly define `load` and `save` methods to access persistent contract data:
...
`onBouncedMessage` is a special entrypoint for handling [bounced messages](/blockchain-basics/primitives/messages/internal#bounces).
...
When [sending a message](/blockchain-basics/tolk/features/message-sending) using `createMessage`, the `bounce` behavior must be specified:
...
In addition to internal messages, a contract can handle [external messages](/blockchain-basics/primitives/messages/external-in) originating off-chain.
For example, [wallet contracts](/blockchain-basics/standard/wallets/how-it-works) process external messages and perform signature validation using a public key.
