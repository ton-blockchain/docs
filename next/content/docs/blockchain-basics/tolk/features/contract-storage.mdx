---
id: tolk/features/contract-storage
title: "Contract storage"
description: ""
---

Contract storage is a [`struct`](/blockchain-basics/tolk/types/structures), serialized into persistent blockchain data. Tolk does not enforce any specific storage layout.
...
- `fun Storage.f(self)` defines an instance [method](/blockchain-basics/tolk/syntax/functions-methods).
- `T.fromCell()` deserializes a cell into `T`, and [`obj.toCell()`](/blockchain-basics/tolk/features/auto-serialization#unpackoptions-and-packoptions) packs it back into a cell.
- The [`lazy`](/blockchain-basics/tolk/features/lazy-loading) operator does the parsing on demand.
- [`contract.getData()`](/blockchain-basics/tolk/features/standard-library) fetches persistent data.

## Default values

In TON, a contract’s address depends on its initial storage when it is [created on-chain](/blockchain-basics/primitives/addresses/derive#on-chain-—-simple). Assign default values to fields that must be defined at deployment:
...
This behavior is not related to [nullable types](/blockchain-basics/tolk/types/nullable). Nullable values such as `int8?` or `cell?` serialize with an explicit null marker 0 bit. In this case, fields are absent entirely, and no extra bits appear in serialization.
...
When this pattern is used, declare both shapes in the [`contract` declaration](/blockchain-basics/tolk/features/contract-abi) so client tools can compute the deployment address correctly:
...
Different shapes with missing fields can also be expressed using [generics](/blockchain-basics/tolk/types/generics) and the [void type](/blockchain-basics/tolk/types/void-never).
