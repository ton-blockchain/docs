...
For unions, [createMessage](/blockchain-basics/tolk/features/message-sending) is lightweight. The compiler generates all `IF-ELSE` and `STU`, but during compile-time analysis, these instructions resolve to constants because all types are known at compile time. The resulting code flattens into `PUSHINT` and `STSLICECONST`.
...
The [`lazy` keyword](/blockchain-basics/tolk/features/lazy-loading) loads only the required fields from a cell or slice:
...
The Tolk compiler outputs the Fift assembler. Fift generates the bitcode. Projects built on [Blueprint](/blockchain-basics/contract-dev/blueprint/overview) use `tolk-js`, which invokes Tolk and then Fift.
...
- For [Acton](https://ton-blockchain.github.io/acton/docs/welcome) users, it is an intermediate result that can be accessed in the build directory.
