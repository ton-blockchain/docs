...
Manual work with slices and builders is error-prone and tedious. By comparison, [auto-serialization](/blockchain-basics/tolk/features/auto-serialization) with structures helps express data with types and prevents many related bugs.
...
All data in TON is stored in cells. To express data relation clearly and to aid in [serialization](/blockchain-basics/tolk/features/auto-serialization#controlling-cell-references-typed-cells), use cells with well-typed contents: `Cell<T>`.
...
When reading data from cells, add the [`lazy` keyword](/blockchain-basics/tolk/features/lazy-loading):
...
Serialization may require custom rules which are not covered by existing types. Tolk allows defining custom [serialization rules for type aliases](/blockchain-basics/tolk/types/overall-serialization#type-aliases):
...
[Contract storage](/blockchain-basics/tolk/features/contract-storage) is a regular `struct`, serialized into persistent on-chain data.
...
By convention, every message in TON has an _opcode_: a unique 32-bit number. In Tolk, every [`struct`](/blockchain-basics/tolk/syntax/structures-fields) can have a serialization prefix of arbitrary length. Use 32-bit prefixes to express message opcodes.
...
When implementing [Jettons](/blockchain-basics/standard/tokens/jettons/overview), [NFTs](/blockchain-basics/standard/tokens/nft/overview), or other standard contracts, use predefined opcodes according to the specification. Otherwise, opcodes are ad hoc.

## Use unions to handle incoming messages

The suggested pattern:

1. Each [incoming message](/blockchain-basics/tolk/features/message-handling) is made a struct with an opcode.
1. Structs are combined into a union type.
1. Union is used to lazily load data from the message body slice.
1. Finally, result is [pattern matched](/blockchain-basics/tolk/syntax/pattern-matching) over union variants.
...
<Aside type="tip">
  See the [Tolk contract examples](/blockchain-basics/tolk/examples#jetton) page for selected contracts from the [`tolk-bench`](https://github.com/ton-blockchain/tolk-bench).
</Aside>

## Target certain shards when deploying sibling contracts

Specify the prefix length and the contract address to aim for the [same shard](/blockchain-basics/tolk/features/message-sending#shard-based-deployment). For example, sharded jetton wallet must be deployed to the same shard as the owner's wallet.
...
[External messages](/blockchain-basics/tolk/features/message-sending#universal-createexternallogmessage) with a special address `none` are used to emit events and logs to the outer world. Indexers catch such messages and provide a picture of on-chain activity.
...
When a [contract getter](/blockchain-basics/tolk/features/contract-getters) needs to return several values, introduce a structure. Avoid returning unnamed tensors like `(int, int, int)`. Field names provide clear metadata for client wrappers and human readers.
...
After parsing an incoming message, validate required fields with [`assert`](/blockchain-basics/tolk/syntax/exceptions#assert-statement):
...
- Have `MyContract.tolk` as an entrypoint, named after the contract in PascalCase. Place a [`contract` declaration](/blockchain-basics/tolk/features/contract-abi) at the top of the file and keep all `get fun` and entrypoint functions within it; use [imports](/blockchain-basics/tolk/syntax/imports) to bring in shared code.
...
Use [methods](/blockchain-basics/tolk/syntax/functions-methods) to avoid name collisions:
...
A nullable [address](/blockchain-basics/tolk/types/address) `address?` is a pattern for an optional address, sometimes called _"maybe address"_:
...
Several [compile-time methods](/blockchain-basics/tolk/types/strings#compile-time-methods) operate on constant [strings](/blockchain-basics/tolk/types/strings):
...
Tolk provides a dedicated [`string` type](/blockchain-basics/tolk/types/strings) backed by snake-encoded cells. Use the [`StringBuilder`](/blockchain-basics/tolk/types/strings#stringbuilder) for concatenation:
...
For fixed-size binary data, use [`bitsN` or `bytesN` types](/blockchain-basics/tolk/types/cells#fixed-size-slices).

## Avoid micro-optimization

The [compiler applies many optimizations](/blockchain-basics/tolk/features/compiler-optimizations): it automatically inlines functions, reduces stack allocations, and handles the underlying work. Attempts to outsmart the compiler yield negligible effects, either positive or negative.
