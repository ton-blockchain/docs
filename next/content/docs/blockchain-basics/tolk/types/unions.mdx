...
A union type represents a value that can be one of several types. [Pattern matching](/blockchain-basics/tolk/syntax/pattern-matching) is used to distinguish union variants. A special case `T | null` is written as `T?` and referred to as a [nullable type](/blockchain-basics/tolk/types/nullable).
...
In [message-handling](/blockchain-basics/tolk/features/message-handling), union values are commonly parsed using `lazy`:
...
This pattern is referred to as [`lazy` match](/blockchain-basics/tolk/features/lazy-loading):
...
`T | void` is a valid serializable union meaning "T or empty slice" — `void` carries no bits and does not participate in the prefix tree. This is useful for chains like wallet-v5's payload format: parse cells until refs run out. See [void-never](/blockchain-basics/tolk/types/void-never#void-inside-unions-for-serialization).

## Stack layout and serialization

Union types have a complex [stack layout](/blockchain-basics/tolk/types/overall-tvm-stack), commonly referred to as tagged unions.

[Serialization](/blockchain-basics/tolk/types/overall-serialization) depends on whether the union consists of structures with manual serialization prefixes:
