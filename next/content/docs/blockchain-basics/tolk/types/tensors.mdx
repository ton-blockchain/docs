---
id: tolk/types/tensors
title: "Tensors"
sidebarTitle: "Tensors"
description: ""
---

Tensors represent ordered collections of values and are written in the form `(T1, T2, ...)`. They occupy multiple [TVM stack entries](/blockchain-basics/tolk/types/overall-tvm-stack) sequentially and are [serialized](/blockchain-basics/tolk/types/overall-serialization) in the same order. For example, `(int, slice)` represents two values following one another.

Tensors are anonymous [structures](/blockchain-basics/tolk/types/structures) and behave identically. Large tensors are impractical — use structures whenever possible.

<Aside
  type="caution"
>
  In many general-purpose languages, syntax `(A, B, C, ...)` is used for _tuples_, not tensors, which are a different type. However, in TON, a [tuple](/blockchain-basics/tolk/types/tuples) is a distinct TVM primitive that has its own syntax `[a, b, ...]` and semantics.
