...
Tolk provides low-level functions for constructing and parsing cells manually, as well as [automatic serialization](/blockchain-basics/tolk/features/auto-serialization) of structures to and from cells.

## Cells

[Cells](/blockchain-basics/primitives/serialization/cells) are a data structure that can hold up to 1023 bits of data and up to 4 references to other cells. They are read-only and immutable once created.
...
<Aside type="note">
  Tolk has a dedicated [`string`](/blockchain-basics/tolk/types/strings) type backed by snake-encoded cells. The examples below show how to embed constant slices with low-level binary data.
</Aside>
...
Structures are [auto-serializable](/blockchain-basics/tolk/features/auto-serialization) to and from cells when their fields are well-typed. Prefer using structures over manual work with builders and slices.
...
<Aside type="note">
  Notice that `T.fromSlice(s)` does not mutate a slice `s`, while `s.loadAny<T>()` does. This is due to how Tolk handles [mutability](/blockchain-basics/tolk/syntax/mutability): all `s.loadXYZ()` methods mutate a slice, but functions in the form `doSomething(s)` do not.
</Aside>
...
Structure fields of `cell` and `Cell<T>` types which are [serialized](/blockchain-basics/tolk/types/overall-serialization) as references to corresponding [TVM](/blockchain-basics/tolk/types/overall-tvm-stack) cells, with nullable types `?` serialized as 0 when the value is `null` and 1, followed by the cell reference otherwise.
