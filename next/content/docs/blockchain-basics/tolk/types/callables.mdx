...
Tolk supports [lambda functions](/blockchain-basics/tolk/syntax/functions-methods):
...
Tolk provides the low-level type `continuation`. For example, the TVM register [`c3`](/blockchain-basics/tvm/registers) contains the smart contract code. Some functions are available in the standard library:
...
A callable is backed by the [TVM `CONT`](/blockchain-basics/tolk/types/overall-tvm-stack). Callable values cannot be [serialized](/blockchain-basics/tolk/types/overall-serialization).
