...
Primitives like `int` or `cell`, when nullable, are [serialized](/blockchain-basics/tolk/types/overall-serialization) as a TVM value or `null`.

Nullable structures and other composites are [represented](/blockchain-basics/tolk/types/overall-tvm-stack) as tagged unions:

- if their type is `null`, they are serialized as `0`;
- otherwise, they are serialized as `1` followed by the non-null value.

The `address?` type is an exception, and it is [serialized in a different way](/blockchain-basics/tolk/types/address#nullable-address).
