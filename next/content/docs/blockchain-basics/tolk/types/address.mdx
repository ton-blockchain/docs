---
id: tolk/types/address
title: "Address"
description: ""
---

Every smart contract has an [address](/blockchain-basics/primitives/addresses/overview) used for all on-chain interactions.
...
When [serialized](/blockchain-basics/tolk/types/overall-serialization), values of `address` type occupy 267 bits:
...
Compare addresses using `==` or `!=`. Internally, an [address is represented as a raw `slice` without references](/blockchain-basics/tolk/types/overall-tvm-stack), so `==` and `!=` test for bits equality.
