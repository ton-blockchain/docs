---
id: tolk/types/type-checks-and-casts
...
Once a variable is checked, the compiler narrows its type. For instance, if a variable [could be `null`](/blockchain-basics/tolk/types/nullable) before the check, and the condition is confirmed to be true, the variable's type will not allow `null` in the true block anymore.
...
Given a [union type](/blockchain-basics/tolk/types/unions) `T1 | T2 | ...`, the `is` operator performs a runtime type test, narrowing the type.
...
For instance, `bool` cannot be assigned directly to `int`. However, a direct cast using the `as` operator is valid since booleans are represented as [TVM](/blockchain-basics/tvm/overview) integers: `true` is -1, and `false` is 0.
