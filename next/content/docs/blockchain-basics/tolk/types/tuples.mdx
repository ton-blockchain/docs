---
id: tolk/types/tuples
title: "Arrays and tuples"
sidebarTitle: "Arrays"
description: ""
---

In [TVM](/blockchain-basics/tvm/overview), a tuple is a dynamic container that stores from 0 to 255 elements in a [single stack slot](/blockchain-basics/tolk/types/overall-tvm-stack). Tolk provides several types built on top of TVM tuples:
...
<Aside
  type="caution"
>
  In many general-purpose languages, syntax `[A, B, C, ...]` is used for array or list literals, while syntax `(A, B, C, ...)` is used for tuples. However, in TON, there are no traditional arrays or lists. The `[A, B, C, ...]` syntax creates arrays or shaped tuples, while `(A, B, C, ...)` is used by [tensors](/blockchain-basics/tolk/types/tensors): a distinct type that represents ordered collections of values that occupy multiple TVM stack entries.
...
Additional methods such as `array.set` and others are [available in the standard library](/blockchain-basics/tolk/features/standard-library#arrays).
...
If there is a clear upper bound on the number of elements, prefer [arrays](/blockchain-basics/tolk/types/tuples#arrays) or [maps](/blockchain-basics/tolk/types/maps). Otherwise, consider [contract sharding](/blockchain-basics/contract-dev/techniques/contract-sharding).
...
Arrays can be [serialized to cells](/blockchain-basics/tolk/types/overall-serialization) when `T` is serializable. The binary format uses snake references: a `uint8` length followed by chained cell references containing the elements.

Raw tuples are not serializable to cells, because `unknown` is unserializable. But they can be returned from [get methods](/blockchain-basics/tvm/get-method#get-methods), since contract getters operate directly on the stack.
