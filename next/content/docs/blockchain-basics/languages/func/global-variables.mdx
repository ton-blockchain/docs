---
id: languages/func/global-variables
...
as long as those `.fc` files are [imported](/blockchain-basics/languages/func/compiler-directives#%23include) after the global
...
Under the hood, global variables in FunC are stored inside the tuple of the [`c7` TVM control register](/blockchain-basics/whitepapers/tvm#1-3-2-list-of-control-registers),
...
The example defines two auxiliary functions: `check_commutative` and `add`, in addition to the program entry point [`main`](/blockchain-basics/languages/func/special-functions#main).
...
<Aside>
  ...
  Reading a global variable that has not been assigned, will result in the [`null` value](/blockchain-basics/languages/func/types#null-values).
</Aside>

<Aside>
  Global variables do not survive after the TVM finishes execution. They are reset every time the
  [TVM initializes](/blockchain-basics/tvm/initialization).
  Use permanent storage to remember values between TVM executions by using the [`set_data`](/blockchain-basics/languages/func/stdlib#set-data) function.
