...
This section covers extra language constructs that are not part of the core but are still important for functionality. Although they could be implemented in [stdlib.fc](/blockchain-basics/languages/func/stdlib), keeping them as built-in features allows the FunC optimizer to work more efficiently.
...
For example, `_+_` can be understood as wrapping the [`+` operator](/blockchain-basics/languages/func/operators#addition%2C-%2B):
...
Same as [`divmod`](#divmod), but allows using [modifying notation](/blockchain-basics/languages/func/expressions#modifying-notation).
...
Same as [`moddiv`](#moddiv), but allows using [modifying notation](/blockchain-basics/languages/func/expressions#modifying-notation).
...
`muldiv` performs a multiply-then-divide operation `(factor1 * factor2) / divisor`, where `/` is the [division operator](/blockchain-basics/languages/func/operators#division%2C-%2F).
...
`muldivr` performs a multiply-then-divide operation `(factor1 * factor2) ~/ divisor`, where `~/` is the [rounding division operator](/blockchain-basics/languages/func/operators#rounding-division%2C-%2F).
It uses a 513-bit intermediate result to prevent overflow if the final result fits within 257 bits.
...
`muldivc` performs a multiply-then-divide operation `(factor1 * factor2) ^/ divisor`, where `^/` is the [ceiling division operator](/blockchain-basics/languages/func/operators#ceiling-division%2C-%5E%2F).
It uses a 513-bit intermediate result to prevent overflow if the final result fits within 257 bits.
...
`muldivmod` performs a multiply-then-divide operation `(factor1 * factor2) / divisor`, where `/` is the [division operator](/blockchain-basics/languages/func/operators#division%2C-%2F),
...
For more info, see [null values](/blockchain-basics/languages/func/types#null-values).
...
`throw` takes only one argument, the error code. See [TVM error codes](/blockchain-basics/tvm/exit-codes) for details about error codes.
...
See [TVM error codes](/blockchain-basics/tvm/exit-codes) for details about error codes.
...
processed in [`try..catch` statements](/blockchain-basics/languages/func/statements#try-catch-statement).
Refer to the `try..catch` statement page for an example on how to use the first argument.
The second argument is the error code. See [TVM error codes](/blockchain-basics/tvm/exit-codes) for details about error codes.
...
processed in [`try..catch` statements](/blockchain-basics/languages/func/statements#try-catch-statement), in the same way as with [`throw_arg`](#throw-arg).
The second argument is the error code. See [TVM error codes](/blockchain-basics/tvm/exit-codes) for details about error codes.
...
Same as [`store_int`](#store-int), but adapted to use [modifying notation](/blockchain-basics/languages/func/expressions#modifying-notation).
...
Same as [`store_uint`](#store-uint), but adapted to use [modifying notation](/blockchain-basics/languages/func/expressions#modifying-notation).
...
`~touch` is identical to [`touch`](#touch), but adapted to use [modifying notation](/blockchain-basics/languages/func/expressions#modifying-notation).
...
`~touch2` is identical to [`touch2`](#touch2), but adapted to use [modifying notation](/blockchain-basics/languages/func/expressions#modifying-notation).
...
[Modifying notation](/blockchain-basics/languages/func/expressions#modifying-notation) can be used on this function.
...
Executes the 0 argument function with [ID](/blockchain-basics/languages/func/functions#method-id-specifier) `method_id`. The function with ID `method_id` must have a signature of the form:
...
<Aside>
  Recall that method ids can be explicitly set when declaring functions using the [`method_id` specifier](/blockchain-basics/languages/func/functions#method-id-specifier):
...
Executes the 1 argument function with [ID](/blockchain-basics/languages/func/functions#method-id-specifier) `method_id`, and passes `arg1` as argument to the function.
...
Executes the 2 argument function with [ID](/blockchain-basics/languages/func/functions#method-id-specifier) `method_id`. The function with ID `method_id` must have a signature of the form:
...
Executes the 3 argument function with [ID](/blockchain-basics/languages/func/functions#method-id-specifier) `method_id`. The function with ID `method_id` must have a signature of the form:
