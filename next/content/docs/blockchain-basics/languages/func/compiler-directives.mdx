---
id: languages/func/compiler-directives
...
In Func, using a variable before it is modified within the same [expression](/blockchain-basics/languages/func/expressions) is prohibited by default.

For example, the following code will **not** compile, because `ds` is used before it is modified in `ds~load_uint(8)`.
See [modifying notation](/blockchain-basics/languages/func/expressions#modifying-notation) for more details on using symbol `~`.
...
Here, the `asm(value index dict key_len)` notation dictates a [rearrangement of arguments](/blockchain-basics/languages/func/asm-functions#rearranging-stack-entries).
