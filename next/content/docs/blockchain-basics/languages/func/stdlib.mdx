...
For detailed descriptions of these commands, refer to the [TVM documentation](/blockchain-basics/whitepapers/tvm).
...
For more details on polymorphic functions, refer to the [Polymorphism with forall](/blockchain-basics/languages/func/functions#polymorphism-with-forall) section.
...
Extracts the head and tail of a lisp-style list. It can be used as a [(non-)modifying method](/blockchain-basics/languages/func/statements#methods-calls).
...
>
  ...
  1. Message exceeds size limits, see the [maximum message bit and cell size](/blockchain-basics/primitives/limits#message-and-transaction-limits) for details.
  ...
  1. The outbound message contains invalid libraries in [`StateInit`](/blockchain-basics/primitives/messages/deploy).
...
See also [set\_c3](/blockchain-basics/languages/func/stdlib#set_c3).
...
It can be used as a [modifying method](/blockchain-basics/languages/func/statements#modifying-methods).
A primitive _preloads_ data when it returns only the extracted data, leaving the original slice unchanged. It can be used as a [non-modifying method](/blockchain-basics/languages/func/statements#non-modifying-methods).
...
See the [TVM whitepaper](/blockchain-basics/whitepapers/tvm), 3.1.2. If `c` is exotic, it is automatically converted into an ordinary cell `c'`
...
A primitive _stores_ a value `x` in a builder `b` by returning a modified version `b'` with `x` added at the end. It can also be used as a [non-modifying method](/blockchain-basics/languages/func/statements#non-modifying-methods).
...
Checks whether the given `cell c` is `null`. In most cases, a `null` cell represents an empty dictionary. FunC also includes a polymorphic `null?` built-in function. See [built-in](/blockchain-basics/languages/func/built-ins#other-primitives) for more details.
...
As stated in [TVM.pdf](/blockchain-basics/whitepapers/tvm):
...
>
> 1. **As a `Maybe cell (c^?)`**, which is either a cell that contains a serialized `Hashmap(n, X)` same as above or `null`, indicating an empty dictionary. See [null values](/blockchain-basics/languages/func/types#null-values) for details. When using this format, the dictionary is typically referred to as `D`.
>
...
Additionally, some primitives have counterparts prefixed with `~`, enabling them to be used as [modifying methods](/blockchain-basics/languages/func/statements#modifying-methods).
...
TVM supports dictionaries with non-fixed length keys that follow a prefix code structure, meaning no key is a prefix of another key. Learn more in the [TVM instruction](/blockchain-basics/tvm/instructions) section.
...
Marks a variable as used, ensuring the code generated isn't removed during optimization, even if it's not impure. See [Impure specifier](/blockchain-basics/languages/func/functions#impure-specifier) section.
