...
<Aside>
  Currently, all the unary and binary operators are integer operators. Logical operators are bitwise integer operators (cf. [absence of boolean type](/blockchain-basics/languages/func/types#no-boolean-type)).
</Aside>
...
<Aside>
  In expressions, all operators should be separated from their arguments, otherwise the expression is interpreted as an [identifier](/blockchain-basics/languages/func/literals#identifiers).
...
The `~` operator also behaves as expected on `-1` and `0`, which are the [emulated values](/blockchain-basics/languages/func/types#no-boolean-type) for `true` and `false`:
...
Going beyond the range of the integer type will result in an error with [exit code 4](/blockchain-basics/tvm/exit-codes#4%3A-integer-overflow): `Integer overflow`.
...
An attempt to divide by zero results in an error with [exit code 4](/blockchain-basics/tvm/exit-codes#4%3A-integer-overflow): `Integer overflow`.
...
If `b` in the expression `a % b` is `0`, the result is an error with [exit code 4](/blockchain-basics/tvm/exit-codes#4%3A-integer-overflow): `Integer overflow`.
...
The slash-percent sign (division and modulo) operator `/%` returns a two element [tensor](/blockchain-basics/languages/func/types#tensor-types), where the first element is the result
...
See more about decomposition notation in the [variable declaration](/blockchain-basics/languages/func/expressions#variable-declaration) section.
...
This is useful when we want to apply a specific [bitmask](https://en.wikipedia.org/wiki/Mask_%28computing%29).
...
`2^n` is exponentiation of `2` by `n`, and `/` is standard [arithmetical division](https://en.wikipedia.org/wiki/Division_%28mathematics%29).
...
Exceeding the range of the integer type will result in an error with [exit code 4](/blockchain-basics/tvm/exit-codes#4%3A-integer-overflow): `Integer overflow`.
...
`2^n` is exponentiation of `2` by `n`, `/` is standard [arithmetical division](https://en.wikipedia.org/wiki/Division_%28mathematics%29), and `+` is standard arithmetical addition.
...
(`?`), then an [expression](/blockchain-basics/languages/func/expressions) to execute (the "consequent" expression) if the condition evaluates to an integer different from `0`, followed by a colon (`:`),
...
The assignment operator `=` is used to assign the result of an [expression](/blockchain-basics/languages/func/expressions) to a variable. After execution, the `=` operator returns the value assigned to the variable.
...
The assignment operator can also occur in [variable declarations](/blockchain-basics/languages/func/expressions#variable-declaration), which are also expressions:
