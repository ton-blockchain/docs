...
FunC is a domain-specific, statically typed language with C-like syntax designed to write smart contracts on TON. It can be characterized as an intermediate-level language sitting on top of the [Fift](/blockchain-basics/languages/fift/overview) assembly language.
...
- [`()`](/blockchain-basics/languages/func/types#tensor-types) is the return type, that stands for "do not return value", similar to `void` in C;
- `send_money` is the name of the function;
- `slice address` is the first parameter, address to send money to; it's type is [`slice`](/blockchain-basics/languages/func/types#atomic-types);
- `int amount` is the second parameter, amount to be sent; it's type is [`int`](/blockchain-basics/languages/func/types#atomic-types);
- `impure inline` are [specifiers](/blockchain-basics/languages/func/functions#specifiers), which are flags that tell the compiler to process the method in a specific way;
- `var msg = ...` defines a variable without specifying its type; it will hold a [cell](/blockchain-basics/primitives/serialization/cells) with a [message](/blockchain-basics/primitives/messages/overview);
- [`begin_cell()`](/blockchain-basics/languages/func/stdlib#begin-cell) creates a [cell builder](/blockchain-basics/tvm/builders-and-slices);
- [`store_uint`](/blockchain-basics/languages/func/stdlib#store-uint), [`store_slice`](/blockchain-basics/languages/func/stdlib#store-slice), [`store_coins`](/blockchain-basics/languages/func/stdlib#store-coins) methods store data into the builder:
  - [flags](/blockchain-basics/primitives/messages/internal);
  - receiving address;
  - amount on Toncoin to attach to the message.
- [`.end_cell()`](/blockchain-basics/languages/func/stdlib#end-cell) method finalizes the builder and turns it into a cell;
- [`send_raw_message`](/blockchain-basics/languages/func/stdlib#send-raw-message) function sends the message, where the parameter `64` describes a [sending mode](/blockchain-basics/primitives/messages/internal).

## Compiler

The compiler converts FunC programs into [Fift](/blockchain-basics/languages/fift/overview) assembly code. The Fift assembly code is then compiled down to the [TON Virtual Machine](/blockchain-basics/tvm/overview) bitcode by the [Fift](/blockchain-basics/languages/fift/overview) compiler.

Developers can use the compiled bitcode, structured as a [bag of cells](/blockchain-basics/primitives/serialization/boc) like all data in the TON blockchain, to test smart contracts, send messages, or execute it in a local TVM instance.
...
The package has both the FunC and [Fift](/blockchain-basics/languages/fift/overview) compilers, and produces bitcode directly from the FunC source code, without manually invoking Fift.
...
Alternatively, use the [Blueprint](/blockchain-basics/contract-dev/blueprint/overview) to start a project pre-configured for development in FunC.
...
To compile the generated Fift file `output.fif` further down to TVM bitcode, use the Fift compiler. See the [Fift](/blockchain-basics/languages/fift/overview) for download and usage instructions.
...
[History of FunC](/blockchain-basics/languages/func/changelog)
