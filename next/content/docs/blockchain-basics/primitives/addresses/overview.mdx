---
id: foundations/addresses/overview
...
In the currently used workchains, an account ID is defined as the result of applying a hash function to a [`StateInit`](/blockchain-basics/primitives/messages/deploy) structure that stores initial code and data of a smart contract.
...
Additionally, a 64-bit prefix of an account ID is crucial for the sharding process and delivering messages from one shard to another during [Hypercube Routing](/blockchain-basics/primitives/shards).

## External addresses

External addresses are closely related to [External messages](/blockchain-basics/primitives/messages/overview): ones that originate outside the blockchain or are intended for actors outside it. These messages enable interaction between smart contracts and the external world.
...
- [Internal address formats](/blockchain-basics/primitives/addresses/formats): encoding rules and practical examples.
- [Account status](/blockchain-basics/primitives/status): how addresses evolve (active, frozen, etc.).
