---
id: foundations/addresses/formats
...
For a detection how wallets and other applications use these formats, see the [address workflow](/overview/wallets/wallet-apps/addresses-workflow) page.
...
- `account_id`: a 256-bit identifier that is derived from a smart contract [`StateInit`](/blockchain-basics/primitives/messages/deploy).
...
<Aside
  type="caution"
>
  This format can be applied only to addresses that are described according to the `addr_std` TL-B scheme, see addresses [general info](/blockchain-basics/primitives/addresses/overview) subpage.
</Aside>
...
<Aside>
  The same encoding is used for the so-called armored versions of [public keys](/blockchain-basics/contract-dev/techniques/security)
  and ADNL addresses.
</Aside>
