...
The `AccStatusChange` field indicates whether the account's status changed during this phase. For example, see [account status variety](/blockchain-basics/primitives/status#status-variety).
...
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cskip_no_state`  | The smart contract has no [state](/blockchain-basics/primitives/status#status-variety) and, therefore, no code, so execution is not possible.                                                                                                                        |
| `cskip_bad_state` | Raised in two cases: when the `fixed_prefix_length` [field has an invalid value][fixed_prefix_length] or when the [`StateInit`](/blockchain-basics/primitives/messages/deploy) provided in the incoming message [does not match account’s address][account_address]. |
...
>
  ...
  This topic is outside the scope of this guide, but more information is available in [Shards page](/blockchain-basics/primitives/shards).
</Aside>
...
> See [Fees → Forward fee](/blockchain-basics/primitives/fees#forward-fee) for more info.
