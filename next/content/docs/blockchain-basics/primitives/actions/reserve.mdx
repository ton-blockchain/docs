---
id: foundations/actions/reserve
title: "Reserve coins"
description: ""
---

_Reserving coins_ is one of the actions that a smart contract can perform during the [action phase](/blockchain-basics/primitives/phases#action-phase). In TVM it is implemented via the [`RAWRESERVE`](/blockchain-basics/tvm/instructions#fb02-rawreserve) instruction that takes from the stack an `amount` of Toncoin (in nanotoncoins) to reserve and an integer `mode` specifying a way of the reservation. The reservation action is queued to the _output action list_ of a smart contract, which contains other actions such as _message sends_.
...
1. Check that `mode` has flag `ReserveBounceIfActionFail`:
   - if so, then in case of any failure during reservation, the [bounce phase](/blockchain-basics/primitives/phases#bounce-phase) is initiated.
...
- If the `mode` bitmask has more than first five ones positive bits, then the error [`34`](/blockchain-basics/tvm/exit-codes#34%3A-invalid-or-unsupported-action) is thrown.
- If `mode` has flag `8` but not flag `4`, then the error [`34`](/blockchain-basics/tvm/exit-codes#34%3A-invalid-or-unsupported-action) is thrown.
- If after step 3, `reserve` is negative, then the error [`34`](/blockchain-basics/tvm/exit-codes#34%3A-invalid-or-unsupported-action) is thrown.
- Passing a negative `amount` also results in the same error.
- If after step 4, `reserve` is greater than `remaining_balance`, then the error [`37`](/blockchain-basics/tvm/exit-codes#37%3A-not-enough-toncoin) ("Not enough Toncoin") is thrown.
- Some problems with unpacking the reserve action cell.
- A problem related to extra-currency.

If the action had flag `16`, i.e., `ReserveBounceIfActionFail`, then in case of any of the above errors the transaction enters the [bounce phase](/blockchain-basics/primitives/phases#bounce-phase).
