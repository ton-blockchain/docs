...
Fees in TON align with the [execution phases](/blockchain-basics/primitives/phases) of a transaction:

- Storage fees are charged in the [storage phase](/blockchain-basics/primitives/phases#storage-phase).
- Compute fees are charged in the [compute phase](/blockchain-basics/primitives/phases#compute-phase).
- Forward and action fees are charged in the [action](/blockchain-basics/primitives/phases#action-phase) and [bounce phases](/blockchain-basics/primitives/phases#bounce-phase).
...
- Storage fees are set in [config parameter 18](/blockchain-basics/primitives/config#param-18:-storage-prices).
- Compute fees are set in [config parameters 20 and 21](/blockchain-basics/primitives/config#param-20-and-21:-gas-prices).
- Forward, import, and action fees are set in [config parameters 24 and 25](/blockchain-basics/primitives/config#param-24-and-25:-message-price).
...
All computation is measured in gas units. A TVM operation typically has a fixed gas cost, but that is [not always the case](/blockchain-basics/tvm/gas). Network configuration defines gas prices; users cannot override them.
...
Action fee is the portion of `fwdFee` granted to the validator of the message's source [shard](/blockchain-basics/primitives/shards#blockchain-sharding). The remaining `fwdFee - actionFee` amount goes to the validator of the destination shard.

Action fee exists only for [internal messages](/blockchain-basics/primitives/messages/internal#internal-messages).

```cpp
action_fee = floor(fwd_fee * first_frac / 2^16)
```

Starting with Global Version 4, a failed [SENDMSG action](/blockchain-basics/primitives/actions/send#send-message) incurs a penalty proportional to the attempted message size. It is calculated as:
