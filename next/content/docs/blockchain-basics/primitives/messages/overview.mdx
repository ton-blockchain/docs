---
id: foundations/messages/overview
title: "Overview"
description: ""
---
...
- `init` is an optional field of [`StateInit`](/blockchain-basics/primitives/messages/deploy) type used to initialize the contract state, stored either in-place or in a ref of the cell with a message.
...
- `info` describes the type of the message, and fields specific to messages of this type:
  - [Internal messages](/blockchain-basics/primitives/messages/internal) are messages from one contract to another.
  - [Incoming external](/blockchain-basics/primitives/messages/external-in) messages are messages from the outside world to the contract.
  - [Outbound external](/blockchain-basics/primitives/messages/external-out) messages are messages from the contract to the outside world. They can be interpreted as logs.
...
- When an incoming external message is processed, a transaction will be created only if the contract [accepts](/blockchain-basics/primitives/messages/external-in#accept-message) the message.
...
The `state_update` field is a [Merkle update](/blockchain-basics/primitives/serialization/merkle-update) that captures the difference between the previous and current state of the account. The [contract status](/blockchain-basics/primitives/status) before and after the transaction is duplicated for convenience.

Other fields, such as `in_msg`, `out_msgs`, and `outmsg_cnt`, relate to the [messages](/blockchain-basics/primitives/messages/overview) processed and emitted during the transaction. A trace may start from a transaction rather than a message, in which case `in_msg` is `Nothing`. However, a trace cannot continue without messages: if it contains `n` transactions it must contain at least `n-1` messages. Consequently, a trace can include at most one non-ordinary transaction, because messages can trigger only an [ordinary transaction](/blockchain-basics/primitives/messages/ordinary-tx).

The `total_fees` field stores the total number of [fees](/blockchain-basics/primitives/fees) paid by the transaction. These fees may eventually be paid in extra currencies, but that feature is not implemented yet.
