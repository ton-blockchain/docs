---
id: foundations/messages/internal
...
The process of transforming the relaxed version of the structure into the strict version occurs in the action phase and is performed by the handler of [send message actions](/blockchain-basics/primitives/actions/send). This action rewrites [some fields](/blockchain-basics/primitives/actions/send#message-normalization) of the structure with correct values and validates the `StateInit` field against the `StateInitWithLibs` type.

## Deploy

It is possible to simultaneously send both the payload in the `body` field and the `StateInit` in the `init` field to [initialized the contract](/blockchain-basics/primitives/messages/deploy) and process the payload in the same transaction.

## Message value

The message may set `value` field that tells to transfer Toncoin and extra-currency to the recipient of the message. The actually transferred amount depends on the value of this field and a [send mode](/blockchain-basics/primitives/messages/modes).
...
- and when either
  - a contract throws an error from the [compute phase](/blockchain-basics/primitives/phases#compute-phase) during message processing, and the state was not committed by [`COMMIT`](/blockchain-basics/tvm/instructions#f80f-commit) TVM instruction,
  - or an action fails in the [action phase](/blockchain-basics/primitives/phases#action-phase) with the `+16` flag enabled: [`SendBounceIfActionFail`](/blockchain-basics/primitives/messages/modes) in case of message sends or [`ReserveBounceIfActionFail`](/blockchain-basics/primitives/actions/reserve#modes) in case of reservations.
