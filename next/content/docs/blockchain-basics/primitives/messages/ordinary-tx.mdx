---
id: foundations/messages/ordinary-tx
title: "Ordinary transactions"
description: ""
---

Ordinary [transactions](/blockchain-basics/primitives/messages/overview#transactions) are the most common type of transaction because they result from processing [incoming external](/blockchain-basics/primitives/messages/external-in) and [internal](/blockchain-basics/primitives/messages/internal) messages. They can be produced only while processing such messages. The phases of a transaction are described in detail on the [dedicated page](/blockchain-basics/primitives/phases).
...
- `compute_ph` holds the result of TVM execution on the contract. If this phase is [skipped](/blockchain-basics/primitives/phases#when-the-compute-phase-is-skipped), for example, because there are no funds or no state on the destination account, the field stores the skip reason instead.
...
- `destroyed` indicates whether the account was deleted in the action phase by using [`SendDestroyIfZero`](/blockchain-basics/primitives/messages/modes). If the contract was deleted in the action phase, this flag is `true`.
