...
You can view the parameter values in the [current configuration](https://tonviewer.com/config), and the method of writing them into [cells](/blockchain-basics/primitives/serialization/boc) is outlined in the [`block.tlb`](https://github.com/ton-blockchain/ton/blob/master/crypto/block/block.tlb) file in [TL-B](/blockchain-basics/languages/tl-b/overview) format.

<Aside>
  Configuration values are TL-B typed cells serialized into [Bags of Cells (BoC)](/blockchain-basics/primitives/serialization/boc).
</Aside>
...
This parameter is the address of the [elector smart contract](/blockchain-basics/primitives/system), responsible for appointing validators, distributing rewards, and voting on changes to blockchain parameters.
...
This parameter defines the cost of filing complaints about the incorrect operation of validators in the [elector smart contract](/blockchain-basics/primitives/system).
...
<Aside>
  You can find more about `gas_credit` and other parameters in the section of external messages [here](/blockchain-basics/tvm/instructions#external-messages).
</Aside>
...
More details about precompiled contracts are on [this page](/blockchain-basics/primitives/precompiled).
...
<Aside>
  Validators enforce TL-B validity only for configuration parameters with non-negative indices. Values with negative indices are not validated against a specific `ConfigParam i` type. See the explanation in [Configuration parameters](/blockchain-basics/primitives/config) section.
</Aside>
...
- The original descriptions are present, but they may be limited, in the documents:
  - [The Open Network Whitepaper](/blockchain-basics/whitepapers/ton)
  - [Telegram Open Network Blockchain](/blockchain-basics/whitepapers/tblkch)
