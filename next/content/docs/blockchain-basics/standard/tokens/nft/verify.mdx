...
1. Read the NFT item index from the item contract via [get\_nft\_data()](/blockchain-basics/standard/tokens/nft/api#get-nft-data).
1. Query the collection with [get\_nft\_address\_by\_index(index)](/blockchain-basics/standard/tokens/nft/api#get-nft-address-by-index).
1. If the returned address equals the NFT item address, the item belongs to the collection.

### High level

There is an [API method](/applications/api/toncenter/v3/nfts/get-nft-items) that performs this verification off‑chain with a single request. Provide the addresses of the NFT item and NFT collection:
