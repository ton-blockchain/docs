...
- Generate a mnemonic; wallet will be still in [`nonexist`](/blockchain-basics/primitives/status#status-variety) status.
- Derive the public key and the wallet address out of it.
- Put some funds to the address; wallet account will move to [`uninit`](/blockchain-basics/primitives/status#status-variety) status.
- Deploy code of wallet contract to that address; account will move to [`active`](/blockchain-basics/primitives/status#status-variety) status.
...
A wallet account's address is [derived](/blockchain-basics/primitives/addresses/derive) from [`StateInit`](/blockchain-basics/primitives/messages/deploy) that stores in its `data` field two values:

- `public_key`, dependent on the [mnemonic](/blockchain-basics/standard/wallets/mnemonics);
- `wallet_id`, a special [salt-like](https://en.wikipedia.org/wiki/Salt_%28cryptography%29) field, dependent on the wallet contract type and the `network_id`.
...
| Network | [V4R2](/blockchain-basics/standard/wallets/v4) | [V5](/blockchain-basics/standard/wallets/v5) |
| ------- | ---------------------------- | -------------------------- |
...
This wallet contract instance can be used to [send messages](/blockchain-basics/standard/wallets/v5-api) to the blockchain.
...
- [Jetton transfers](/blockchain-basics/standard/tokens/jettons/api#forward-payload-formats), where the wallet forwards an internal message to the token wallet along with the comment payload;
- [NFT transfers](/blockchain-basics/standard/tokens/nft/api#forward-payload-formats), where the comment travels in the same forwarding message that moves the ownership record.
...
1. Store the text as a [byte string](/blockchain-basics/standard/tokens/metadata#snake-data-encoding) (UTF-8 encoded).
1. Send the internal message along with the desired Toncoin, [Jettons](/blockchain-basics/standard/tokens/jettons/how-it-works), or [NFT](/blockchain-basics/standard/tokens/nft/how-it-works) payload.
