...
This is TL-B for V5 wallet actions, it includes some [complex TL-B patterns](/blockchain-basics/languages/tl-b/complex-and-non-trivial-examples). You can also find it on [GitHub](https://github.com/ton-blockchain/wallet-contract-v5/blob/321186127e8cc5e395ad3b2f1870839237c56f5f/types.tlb), in the wallet repo.
...
Message structure for V5 wallet contract is quite cumbersome and hard to read, it's made for optimal (de-)serialization and not optimized for understanding. It is described in [TL-B](/blockchain-basics/languages/tl-b/overview) languages and includes snake-cell pattern. We will try to get a grip of it by breaking down core data structures and how they are used.
...
Now we have to deal with basic action serialization. However, since these are the messages that are described in [`block.tlb`](/blockchain-basics/languages/tl-b/overview), we can use contract-agnostic code from serialization library to store them.
...
You can read more about where to obtain them and what there purpose is in [How it works](/blockchain-basics/standard/wallets/how-it-works) section.
...
You can send this BoC to the network in any convenient way, e.g. with API provider like [TonCenter /message](https://toncenter.com/api/v3/index.html#/api%2Fv2/api_v3_post_v2_message) or with your own [liteserver](/blockchain-basics/nodes/overview).
