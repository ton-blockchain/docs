...
You can only send funds to the predefined owner address or do operations with [Elector](/blockchain-basics/primitives/system#elector) (new stake, recover stake request, new complaint, and vote for a complaint) and [Config](/blockchain-basics/primitives/system#config) (vote for specific proposal to change config) contracts.
...
1. Send "vote for a configuration proposal" message to the [Config contract](/blockchain-basics/primitives/system#config).
1. Send "new stake", "recover stake request", "new complaint", or "vote for a complaint" messages to the [Elector contract](/blockchain-basics/primitives/system#elector).

It is allowed to use [message modes](/blockchain-basics/languages/func/stdlib#send-raw-message) 128, 64, 2, and 1. Mode 2 (ignore errors) is always enabled when sending messages from this wallet.
