---
id: contract-dev/techniques/on-chain-jetton-processing
...
<Aside type="note">
  Understand [the jetton architecture](/blockchain-basics/standard/tokens/jettons/overview) before reading this article.
</Aside>
...
If the jetton type is known in advance, [calculate the corresponding jetton wallet address off-chain](/blockchain-basics/standard/tokens/jettons/find) and set it via a message after deployment. It is impossible to hardcode the jetton wallet address in the contract’s [`StateInit`](/blockchain-basics/primitives/messages/deploy) because the wallet address depends on the contract address and creates a circular dependency. This is the most efficient approach, because it only requires one address comparison.
...
With the state of a jetton minter, the [RUNVM](/blockchain-basics/tvm/instructions#db4-runvm) instruction can emulate the execution of the `get_wallet_address` [get-method](/blockchain-basics/tvm/get-method) to derive the wallet address for any owner.

Use the following helper function. It relies on [Fift](/blockchain-basics/languages/fift/overview) because it's impossible to assign a type to `c7`. It executes the `get_wallet_address` method of a jetton minter on-chain, and calculates the corresponding wallet address for a given owner.
...
fun calculateJettonWallet(owner: address, jettonData: cell, jettonCode: cell, jettonMinter: address): address asm """
    ...
    // https://docs.ton.org/blockchain-basics/tvm/registers#c7--environment-information-and-global-variables
    0 INDEX
    SWAP
    8 SETINDEX
    SWAP
...
Minter address and `StateInit` will match if the jetton minter was not deployed with [a vanity contract](/blockchain-basics/contract-dev/techniques/vanity). The logic for the contract that handles `JettonNotify` messages is thus:
...
A full [state proof](/blockchain-basics/primitives/proofs/overview) is required to prove that the state is currently at the jetton minter address.
