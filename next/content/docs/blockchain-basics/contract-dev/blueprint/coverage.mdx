...
<Aside>
  To understand the TVM assembly output, read [TVM](/blockchain-basics/whitepapers/tvm).
</Aside>

## Limitations

Note that when code of other contracts is stored directly in the code of the contract ([Tact](/blockchain-basics/languages/tact) does that automatically if a contract system does not contain circular dependencies), that affects the overall code coverage percentage.
