---
id: contract-dev/blueprint/first-smart-contract
title: "Write your first smart contract with Blueprint"
...
This tutorial covers building, deploying, and interacting with a smart contract on TON from start to finish using [Blueprint](/contract-dev/blueprint/).
...
- Installed [TON wallet](/overview/wallets/wallet-apps/tonkeeper) with [Toncoin on testnet](/overview/wallets/wallet-apps/get-coins)
...
<Steps>
  ...
  >
    Use the [Blueprint](/blockchain-basics/contract-dev/blueprint/overview) development toolkit for smart contracts. Start a new project with:
...
A smart contract is a program stored on <Tooltip tip="A distributed database that many computers maintain together." cta="Learn more" href="/blockchain-basics/primitives/shards">TON blockchain</Tooltip> and executed by the <Tooltip tip="The “computer” which executes smart contracts on TON blockchain." cta="Learn more" href="/blockchain-basics/tvm/overview">TVM</Tooltip>.
On-chain, every contract consists of two components:

- Code — compiled [TVM instructions](/blockchain-basics/tvm/instructions), defines the contract's logic.
- Data — persistent state, stores information between interactions.

Both are stored at a specific [address](/blockchain-basics/primitives/addresses/overview) on TON blockchain, which is a unique identifier for each smart contract. Smart contracts interact with each other only through [messages](/blockchain-basics/primitives/messages/overview).
...
- [Get methods](/blockchain-basics/tvm/get-method) are read-only functions that return contract data without modifying state. Example: a get method that returns the current `counter` value.

  Due to the [TON architecture](/overview/learn-more/from-ethereum#on-chain-get-methods), get methods cannot be called from other contracts. Inter-contract communication uses **messages** only.
...
The contract uses [**Tolk**](/blockchain-basics/tolk/overview) language.

The TON ecosystem provides editor plugins with syntax support for IDEs and code editors. View them [here](/blockchain-basics/contract-dev/ide/vscode).
...
<Steps>
  ...
  >
    ...
    To define contract storage, store the `counter` value. Tolk makes it simple with <Tooltip tip="A structure is a composite data type that groups named fields, each having its own type, into one unit." cta="Learn more" href="/blockchain-basics/tolk/overview">structures</Tooltip>:
    ...
    Structures serialize and deserialize automatically into [cells](/blockchain-basics/primitives/serialization/cells), the storage unit in TON. The `fromCell` and `toCell` functions handle conversion between structures and cells.
  </Step>
...
<Steps>
  ...
  >
    ...
    - The contract [address](/blockchain-basics/primitives/addresses/overview) is derived deterministically from `code` and `data` using `contractAddress`.
  ...
  >
    ...
    For flags and options, see the [Blueprint deployment guide](/blockchain-basics/contract-dev/blueprint/deploy).
  </Step>
  ...
  >
    <Aside
      ...
      type="note"
    >
      If a wallet is not installed, check the [wallet section](/overview/wallets/wallet-apps/tonkeeper) to select and install a wallet before deploying the contract. Make sure the wallet is funded with [Toncoin on the testnet](/overview/wallets/wallet-apps/get-coins).
    </Aside>
    ...
    The link opens the contract on [Tonviewer](/overview/infrastructure/explorers/tonviewer), a [blockchain explorer](/overview/infrastructure/explorers/overview) showing transactions, messages and [account states](/blockchain-basics/primitives/status).
...
<Steps>
  ...
  >
    ...
    1. Blueprint connects to the wallet using the [TON Connect](/applications/ton-connect/overview) protocol.
