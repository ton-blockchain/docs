---
id: contract-dev/blueprint/debug
title: "Debugging smart contracts with Blueprint"
...
Errors in smart contracts can produce an [exit code](/blockchain-basics/tvm/exit-codes), often indicating a bug in the contract. Use debugging methods to locate and fix the issue.
...
There are three TVM debug [instructions](/blockchain-basics/tvm/instructions#fe-debug): `DUMPSTK`, `STRDUMP`, and `DUMP`.
...
Of all [verbosity levels](/blockchain-basics/contract-dev/blueprint/testing/reference#verbosity-2), the two are the most useful:
...
The contract attempts to load a 64-bit integer from the slice using `LDU 64`. Since there is not enough data, execution stops with [exit code 9](/blockchain-basics/tvm/exit-codes#9%3A-cell-underflow).
...
<Aside type="tip">
  ...
  Sometimes several instructions are implemented within a single `exec_*` method. For example, [LDU](/blockchain-basics/tvm/instructions#d3-ldu) (`load_uint`), [LDI](/blockchain-basics/tvm/instructions#d2-ldi) (`load_int`) and it's preload versions (`preload_uint`and`preload_int`).
...
However, the slice contains only `725` bits, of which `711` bits and both [references](/blockchain-basics/primitives/serialization/cells) have already been read. The contract attempted to read `64` more bits, but the slice did not contain enough remaining data.
...
- [TxTracer Sandbox](/blockchain-basics/tvm/tools/txtracer) — requires a custom `@ton/sandbox` package; runs in the browser.

Also, these tools allow to explore logs of each transaction.

## Inspect BoC returned by APIs

When interacting with the blockchain using APIs, responses may include serialized cells in [BoC (bag of cells)](/blockchain-basics/primitives/serialization/boc), TON's standard cell serialization format.
...
Even when a contract executes successfully (exit code = `0`) with no errors, the actions may not produce the expected on-chain result. [TVM Retracer](/blockchain-basics/tvm/tools/retracer) replays the transaction and displays VM-level execution in detail.

### Scenarios for retracing

- All [execution phases](/blockchain-basics/primitives/phases) complete without errors, yet the expected outcome is missing.
...
1. Obtain the transaction hash from a [blockchain explorer](/overview/infrastructure/explorers/overview).
1. Open [TVM Retracer](/blockchain-basics/tvm/tools/retracer) and enter the transaction hash.
