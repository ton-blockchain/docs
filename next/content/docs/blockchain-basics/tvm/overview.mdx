...
- **Stack**. A regular [stack data structure](https://en.wikipedia.org/wiki/Stack_%28abstract_data_type%29). The vast majority of instructions `pop()` operands from the top and `push()` results back.
...
1. Three more values `s2`, `s3`, `s4` [are pushed](/blockchain-basics/tvm/initialization#external%2Finternal-message) by TVM onto a stack. They won't be used in the example. After execution finishes, they'll still be on the stack, and will be silently ignored.
