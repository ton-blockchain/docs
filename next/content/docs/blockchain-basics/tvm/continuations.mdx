...
_Call_ is a special type of jump, which also saves `cc` to `c0`, so the callee can pass execution back to the caller. That is how [`CALLREF`](/blockchain-basics/tvm/instructions#db3c-callref) works.
