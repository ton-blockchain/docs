---
id: ecosystem/appkit/get-started/streaming
title: "Streaming"
sidebarTitle: "Streaming"
description: ""
---

Streaming is the live channel of AppKit: a per-network subscription layer that turns provider events into balance, jetton, and transaction updates pushed to the app. Use it instead of polling whenever a UI needs to reflect chain changes without a user-driven refresh.
...
You need a streaming provider registered on the AppKit instance before any watcher will fire. See [Providers](/ecosystem/appkit/get-started/providers#provider-types) for the registration pattern, and use `hasStreamingProvider(appKit, network)` from `@ton/appkit` as the runtime check before subscribing.

## Mount global watchers

Mount the `useWatch*` hooks once at a high level — the router, the app shell, or a layout component — so the underlying cache stays current for every consumer below. Calling a hook with no arguments simply keeps the cache warm; passing an `onChange` callback is for one-shot side effects (toast, log, navigate). The official example app calls `useWatchBalance()`, `useWatchJettons()`, and `useWatchTransactions({ onChange })` from its router for exactly this reason.
...
- See [How to](/ecosystem/appkit/howto) for task-level patterns that combine sends with settlement tracking.
