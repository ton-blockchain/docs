---
id: ecosystem/wallet-apps/tonkeeper
title: "Tonkeeper"
description: ""
---
...
It supports regular [wallets](/blockchain-basics/standard/wallets/how-it-works), [Jettons](/blockchain-basics/standard/tokens/jettons/overview), [NFTs](/blockchain-basics/standard/tokens/nft/overview), and [TON Connect](/ecosystem/ton-connect). Its github repository can be found [here](https://github.com/tonkeeper).
...
- Generate a mnemonic (a key). It uniquely determines wallet's address, but the wallet doesn't exist on blockchain yet, i.e. is in [`nonexist`](/blockchain-basics/primitives/status#status-variety) status.
- Send some funds to the wallet's account. Now it will be in [`uninit`](/blockchain-basics/primitives/status#status-variety) status, i.e. already with some balance on it, but without any code yet.
- Deploy wallet's code to this address. Some of these funds will be used to pay for the deploy process. Now the wallet is in [`active`](/blockchain-basics/primitives/status#status-variety) status, and can be used for any purpose.
...
1. Save 24 words of the [mnemonic](/blockchain-basics/standard/wallets/mnemonics).
...
1. The main interface of the app should now appear, indicating that the testnet is used. Also address of the testnet wallet in the [user-friendly format](/blockchain-basics/primitives/addresses/formats#user-friendly-format) starts with `k` or `0`.
...
To get free coins on testnet, follow [the guide](/overview/wallets/wallet-apps/get-coins).
...
Use a [blockchain explorer](/overview/infrastructure/explorers/overview) to inspect the account. For **Testnet**, use [Tonviewer Testnet](https://testnet.tonviewer.com/).
...
By default, Tonkeeper creates wallets with the [Wallet v5](/blockchain-basics/standard/wallets/v5) code deployed on them. To switch [wallet contract version](/blockchain-basics/standard/wallets/comparison) to [v4](/blockchain-basics/standard/wallets/v4):

1. On the main menu, tap the gear icon <kbd><Icon icon="settings" size={16} /></kbd> in the upper-right corner.
