...
serialization format for cells. Commonly used in code. [Article](/ecosystem/primitives/serialization/boc).
...
a program connecting various blockchains to transfer tokens and data from one network to another. [Article](/infrastructure/oracles/overview).
