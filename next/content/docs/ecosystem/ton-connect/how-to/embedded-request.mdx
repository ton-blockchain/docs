...
To hide wallets that do not advertise `EmbeddedRequest` from the modal, set `walletsRequiredFeatures.embeddedRequest` on the `TonConnectUI` instance. See [Filter wallets by required features](/ecosystem/ton-connect/how-to/filter-wallets) for the full filter shape.
...
- [Sign data](/ecosystem/ton-connect/how-to/sign-data)
