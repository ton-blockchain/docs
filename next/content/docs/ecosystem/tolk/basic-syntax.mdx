...
[Imports](/ecosystem/tolk/syntax/imports) must appear at the top of the file:
...
A [struct](/ecosystem/tolk/syntax/structures-fields) `Point` holding two 8-bit integers:
...
- Fields can use any [types](/ecosystem/tolk/types/list-of-types): numeric, cell, union, and others.
- Fields can define default values: `x: int8 = 0`.
- Fields can be `private` and `readonly`.
- Structs can be generic: `struct Wrapper<T> { ... }`.

If all fields are serializable, a struct can be [automatically serialized](/ecosystem/tolk/features/auto-serialization):
...
A [function](/ecosystem/tolk/syntax/functions-methods) that returns the sum of two integers:
...
A function declared as `fun <receiver>.name(...)` is a [method](/ecosystem/tolk/syntax/functions-methods).
...
Within functions, [variables](/ecosystem/tolk/syntax/variables) are declared with `val` or `var` keywords. The `val` keyword declares an immutable variable that can be assigned only once:
...
To group integer constants, [enums](/ecosystem/tolk/types/enums) are useful.

## Value semantics

Tolk follows value semantics: assignments create independent copies, and function calls do not [mutate](/ecosystem/tolk/syntax/mutability) arguments unless explicitly specified.
...
In [conditions](/ecosystem/tolk/syntax/conditions-loops), `if` is a statement. `else if` and `else` blocks are optional.
...
[Union types](/ecosystem/tolk/types/unions) allow a variable to hold one of possible types. They are typically handled by `match`:
...
Union types are commonly used when [handling incoming messages](/ecosystem/tolk/features/message-handling).

## While loop

Tolk does not have a `for` loop; use [`while` loop](/ecosystem/tolk/syntax/conditions-loops) for repeated execution.
...
The `try-catch` statement is supported for [exceptions](/ecosystem/tolk/syntax/exceptions), although it is not commonly used in contracts.
...
[Arrays](/ecosystem/tolk/types/tuples) are dynamically sized containers created with `[...]`:
...
To iterate, [maps](/ecosystem/tolk/types/maps) can be used:
...
To [construct and send a message](/ecosystem/tolk/features/message-sending), a message body is typically represented by a structure. For example, `RequestedInfo`:
...
[Contract getters](/ecosystem/tolk/features/contract-getters) or get-methods are declared with `get fun`:
