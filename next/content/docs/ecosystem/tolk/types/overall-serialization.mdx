...
This page assumes prior knowledge of the [TL-B](/ecosystem/languages/tl-b/overview) and [TVM](/ecosystem/tvm/overview). It serves as a concise low-level reference.
...
Input values are validated during deserialization. For `enum Role: int8`, any input value outside the range of defined enum variants, e.g., \< 0 or > 2, triggers [exception 5](/ecosystem/tvm/exit-codes).
...
- TL-B: [`HashmapE n X`](/ecosystem/languages/tl-b/complex-and-non-trivial-examples#hashmap)
- Stored using `STDICT`
- Loaded using `LDDICT`
