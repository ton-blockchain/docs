...
In the [TVM](/ecosystem/tvm/overview), all enums are represented as integers. At the compiler level, an enum is a distinct type.
...
[`match`](/ecosystem/tolk/syntax/pattern-matching) on enums requires coverage of all cases:
...
<Aside
  type="caution"
>
  Operator `as` can produce invalid values, for example `100 as Color`. In this case, operator `==` returns `false`, and an exhaustive `match` throws exception [5](/ecosystem/tvm/exit-codes#5%3A-integer-out-of-expected-range).
</Aside>
...
Every enum is [backed by TVM](/ecosystem/tolk/types/overall-tvm-stack) `INT` and [serialized](/ecosystem/tolk/types/overall-serialization) as `(u)intN`, where `N` is:
