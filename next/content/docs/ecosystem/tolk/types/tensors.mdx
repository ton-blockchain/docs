...
Tensors represent ordered collections of values and are written in the form `(T1, T2, ...)`. They occupy multiple [TVM stack entries](/ecosystem/tolk/types/overall-tvm-stack) sequentially and are [serialized](/ecosystem/tolk/types/overall-serialization) in the same order. For example, `(int, slice)` represents two values following one another.

Tensors are anonymous [structures](/ecosystem/tolk/types/structures) and behave identically. Large tensors are impractical — use structures whenever possible.

<Aside
  type="caution"
>
  In many general-purpose languages, syntax `(A, B, C, ...)` is used for _tuples_, not tensors, which are a different type. However, in TON, a [tuple](/ecosystem/tolk/types/tuples) is a distinct TVM primitive that has its own syntax `[a, b, ...]` and semantics.
