...
Every smart contract has an [address](/ecosystem/primitives/addresses/overview) used for all on-chain interactions.
...
When [serialized](/ecosystem/tolk/types/overall-serialization), values of `address` type occupy 267 bits:
...
Compare addresses using `==` or `!=`. Internally, an [address is represented as a raw `slice` without references](/ecosystem/tolk/types/overall-tvm-stack), so `==` and `!=` test for bits equality.
