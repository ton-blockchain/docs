...
Without `self` parameter, a [method](/ecosystem/tolk/syntax/functions-methods) will be static. By default, `self` is immutable. The form `mutate self` enables mutation.
...
[Structure syntax](/ecosystem/tolk/syntax/structures-fields) includes:
...
Fields are placed on the [stack](/ecosystem/tolk/types/overall-tvm-stack) sequentially and [are serialized](/ecosystem/tolk/types/overall-serialization) in the same order. If a struct has a prefix, it is written first.

Serialization behavior can be overridden by defining [custom serializers](/ecosystem/tolk/features/auto-serialization#custom-serializers-for-custom-types).
