...
When handling a message, some values can be represented in multiple valid forms. [Union types](/ecosystem/tolk/types/unions) allow expressing these alternatives explicitly, so the same message-handling logic can accept and correctly process any of them.
...
When sending a message to the wallet, it may not yet exist on-chain. In this case, the message must include the wallet's code and initial data. The message destination is therefore defined by the wallet's [`StateInit`](/ecosystem/primitives/messages/deploy#deploy-message).
...
- This closeness is defined by [`shard_prefix`](/ecosystem/primitives/shards#sharding-process).
...
A [message](/ecosystem/primitives/messages/overview) is a cell. Its body can either be embedded into the same cell or placed into a separate cell and referenced.
...
This is implemented using [generics](/ecosystem/tolk/types/generics):
...
A message created with `createMessage()` is typically sent using [`msg.send(mode)`](/ecosystem/primitives/messages/modes#sending-modes).
...
`createExternalLogMessage` follows the same general model as `createMessage`. [External outgoing messages](/ecosystem/primitives/messages/external-out) do not support bounce behavior, attached Toncoin, or related options, so the set of available fields is different. External messages are used only for emitting logs intended for indexers.
