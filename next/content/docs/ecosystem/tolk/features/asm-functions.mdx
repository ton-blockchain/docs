...
Functions in Tolk can be defined using assembler code. It's a low-level feature that requires understanding of stack layout, [Fift](/ecosystem/languages/fift/overview), and [TVM](/ecosystem/tvm/overview).

## Standard functions

Standard functions are `asm` wrappers. Many functions from the [standard library](/ecosystem/tolk/features/standard-library) are translated to the Fift assembler directly. For example, TVM has a `HASHCU` instruction, which is "calculate hash of a cell". It pops a cell from the stack and pushes an integer in the range 0 to 2<sup>256</sup>-1. Therefore, the method `cell.hash` is defined:
...
The `mutate` keyword, which makes a parameter [mutable](/ecosystem/tolk/syntax/mutability), implicitly returns updated values through the stack in both regular and `asm` functions.
