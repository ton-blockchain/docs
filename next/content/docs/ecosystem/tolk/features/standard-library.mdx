...
Several methods operate on constant [string](/ecosystem/tolk/types/strings) literals. They require constant inputs and are evaluated at compile time.
...
Array-related methods allow interacting with [`array<T>`](/ecosystem/tolk/types/tuples) — dynamically sized containers of up to 255 elements backed by TVM tuples.

#### Creating arrays

Use `[...]` to create arrays. See the [arrays](/ecosystem/tolk/types/tuples#creating-arrays-with) page for details.
...
These functions accept and return integers unless stated otherwise. All [integers](/ecosystem/tolk/types/numbers) are 257-bit.
...
Returns `coins` — the balance of the smart contract in nanotoncoins at the start of the [compute phase](/ecosystem/tvm/exit-codes#compute-phase).
...
Returns `cell` — the persistent contract storage cell. Typically, its result is used as [`Storage.fromCell()`](/ecosystem/tolk/features/contract-storage).

#### `contract.setData(cell)`

Sets the persistent contract storage. Typically, the argument is [`storageObject.toCell()`](/ecosystem/tolk/features/contract-storage).

#### `contract.getCode`

Returns `cell` — the smart contract code stored in TVM [register `c7`](/ecosystem/tvm/registers).
...
Prefer [automatic serialization](/ecosystem/tolk/features/auto-serialization) over manual slice parsing.
...
Prefer [automatic serialization](/ecosystem/tolk/features/auto-serialization)
over manual builder construction.

### Address manipulation primitives

Methods for [`address` and `any_address`](/ecosystem/tolk/types/address) are suggested by the IDE after a dot.
...
Returns `int8` — the workchain from an [internal address](/ecosystem/primitives/addresses/formats).
...
To represent [internal or none](/ecosystem/tolk/types/address) , use `address?` and `null`.
...
Casts `any_address` to `address`, with a runtime check that the value is internal. To skip the check, use an [unsafe cast](/ecosystem/tolk/types/type-checks-and-casts):
`addr = myAny as address`.

### Reserving Toncoin on the contract balance

The standard library provides several `RESERVE_MODE_XXX` constants for [reserve modes](/ecosystem/primitives/actions/reserve), as well as the following functions.
...
Stdlib provides several `SEND_MODE_XXX` constants for [sending modes](/ecosystem/primitives/messages/modes).

[Message cell composition](/ecosystem/tolk/features/message-sending) is encapsulated by the `createMessage` function.
...
[String](/ecosystem/tolk/types/strings) utilities: `StringBuilder` for concatenation, `string.calculateLength`, `string.equalTo`, and other.
...
Functions for loading and parsing [exotic cells](/ecosystem/primitives/serialization/cells), including library references.
...
Low-level API for working with TVM dictionaries. Their use is not recommended; prefer [built-in maps](/ecosystem/tolk/types/maps).
...
This is done for [optimization](/ecosystem/tolk/features/compiler-optimizations) purposes. The compiler generates different assembler code depending on the arguments. For example, if `len` is a constant value such as `32`, `loadUint` is translated to `32 LDU`. If `len` is a variable, it is taken from the stack and the `LDUX` instruction is emitted.
