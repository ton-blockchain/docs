...
All data in TON, such as messages, storage, etc., is represented as [cells](/ecosystem/primitives/serialization/cells). The Tolk type system is designed to express cell contents, enabling auto-serialization through `fromCell` and `toCell`:
...
A `struct` can define [a serialization](/ecosystem/tolk/types/overall-serialization) prefix. 32-bit prefixes are commonly used as opcodes for incoming and outgoing messages:
...
- [9 – cell underflow](/ecosystem/tvm/exit-codes#9%3A-cell-underflow);
- [5 – out of range](/ecosystem/tvm/exit-codes#5%3A-integer-out-of-expected-range).
...
A common mistake is using `int`. [It cannot be serialized](/ecosystem/tolk/types/numbers#stack-layout-and-serialization); instead, use `int32`, `uint64`, etc.
...
Auto-serialization can be applied when [sending messages](/ecosystem/tolk/features/message-sending) to other contracts:
...
Tolk provides a special keyword [`lazy`](/ecosystem/tolk/features/lazy-loading) for use with auto-deserialization.
