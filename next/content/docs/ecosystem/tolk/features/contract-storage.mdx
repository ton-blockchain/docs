...
Contract storage is a [`struct`](/ecosystem/tolk/types/structures), serialized into persistent blockchain data. Tolk does not enforce any specific storage layout.
...
- `fun Storage.f(self)` defines an instance [method](/ecosystem/tolk/syntax/functions-methods).
- `T.fromCell()` deserializes a cell into `T`, and [`obj.toCell()`](/ecosystem/tolk/features/auto-serialization#unpackoptions-and-packoptions) packs it back into a cell.
- The [`lazy`](/ecosystem/tolk/features/lazy-loading) operator does the parsing on demand.
- [`contract.getData()`](/ecosystem/tolk/features/standard-library) fetches persistent data.

## Default values

In TON, a contract’s address depends on its initial storage when it is [created on-chain](/ecosystem/primitives/addresses/derive#on-chain-—-simple). Assign default values to fields that must be defined at deployment:
...
This behavior is not related to [nullable types](/ecosystem/tolk/types/nullable). Nullable values such as `int8?` or `cell?` serialize with an explicit null marker 0 bit. In this case, fields are absent entirely, and no extra bits appear in serialization.
...
When this pattern is used, declare both shapes in the [`contract` declaration](/ecosystem/tolk/features/contract-abi) so client tools can compute the deployment address correctly:
...
Different shapes with missing fields can also be expressed using [generics](/ecosystem/tolk/types/generics) and the [void type](/ecosystem/tolk/types/void-never).
