---
id: tolk/features/compiler-optimizations
...
For unions, [createMessage](/ecosystem/tolk/features/message-sending) is lightweight. The compiler generates all `IF-ELSE` and `STU`, but during compile-time analysis, these instructions resolve to constants because all types are known at compile time. The resulting code flattens into `PUSHINT` and `STSLICECONST`.
...
The [`lazy` keyword](/ecosystem/tolk/features/lazy-loading) loads only the required fields from a cell or slice:
...
The Tolk compiler outputs the Fift assembler. Fift generates the bitcode. Projects built on [Blueprint](/ecosystem/contract-dev/blueprint/overview) use `tolk-js`, which invokes Tolk and then Fift.
