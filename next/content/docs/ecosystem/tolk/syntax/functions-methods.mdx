...
The same convention has long been allowed for [`void`-typed struct fields](/ecosystem/tolk/types/void-never#void-typed-fields).
...
To allow modifications, declare [`mutate self`](/ecosystem/tolk/syntax/mutability) explicitly.
...
The [compiler automatically inlines small functions](/ecosystem/tolk/features/compiler-optimizations) in-place when possible. For example:
...
Tolk supports declaring low-level [assembler functions](/ecosystem/tolk/features/asm-functions) with embedded Fift code:
...
Tolk supports first-class functions: [they can be passed as callbacks](/ecosystem/tolk/types/callables).
...
Lambdas can capture outer variables by value, becoming [closures](/ecosystem/tolk/types/callables#closures-lambdas-with-captures).
