...
Methods for [structures](/ecosystem/tolk/types/structures) are declared in the same way:
...
Tolk code is executed by [TVM](/ecosystem/tvm/overview) – a stack-based virtual machine. Mutations work by implicitly returning new values through the stack.
...
[Auto-serialization](/ecosystem/tolk/features/auto-serialization) with `fromSlice` follows the same mutability rules. Passing an argument without `mutate` never modifies the original variable.
