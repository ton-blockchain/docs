...
In [TVM](/ecosystem/tvm/overview), exceptions are numbers. There is no `Error` class or exception hierarchy, only [exit codes](/ecosystem/tvm/exit-codes).
...
>
  Lower values are [reserved](/ecosystem/tvm/exit-codes) by TVM. Larger ones are more gas-expensive.
</Aside>
...
An exception can carry an argument of type [`unknown`](/ecosystem/tolk/types/unknown):
...
The `condition` must be a [`bool`](/ecosystem/tolk/types/booleans). Integers are not implicitly converted: use `assert (someInt != 0)` instead of `assert (someInt)`.
...
Additionally, an [out-of-gas exception](/ecosystem/tvm/exit-codes#14%3A-out-of-gas-error) may occur at **any** point, and it cannot be caught using a [`catch`](#try-catch-statement) block. It can only be prevented by writing [exhaustive testing suites](/ecosystem/contract-dev/blueprint/testing/overview).
