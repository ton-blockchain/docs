...
- Use `match` expression for [pattern matching](/ecosystem/tolk/syntax/pattern-matching).
...
A condition must be a [`bool`](/ecosystem/tolk/types/booleans). Integers are not implicitly converted — write the comparison explicitly:
...
The same rule applies to `while`, `do while`, ternary, [`assert`](/ecosystem/tolk/syntax/exceptions#assert-statement), and the unary `!` operator. There is no gas overhead — the compiler strips the `!= 0` comparison off whenever it would translate to a redundant TVM instruction.
...
`assert` throws an [exceptions](/ecosystem/tolk/syntax/exceptions) if a condition is false.
...
`match` is used to perform different actions for different values of a variable. A common use case is routing values of a [union type](/ecosystem/tolk/types/unions):
