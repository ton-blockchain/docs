...
To make updates to `userId` visible in `demo`, declare the parameter with [`mutate`](/ecosystem/tolk/syntax/mutability#mutate-for-a-parameter): `mutate userId`.
...
To calculate CRC32 and similar values at compile time, use [`"...".crc32()`](/ecosystem/tolk/types/strings#compile-time-methods) and similar compile-time methods.

To group integer constants, use [enums](/ecosystem/tolk/types/enums).
...
In Tolk, avoid globals when possible. Use [auto-serialization](/ecosystem/tolk/features/auto-serialization) and [`lazy` loading](/ecosystem/tolk/features/lazy-loading).
