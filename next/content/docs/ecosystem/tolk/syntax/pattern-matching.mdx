...
Pattern matching on union types is used in [message handling](/ecosystem/tolk/features/message-handling).
...
`else` is not allowed for unions, but is permitted for a [`lazy` match](/ecosystem/tolk/features/lazy-loading#lazy-matching).
...
Pattern matching on [enums](/ecosystem/tolk/types/enums) requires exhaustive coverage of all cases:
