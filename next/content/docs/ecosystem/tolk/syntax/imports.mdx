...
1. [Prefer methods to global-scope functions](/ecosystem/tolk/idioms-conventions#prefer-methods-to-functions).

## Repeated symbols across contracts

When developing multiple contracts, each contract has its own file and compilation target. Place a [`contract` declaration](/ecosystem/tolk/features/contract-abi) in every entrypoint file:
