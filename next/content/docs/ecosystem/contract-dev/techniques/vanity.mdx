...
A [vanity contract](https://github.com/ton-community/vanity-contract) allows customization of the address of a smart contract being deployed. It does this by making its own [`StateInit`](/ecosystem/primitives/messages/deploy) depend on constant data that is randomly generated many times until a desired address is found. It is often used to deploy contracts with a specific prefix or suffix so the address is visible in block explorers.

The contract code and data are included in the vanity deploy message. The vanity contract is first deployed with a `StateInit` that produces the [desired address](/ecosystem/primitives/addresses/overview#account-id), and then immediately sets its actual state from the payload. This is a special case of [upgrading](/ecosystem/contract-dev/techniques/upgrades) contract's code.
...
- [Blueprint](/ecosystem/contract-dev/blueprint/overview)
...
Run the command with the desired search parameters, including `-w` for the [workchain](/ecosystem/primitives/addresses/overview#workchain-id) and the owner address allowed to perform the deployment. The example below searches on the basechain for the specified suffix.
...
This example uses [Blueprint](/ecosystem/contract-dev/blueprint/overview) to create and send this message. Define a vanity contract wrapper at `wrappers/VanityContract.ts`:
