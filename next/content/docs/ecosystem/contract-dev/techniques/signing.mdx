...
Cryptographic signatures are the foundation of access control. Contracts verify signatures on-chain and implement their own authorization policies for [wallet operations](/ecosystem/standard/wallets/how-it-works#how-ownership-verification-works), server-authorized actions, [gasless transactions](/ecosystem/standard/wallets/v5#gasless-transactions), multisig wallets, and delegation.

## Ed25519 in TON

TON uses [Ed25519](https://en.wikipedia.org/wiki/EdDSA#Ed25519) as the [standard](/ecosystem/whitepapers/tblkch#a-3-10-cryptographic-ed25519-signatures) signature scheme. All [wallets](/ecosystem/standard/wallets/history) (v1–v5) and [highload wallets](/ecosystem/standard/wallets/highload/overview) rely on Ed25519.
...
<Aside
  type="caution"
>
  The public key is _not_ the wallet address. The address is derived from the contract's initial code and data that are included in [`StateInit`](/ecosystem/primitives/messages/deploy) structure. Multiple contracts can use the same public key but have [different addresses](/ecosystem/primitives/addresses/overview).
</Aside>
...
For details, see crypto instructions in [TVM instructions](/ecosystem/tvm/instructions).
...
- [`CHKSIGNS`](/ecosystem/tvm/instructions#f911-chksigns) checks signature of data;
- [`CHKSIGNU`](/ecosystem/tvm/instructions#f910-chksignu) checks signature of hash.
...
Standard wallet contracts are [described in more detail](/ecosystem/standard/wallets/history) in the wallets section.
