...
Errors in smart contracts can produce an [exit code](/ecosystem/tvm/exit-codes), often indicating a bug in the contract. Use debugging methods to locate and fix the issue.
...
There are three TVM debug [instructions](/ecosystem/tvm/instructions#fe-debug): `DUMPSTK`, `STRDUMP`, and `DUMP`.
...
Of all [verbosity levels](/ecosystem/contract-dev/blueprint/testing/reference#verbosity-2), the two are the most useful:
...
The contract attempts to load a 64-bit integer from the slice using `LDU 64`. Since there is not enough data, execution stops with [exit code 9](/ecosystem/tvm/exit-codes#9%3A-cell-underflow).
...
<Aside type="tip">
  ...
  Sometimes several instructions are implemented within a single `exec_*` method. For example, [LDU](/ecosystem/tvm/instructions#d3-ldu) (`load_uint`), [LDI](/ecosystem/tvm/instructions#d2-ldi) (`load_int`) and it's preload versions (`preload_uint`and`preload_int`).
...
However, the slice contains only `725` bits, of which `711` bits and both [references](/ecosystem/primitives/serialization/cells) have already been read. The contract attempted to read `64` more bits, but the slice did not contain enough remaining data.
...
- [TxTracer Sandbox](/ecosystem/tvm/tools/txtracer) — requires a custom `@ton/sandbox` package; runs in the browser.

Also, these tools allow to explore logs of each transaction.

## Inspect BoC returned by APIs

When interacting with the blockchain using APIs, responses may include serialized cells in [BoC (bag of cells)](/ecosystem/primitives/serialization/boc), TON's standard cell serialization format.
...
Even when a contract executes successfully (exit code = `0`) with no errors, the actions may not produce the expected on-chain result. [TVM Retracer](/ecosystem/tvm/tools/retracer) replays the transaction and displays VM-level execution in detail.

### Scenarios for retracing

- All [execution phases](/ecosystem/primitives/phases) complete without errors, yet the expected outcome is missing.
...
1. Obtain the transaction hash from a [blockchain explorer](/infrastructure/explorers/overview).
1. Open [TVM Retracer](/ecosystem/tvm/tools/retracer) and enter the transaction hash.
