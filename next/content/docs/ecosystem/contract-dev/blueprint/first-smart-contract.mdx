...
- Installed [TON wallet](/wallets/wallet-apps/tonkeeper) with [Toncoin on testnet](/wallets/wallet-apps/get-coins)
...
<Steps>
  ...
  >
    Use the [Blueprint](/ecosystem/contract-dev/blueprint/overview) development toolkit for smart contracts. Start a new project with:
...
A smart contract is a program stored on <Tooltip tip="A distributed database that many computers maintain together." cta="Learn more" href="/ecosystem/primitives/shards">TON blockchain</Tooltip> and executed by the <Tooltip tip="The “computer” which executes smart contracts on TON blockchain." cta="Learn more" href="/ecosystem/tvm/overview">TVM</Tooltip>.
On-chain, every contract consists of two components:

- Code — compiled [TVM instructions](/ecosystem/tvm/instructions), defines the contract's logic.
- Data — persistent state, stores information between interactions.

Both are stored at a specific [address](/ecosystem/primitives/addresses/overview) on TON blockchain, which is a unique identifier for each smart contract. Smart contracts interact with each other only through [messages](/ecosystem/primitives/messages/overview).
...
- [Get methods](/ecosystem/tvm/get-method) are read-only functions that return contract data without modifying state. Example: a get method that returns the current `counter` value.

  Due to the [TON architecture](/learn-more/from-ethereum#on-chain-get-methods), get methods cannot be called from other contracts. Inter-contract communication uses **messages** only.
...
The contract uses [**Tolk**](/ecosystem/tolk/overview) language.
...
<Steps>
  ...
  >
    ...
    To define contract storage, store the `counter` value. Tolk makes it simple with <Tooltip tip="A structure is a composite data type that groups named fields, each having its own type, into one unit." cta="Learn more" href="/ecosystem/tolk/overview">structures</Tooltip>:
    ...
    Structures serialize and deserialize automatically into [cells](/ecosystem/primitives/serialization/cells), the storage unit in TON. The `fromCell` and `toCell` functions handle conversion between structures and cells.
  </Step>
...
<Steps>
  ...
  >
    ...
    - The contract [address](/ecosystem/primitives/addresses/overview) is derived deterministically from `code` and `data` using `contractAddress`.
  ...
  >
    ...
    For flags and options, see the [Blueprint deployment guide](/ecosystem/contract-dev/blueprint/deploy).
  </Step>
  ...
  >
    <Aside
      ...
      type="note"
    >
      If a wallet is not installed, check the [wallet section](/wallets/wallet-apps/tonkeeper) to select and install a wallet before deploying the contract. Make sure the wallet is funded with [Toncoin on the testnet](/wallets/wallet-apps/get-coins).
    </Aside>
    ...
    The link opens the contract on [Tonviewer](/infrastructure/explorers/tonviewer), a [blockchain explorer](/infrastructure/explorers/overview) showing transactions, messages and [account states](/foundations/status).
