...
A wallet account's address is [derived](/ecosystem/primitives/addresses/derive) from [`StateInit`](/ecosystem/primitives/messages/deploy) that stores in its `data` field two values:

- `public_key`, dependent on the [mnemonic](/ecosystem/standard/wallets/mnemonics);
...
| Network | [V4R2](/ecosystem/standard/wallets/v4) | [V5](/ecosystem/standard/wallets/v5) |
| ------- | ---------------------------- | -------------------------- |
...
This wallet contract instance can be used to [send messages](/ecosystem/standard/wallets/v5-api) to the blockchain.
...
- [Jetton transfers](/ecosystem/standard/tokens/jettons/api#forward-payload-formats), where the wallet forwards an internal message to the token wallet along with the comment payload;
- [NFT transfers](/ecosystem/standard/tokens/nft/api#forward-payload-formats), where the comment travels in the same forwarding message that moves the ownership record.
...
1. Store the text as a [byte string](/ecosystem/standard/tokens/metadata#snake-data-encoding) (UTF-8 encoded).
1. Send the internal message along with the desired Toncoin, [Jettons](/ecosystem/standard/tokens/jettons/how-it-works), or [NFT](/ecosystem/standard/tokens/nft/how-it-works) payload.
