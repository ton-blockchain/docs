...
1. opcode = `0x706c7567`, request funds operation.
   - `amount`: [Coins](/ecosystem/tvm/instructions#fa02-stgrams) amount of requested Toncoin.
...
Wallet V4 provides standard functionality through the `0x0` opcode, similar to previous versions (see [wallet overview](/ecosystem/standard/wallets/history) for details on message layout). The `0x2` and `0x3` operations allow manipulation of the plugin dictionary.
