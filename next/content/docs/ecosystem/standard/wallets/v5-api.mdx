...
Message structure for V5 wallet contract is quite cumbersome and hard to read, it's made for optimal (de-)serialization and not optimized for understanding. It is described in [TL-B](/ecosystem/languages/tl-b/overview) languages and includes snake-cell pattern. We will try to get a grip of it by breaking down core data structures and how they are used.
...
Now we have to deal with basic action serialization. However, since these are the messages that are described in [`block.tlb`](/ecosystem/languages/tl-b/overview), we can use contract-agnostic code from serialization library to store them.
...
You can read more about where to obtain them and what there purpose is in [How it works](/ecosystem/standard/wallets/how-it-works) section.
