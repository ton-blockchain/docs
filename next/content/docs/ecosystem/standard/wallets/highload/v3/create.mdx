...
<Aside
  type="caution"
>
  **Cannot be changed later:** `timeout` is part of the contract's [initial configuration](/ecosystem/standard/wallets/highload/v3/specification#storage-structure). Changing it would change the wallet address entirely.
</Aside>

See [Timeout constraints](/ecosystem/standard/wallets/highload/v3/specification#timeout-constraints) in the specification for details.
...
See [Storage structure](/ecosystem/standard/wallets/highload/v3/specification#storage-structure) in the specification for details.
...
<Aside type="tip">
  **Why non-bounceable?** When funding a `nonexist` account, use the non-bounceable format to prevent funds from bouncing back if the account doesn't exist yet. See [Address formats](/ecosystem/primitives/addresses/formats) for details.
</Aside>
