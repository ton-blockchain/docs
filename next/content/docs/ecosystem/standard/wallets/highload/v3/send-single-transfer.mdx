...
- Understand how `query_id`, `created_at`, and [send modes](/ecosystem/primitives/messages/modes) work

## Prerequisites

- Completed [wallet creation](/ecosystem/standard/wallets/highload/v3/create) with funded balance and saved configuration in `.wallet.json`
...
<Aside
  type="caution"
>
  **No state-init support:** Highload Wallet v3 cannot send messages containing [`StateInit`](/ecosystem/primitives/messages/deploy) using this method.

  If you need to deploy a contract (send a message with `StateInit`), use [batch transfers](/ecosystem/standard/wallets/highload/v3/send-batch-transfers) with an action list instead. See [Limitations and constraints](/ecosystem/standard/wallets/highload/v3/specification#limitations-and-constraints) in the specification for details.
</Aside>
...
- See [Query ID structure](/ecosystem/standard/wallets/highload/v3/specification#query-id-structure) for details
...
- See [Timestamp validation](/ecosystem/standard/wallets/highload/v3/specification#timestamp-validation) for why this is necessary
...
To verify that your message, see [How to verify message is processed](/ecosystem/standard/wallets/highload/v3/verify-is-processed).
