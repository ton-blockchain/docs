...
Keep a precomputed mapping of recipient → allocation in the contract. When a user sends a claim message, the contract releases the preassigned amount. This works until the list becomes too large. Starting at roughly 3,000 entries, problems begin to surface with the external limit (see more in [limits](/ecosystem/primitives/limits#message-and-transaction-limits)).
...
The airdrop contract stores a root hash of a dictionary (see [hashmap](/ecosystem/languages/tl-b/complex-and-non-trivial-examples#hashmap)) containing all allocations. Users present a [Merkle proof](/ecosystem/primitives/proofs/overview) to verify their allocation against this root.
...
For signature implementation details and security considerations, see [signing messages](/ecosystem/contract-dev/techniques/signing).
...
- [cNFT](/ecosystem/standard/tokens/nft/comparison#cnft)
- [Mintless Jetton](/ecosystem/standard/tokens/jettons/mintless/overview)
