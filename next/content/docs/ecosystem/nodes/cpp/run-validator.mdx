...
[Blockchain explorer](/infrastructure/explorers/overview) should also display the now-deployed contract with `Active` status.
...
<Aside
  type="caution"
>
  After activation, verify the following in a [blockchain explorer](/infrastructure/explorers/overview):
...
In the next election cycle (odd/even), MyTonCtrl will send half of the stake from the single nominator pool. Track this with a [blockchain explorer](/infrastructure/explorers/overview).

### 4.6 Track second cycle progress

In the next paired election cycle (even/odd), MyTonCtrl will send the second half of the stake from the single nominator pool. Track this with a [blockchain explorer](/infrastructure/explorers/overview).
...
Confirm that the validator participates in elections and recovers stakes on time by checking the pool balance in a [blockchain explorer](/infrastructure/explorers/overview). For greater observability, [set up the monitoring](/ecosystem/nodes/cpp/run-validator#5-7-set-up-monitoring) of various validator metrics.
...
- Check the elector contract status using a [blockchain explorer](/infrastructure/explorers/overview).
