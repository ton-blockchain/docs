...
Each of these types occupies a single entry on the [TVM stack](/ecosystem/tvm/overview#tvm-state).

|           |                                                                                                                                                                                                                                                                                 |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `int`     | a 257-bit signed integer type. Overflow checks are enabled by default and trigger an exception.                                                                                                                                                                                 |
| `cell`    | a [TVM cell type](/ecosystem/primitives/serialization/cells) for persistent data storage on TON Blockchain. Data is organized in a [bag of cells](/ecosystem/primitives/serialization/boc), with each cell containing up to 1023 bits of arbitrary data and up to four references to other cells. |
...
| `cont`    | a [TVM continuation](/ecosystem/tvm/continuations) employed for execution flow management in [TVM instructions](/ecosystem/tvm/instructions).                                                                                                                                                       |
...
The `null` value can be obtained by calling the function [`null()`](/ecosystem/languages/func/stdlib#null). For example, in this snippet, an integer variable is declared and initialized with `null`:
...
- For [library functions](/ecosystem/languages/func/stdlib) specifications explicitly indicate when `null` is acceptable as a valid input or output.

Example: [`cell_depth(cell c)`](/ecosystem/languages/func/stdlib#cell-depth) receives a `cell` as input, and its specification states that if the input cell is `null`, the function returns `0`.
...
- Logical operations are performed using [bitwise operations](/ecosystem/languages/func/operators).
- In [conditional statements](/ecosystem/languages/func/statements#conditional-statements), [loops](/ecosystem/languages/func/statements#loops), and [conditional expressions](/ecosystem/languages/func/operators#ternary%2C-%3F%3A) any nonzero integer is regarded as `true`.
...
See [Function declarations](/ecosystem/languages/func/functions#function-declaration) for more details.
...
For more details, see the [Polymorphism with forall](/ecosystem/languages/func/functions#polymorphism-with-forall) section.
