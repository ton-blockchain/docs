...
The return type can be any atomic or composite type, as described in the [Types](/ecosystem/languages/func/types) section.
...
There, the function `divAndMod` has the inferred type `(int, int) -> (int, int)`. The function computes the division and modulo of the parameters `m` and `n` by using the [division and modulo](/ecosystem/languages/func/operators#division-and-modulo%2C-%2F%25) operator `/%`, which always returns a two-element tensor `(int, int)`.

## Function name

A function name can be any valid [identifier](/ecosystem/languages/func/literals#identifiers). Additionally, it may start with the symbols `.` or `~`, which have specific meanings explained in the [special function call notation](/ecosystem/languages/func/expressions#special-function-call-notation) section. Specifically, refer to this [section](/ecosystem/languages/func/expressions#and-in-function-names) to understand how the symbols `.` or `~` affect the function name.

For example, `udict_add_builder?`, `dict_set`, and `~dict_set` are all valid function names, and each is distinct. These functions are defined in [stdlib.fc](/ecosystem/languages/func/stdlib).

FunC reserves several function names. See the [reserved functions](/ecosystem/languages/func/special-functions) article for more details.
...
<Aside>
  Even though a function may appear to take multiple arguments, it takes a single [tensor type](/ecosystem/languages/func/types#tensor-types) argument. For more details on this distinction, refer to the [function call](/ecosystem/languages/func/expressions#function-call) section.
...
For example, the [stdlib.fc](/ecosystem/languages/func/stdlib) function [`random`](/ecosystem/languages/func/stdlib#random) changes the internal state of the random number generator:
...
>
  ...
  **Reserved ranges**:
  ...
  - -4 to 0 for [special functions](/ecosystem/languages/func/special-functions): `main` or `recv_internal` (ID = 0), `recv_external` (ID = -1), `run_ticktock` (ID = -2), `split_prepare` (ID = -3), `split_install` (ID = -4)
...
Refer to the [assembler functions](/ecosystem/languages/func/asm-functions) article for more details.

### Standard body

A standard body uses a [block statement](/ecosystem/languages/func/statements#block-statement), i.e., the body of the function is defined inside curly braces `{ }`.
...
The declarator starts with the `forall` keyword and finishes with the symbol `->`. Each element in the comma separated list must be a type variable name. A type variable name can be any [identifier](/ecosystem/languages/func/literals#identifiers), but capital letters are commonly used.
...
This function declares two type variables `X` and `Y`. The function uses these two type variables to declare an argument `pair` of type `[X, Y]`, i.e., a [tuple](/ecosystem/languages/func/types#tuple-types) where the first component is of type `X` and the second component of type `Y`. The function then swaps the components of the tuple and returns a tuple of type `[Y, X]`.
