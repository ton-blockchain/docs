...
FunC, or more specifically, the Fift assembler, reserves several function names with predefined [IDs](/ecosystem/languages/func/functions#method-id-specifier):
...
The `recv_internal` function is invoked when a smart contract receives an inbound [internal message](/ecosystem/primitives/messages/internal).
...
<Aside>
  Each time a contract receives a message, the [TVM initializes](/ecosystem/tvm/initialization) and pushes into the stack the following four pieces of data, in this order:
...
The `recv_external` function handles inbound [external messages](/ecosystem/primitives/messages/external-in). It allows declarations similar to those for [`recv_internal`](#receive-internal):
