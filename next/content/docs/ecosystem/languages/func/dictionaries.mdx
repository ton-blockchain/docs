...
>
  ...
  - **Gas consumption for updates**
    - Every update operation generates many new cells, costing 500 gas, as detailed on the [TVM instructions](/ecosystem/tvm/instructions) page.
...
Hashmaps are the most widely used dictionary type in TON. They have a dedicated set of TVM opcodes for manipulation and are commonly used in smart contracts (see [TVM instructions](/ecosystem/tvm/instructions) - Dictionary manipulation).
...
- Move to the next/previous key-value pair (entries are ordered by keys, enabling [iteration](/ecosystem/languages/func/cookbook#how-to-iterate-dictionaries) if gas constraints allow);
...
For further [examples of hashmap parsing](/ecosystem/languages/tl-b/overview#hashmap-parsing-example), refer to the official documentation.
