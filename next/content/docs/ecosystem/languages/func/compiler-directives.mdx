...
In Func, using a variable before it is modified within the same [expression](/ecosystem/languages/func/expressions) is prohibited by default.

For example, the following code will **not** compile, because `ds` is used before it is modified in `ds~load_uint(8)`.
See [modifying notation](/ecosystem/languages/func/expressions#modifying-notation) for more details on using symbol `~`.
...
Here, the `asm(value index dict key_len)` notation dictates a [rearrangement of arguments](/ecosystem/languages/func/asm-functions#rearranging-stack-entries).
