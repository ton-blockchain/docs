...
<Aside>
  Currently, all the unary and binary operators are integer operators. Logical operators are bitwise integer operators (cf. [absence of boolean type](/ecosystem/languages/func/types#no-boolean-type)).
</Aside>
...
<Aside>
  In expressions, all operators should be separated from their arguments, otherwise the expression is interpreted as an [identifier](/ecosystem/languages/func/literals#identifiers).
...
The `~` operator also behaves as expected on `-1` and `0`, which are the [emulated values](/ecosystem/languages/func/types#no-boolean-type) for `true` and `false`:
...
Going beyond the range of the integer type will result in an error with [exit code 4](/ecosystem/tvm/exit-codes#4%3A-integer-overflow): `Integer overflow`.
...
An attempt to divide by zero results in an error with [exit code 4](/ecosystem/tvm/exit-codes#4%3A-integer-overflow): `Integer overflow`.
...
If `b` in the expression `a % b` is `0`, the result is an error with [exit code 4](/ecosystem/tvm/exit-codes#4%3A-integer-overflow): `Integer overflow`.
...
The slash-percent sign (division and modulo) operator `/%` returns a two element [tensor](/ecosystem/languages/func/types#tensor-types), where the first element is the result
...
See more about decomposition notation in the [variable declaration](/ecosystem/languages/func/expressions#variable-declaration) section.
...
Exceeding the range of the integer type will result in an error with [exit code 4](/ecosystem/tvm/exit-codes#4%3A-integer-overflow): `Integer overflow`.
...
(`?`), then an [expression](/ecosystem/languages/func/expressions) to execute (the "consequent" expression) if the condition evaluates to an integer different from `0`, followed by a colon (`:`),
...
The assignment operator `=` is used to assign the result of an [expression](/ecosystem/languages/func/expressions) to a variable. After execution, the `=` operator returns the value assigned to the variable.
...
The assignment operator can also occur in [variable declarations](/ecosystem/languages/func/expressions#variable-declaration), which are also expressions:
