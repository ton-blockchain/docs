...
For detailed descriptions of these commands, refer to the [TVM documentation](/ecosystem/whitepapers/tvm).
...
For more details on polymorphic functions, refer to the [Polymorphism with forall](/ecosystem/languages/func/functions#polymorphism-with-forall) section.
...
Extracts the head and tail of a lisp-style list. It can be used as a [(non-)modifying method](/ecosystem/languages/func/statements#methods-calls).
...
>
  ...
  1. Message exceeds size limits, see the [maximum message bit and cell size](/ecosystem/primitives/limits#message-and-transaction-limits) for details.
  ...
  1. The outbound message contains invalid libraries in [`StateInit`](/ecosystem/primitives/messages/deploy).
...
See also [set\_c3](/ecosystem/languages/func/stdlib#set_c3).
...
It can be used as a [modifying method](/ecosystem/languages/func/statements#modifying-methods).
A primitive _preloads_ data when it returns only the extracted data, leaving the original slice unchanged. It can be used as a [non-modifying method](/ecosystem/languages/func/statements#non-modifying-methods).
...
A primitive _stores_ a value `x` in a builder `b` by returning a modified version `b'` with `x` added at the end. It can also be used as a [non-modifying method](/ecosystem/languages/func/statements#non-modifying-methods).
...
Checks whether the given `cell c` is `null`. In most cases, a `null` cell represents an empty dictionary. FunC also includes a polymorphic `null?` built-in function. See [built-in](/ecosystem/languages/func/built-ins#other-primitives) for more details.
...
>
> 1. **As a `Maybe cell (c^?)`**, which is either a cell that contains a serialized `Hashmap(n, X)` same as above or `null`, indicating an empty dictionary. See [null values](/ecosystem/languages/func/types#null-values) for details. When using this format, the dictionary is typically referred to as `D`.
>
...
Additionally, some primitives have counterparts prefixed with `~`, enabling them to be used as [modifying methods](/ecosystem/languages/func/statements#modifying-methods).
...
TVM supports dictionaries with non-fixed length keys that follow a prefix code structure, meaning no key is a prefix of another key. Learn more in the [TVM instruction](/ecosystem/tvm/instructions) section.
...
Marks a variable as used, ensuring the code generated isn't removed during optimization, even if it's not impure. See [Impure specifier](/ecosystem/languages/func/functions#impure-specifier) section.
