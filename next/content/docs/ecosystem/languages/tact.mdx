...
>
  The official smart contract language of TON Blockchain is [Tolk](/ecosystem/tolk/overview), and the Tact language is being **deprecated**. This page will be removed in mid-April 2026.
...
- a convenient `unboc` disassembler of a contract's code compiled into a [bag of cells](/ecosystem/primitives/serialization/boc) `.boc` format.
...
- BoC Disassembler (`unboc`) — Disassembler for [`.boc`](/ecosystem/primitives/serialization/boc) files. Ships as a standalone binary with the compiler. You can invoke it by running `npx unboc` in your Tact projects.
