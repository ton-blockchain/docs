...
TON Blockchain is a collection of blockchains that are called _workchains_. Each workchain might have different formats of [account addresses](/ecosystem/primitives/addresses/overview), formats of [transactions](/ecosystem/primitives/messages/ordinary-tx), and different virtual machines for smart contracts.
...
There might be up to `2^32` workchains, identified with `workchain_id`. Each workchain might use its own format for [`account_id`](/ecosystem/primitives/addresses/overview#account-id), but every such ID must be at least 64-bit long. A pair of `workchain_id` and `account_id` uniquely identifies an account.
