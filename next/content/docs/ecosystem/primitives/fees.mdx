...
Fees in TON align with the [execution phases](/ecosystem/primitives/phases) of a transaction:

- Storage fees are charged in the [storage phase](/ecosystem/primitives/phases#storage-phase).
- Compute fees are charged in the [compute phase](/ecosystem/primitives/phases#compute-phase).
- Forward and action fees are charged in the [action](/ecosystem/primitives/phases#action-phase) and [bounce phases](/ecosystem/primitives/phases#bounce-phase).
...
All computation is measured in gas units. A TVM operation typically has a fixed gas cost, but that is [not always the case](/ecosystem/tvm/gas). Network configuration defines gas prices; users cannot override them.
...
Action fee is the portion of `fwdFee` granted to the validator of the message's source [shard](/ecosystem/primitives/shards#blockchain-sharding). The remaining `fwdFee - actionFee` amount goes to the validator of the destination shard.

Action fee exists only for [internal messages](/ecosystem/primitives/messages/internal#internal-messages).

```cpp
action_fee = floor(fwd_fee * first_frac / 2^16)
```

Starting with Global Version 4, a failed [SENDMSG action](/ecosystem/primitives/actions/send#send-message) incurs a penalty proportional to the attempted message size. It is calculated as:
