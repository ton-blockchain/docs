...
To assign the ADNL address to a `.ton` domain, open the domain in the TON DNS management interface, paste the ADNL address into the "Site" field, and confirm the transaction with the domain owner's wallet. For record types and resolution details, see [TON DNS](/ecosystem/primitives/web3/ton-dns).

## Domain name resolution

### TON DNS

A `dns_adnl_address` record in TON DNS maps a `.ton` domain name to the ADNL address of the TON Site. The proxy client resolves this record and connects to the correct ADNL address. See [TON DNS](/ecosystem/primitives/web3/ton-dns) for the full record type specification.
...
- [TON Proxy](/ecosystem/primitives/web3/ton-proxy): bridges HTTP requests from a conventional browser to the ADNL network.
- [TON DNS](/ecosystem/primitives/web3/ton-dns): maps `.ton` domains to ADNL addresses.
