...
| `cskip_bad_state` | Raised in two cases: when the `fixed_prefix_length` [field has an invalid value][fixed_prefix_length] or when the [`StateInit`](/ecosystem/primitives/messages/deploy) provided in the incoming message [does not match account’s address][account_address]. |
...
>
  ...
  This topic is outside the scope of this guide, but more information is available in [Shards page](/ecosystem/primitives/shards).
</Aside>
...
> See [Fees → Forward fee](/ecosystem/primitives/fees#forward-fee) for more info.
