...
You can view the parameter values in the [current configuration](https://tonviewer.com/config), and the method of writing them into [cells](/ecosystem/primitives/serialization/boc) is outlined in the [`block.tlb`](https://github.com/ton-blockchain/ton/blob/master/crypto/block/block.tlb) file in [TL-B](/ecosystem/languages/tl-b/overview) format.

<Aside>
  Configuration values are TL-B typed cells serialized into [Bags of Cells (BoC)](/ecosystem/primitives/serialization/boc).
</Aside>
...
This parameter is the address of the [elector smart contract](/ecosystem/primitives/system), responsible for appointing validators, distributing rewards, and voting on changes to blockchain parameters.
...
This parameter defines the cost of filing complaints about the incorrect operation of validators in the [elector smart contract](/ecosystem/primitives/system).
...
<Aside>
  You can find more about `gas_credit` and other parameters in the section of external messages [here](/ecosystem/tvm/instructions#external-messages).
</Aside>
...
More details about precompiled contracts are on [this page](/ecosystem/primitives/precompiled).
...
- The original descriptions are present, but they may be limited, in the documents:
  - [The Open Network Whitepaper](/ecosystem/whitepapers/ton)
  - [Telegram Open Network Blockchain](/ecosystem/whitepapers/tblkch)
