...
Sending a message is the most frequent action that a smart contract performs during the [action phase](/ecosystem/primitives/phases#action-phase). It can get into the [out action list](https://github.com/ton-blockchain/ton/blob/5c0349110bb03dd3a241689f2ab334ae1a554ffb/crypto/block/block.tlb#L411) by:

- The [`SENDRAWMSG`](/ecosystem/tvm/instructions#fb00-sendrawms) instruction.
- The [`SENDMSG`](/ecosystem/tvm/instructions#fb08-sendmsg) instruction.
- The [`POPCTR`](/ecosystem/tvm/instructions#ed5-popctr) instruction.

The sending message action consists of:

- an 8-bit bitmask [`mode`](/ecosystem/primitives/messages/modes) specifying the way of sending;
...
- fill the `src` field with [`addr_none`](/ecosystem/primitives/addresses/overview#external-addresses).
