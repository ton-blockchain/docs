...
Unlike [incoming](/ecosystem/primitives/messages/external-in) external messages, a third party can't manipulate the contents of outgoing external messages, so there is no need to sign them.

- `src` — an [internal address](/ecosystem/primitives/addresses/overview#internal-addresses) of the contract that sends the message. It has to be a valid address, but is not important, as it will be overwritten by the address of sender after the message is sent.
- `dest` — any valid [external address](/ecosystem/primitives/addresses/overview#external-addresses).
...
As outbound external messages do not carry any Toncoin value, the price of their handling is paid similarly to [`SendPayFwdFeesSeparately`](/ecosystem/primitives/messages/modes) mode. However, using any mode other than [`SendIgnoreErrors`](/ecosystem/primitives/messages/modes), [`SendBounceIfActionFail`](/ecosystem/primitives/messages/modes), or [`SendPayFwdFeesSeparately`](/ecosystem/primitives/messages/modes) for outbound external messages results in [exit code `34`](/ecosystem/tvm/exit-codes#34:-invalid-or-unsupported-action). In this case, regardless of whether the [`SendIgnoreErrors`](/ecosystem/primitives/messages/modes) mode was used, the action phase will fail. Mode [`SendPayFwdFeesSeparately`](/ecosystem/primitives/messages/modes) does not change the message behavior.
