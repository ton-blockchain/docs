...
>
  A _workchain_ is a TON blockchain with its own rules and account space. The _masterchain_ holds global configuration and system smart contracts, while the _basechain_ hosts most accounts and user space smart contracts and can be split into shardchains for scalability. In future there might be new workchains with their own set of rules and logic. Practical note: [addresses](/foundations/addresses/overview#internal-addresses) include the workchain ID, and most apps use workchain 0 (basechain).
</Aside>
