...
First, create an empty builder using [`NEWC`](/ecosystem/tvm/instructions#c8-newc):
...
And call [`STU`](/ecosystem/tvm/instructions#cb-stu) ("STore Unsigned integer") to store integer into builder (swapping builder and value to meet `STU` input order):
...
And, finally, [`ENDC`](/ecosystem/tvm/instructions#c9-endc) instruction finalizes the builder to a cell.
...
[`CTOS`](/ecosystem/tvm/instructions#d0-ctos) ("Cell TO Slice") loads a `Cell` to a `Slice`.
...
Then, call [`LDU`](/ecosystem/tvm/instructions#d3-ldu) ("LoaD Unsigned integer") to read first value (`uint4`).
...
A common way to ensure there is no data left inside the slice is to call [`ENDS`](/ecosystem/tvm/instructions#d1-ends).
