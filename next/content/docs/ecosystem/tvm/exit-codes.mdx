...
{/*
  ...
  Read more about expecting specific exit codes: [Transactions with intentional errors](/ecosystem/contract-dev/blueprint/debug#tests-errors).
...
If there are too many elements copied into a closure continuation, an error with exit code 3 is thrown: `Stack overflow`. This occurs rarely unless you're deep in the [Fift and TVM assembly](/ecosystem/languages/fift/fift-and-tvm-assembly) trenches:
...
In Tolk, the `map<K, V>` type is an abstraction over the ["hash" map dictionaries of TVM](/ecosystem/languages/func/dictionaries).

If there is incorrect manipulation of dictionaries, such as improper assumptions about their memory layout, an error with exit code 10 is thrown: `Dictionary error`. Note that Tolk prevents you from getting this error unless you perform [TVM assembly](/ecosystem/languages/fift/fift-and-tvm-assembly) work yourself:
