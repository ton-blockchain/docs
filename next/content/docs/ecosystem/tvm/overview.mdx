...
TVM is invoked when a message is sent to an account that has deployed contract code, when a get method is called on an account, and in some [more rare cases](/ecosystem/tvm/initialization#stack).
...
Every instruction consumes [gas](/ecosystem/tvm/gas). Gas exhaustion stops execution. This limit is imposed so that expensive computations (i.e. infinite loops) cannot be used to exhaust validators' computation resources, causing [denial of service](https://en.wikipedia.org/wiki/Denial-of-service_attack).
...
- All values are [immutable](https://en.wikipedia.org/wiki/Immutable_object).
  Most of the data is stored as immutable tree of [cells](/ecosystem/primitives/serialization/cells).
- Reading and writing of cells is done with [slices and builders](/ecosystem/tvm/builders-and-slices).
- There are no function addresses or function pointers. Code is executed from bitcode inside [continuations](/ecosystem/tvm/continuations).

## TVM state

On incoming messages or get method call, a new instance of TVM is started, with a new state. Derivation of the initial state from the message is described in [its own article](/ecosystem/tvm/initialization).
...
- [**Control registers**](/ecosystem/tvm/registers). A small fixed set of special registers, denoted as `c0`, `c1`, ..., `c5`, and `c7` (`c6` does not exist).
- [**Gas counter**](/ecosystem/tvm/gas). Tracks remaining computation budget. Each instruction decrements gas. When counter hits zero/negative value, an exception is raised, and the run aborts.
...
Here is a sample contract, written in [Fift](/ecosystem/languages/fift/overview). It implements the following logic:
...
When an account with this code gets an internal message, TVM stack [is initialized](/ecosystem/tvm/initialization#stack) with these values:
...
1. Three more values `s2`, `s3`, `s4` [are pushed](/ecosystem/tvm/initialization#external%2Finternal-message) by TVM onto a stack. They won't be used in the example. After execution finishes, they'll still be on the stack, and will be silently ignored.
