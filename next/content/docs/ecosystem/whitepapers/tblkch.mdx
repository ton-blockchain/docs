...
A detailed description of the elliptic curve cryptography used for signing blocks and messages, also accessible through TVM primitives, is provided in [Appendix A](#a-elliptic-curve-cryptography). TVM itself is described in a [separate document](/ecosystem/whitepapers/tvm).
...
In addition to the transparent and representation hashes of a cell $c$, a sequence of [higher hashes](/ecosystem/whitepapers/tvm#3-1-6-the-higher-hashes-of-a-cell) $\text{Hash}_i(c)$, $i=1,2,\dots$ may be defined, which eventually stabilizes at $\text{Hash}_\infty(c)$.
...
All of the most important non-split shardchain data structures related to messages are organized as *hashmaps* or *dictionaries* (implemented by means of [Patricia trees](/ecosystem/whitepapers/tvm#3-3-2-hashmaps-as-patricia-trees) serialized into a tree of cells), with the following keys:
...
When some amounts of one or several such cryptocurrencies need to be represented, a [dictionary](/ecosystem/whitepapers/tvm#3-3-hashmaps,-or-dictionaries) with 32-bit $\textit{currency\_id}$s as keys and `VarUInteger 32` values is used:
...
Augmented hashmaps are key-value storage structures with $n$-bit keys and values of some type $X$, similar to the [ordinary hashmaps](/ecosystem/whitepapers/tvm#3-3-hashmaps,-or-dictionaries). However, each intermediate node of the Patricia tree representing an *augmented* hashmap is augmented by a value of type $Y$.
...
The [serialization of augmented hashmaps](/ecosystem/whitepapers/tvm#3-3-3-serialization-of-hashmaps) with $n$-bit keys, values of type $X$, and augmentation values of type $Y$ is given by the following TL-B scheme, which is an extension:
...
The exclamation mark in the TL-B declaration of a `merkle_update` indicates special processing required for such values. In particular, they must be kept in a separate cell, which must be marked as [*exotic*](/ecosystem/whitepapers/tvm#3-1-7-types-of-exotic-cells) by a bit in its header.
...
In addition to this dictionary, an *AccountBlock* contains a [Merkle update](/ecosystem/whitepapers/tvm#3-1-7-types-of-exotic-cells) of the total state of the account. If an account did not exist before the block, its state is represented by an `account_none`.
...
2. Choose an integer $s$, such that $n\leq 2^s$. Represent each cell $c_i$ by an integral number of octets in the standard way ([1.1.3](#1-1-3-the-layout-of-a-single-cell) or [4, 3.1.4](/ecosystem/whitepapers/tvm#3-1-4-standard-cell-representation)), but using unsigned big-endian $s$-bit integer $j$ instead of hash $\text{Hash}(c_j)$ to [represent](#5-3-6-serialization-of-one-cell-from-a-bag-of-cells) internal references to cell $c_j$.
...
1. Two descriptor bytes $d_1$ and $d_2$ are computed similarly to [Standard cell representation](/ecosystem/whitepapers/tvm#3-1-4-standard-cell-representation) by setting $d_1=r+8s+16h+32l$ and $d_2=\lfloor b/8\rfloor+\lceil b/8\rceil$, where:
   ...
   - $0\leq l\leq 3$ is the [level of cell](/ecosystem/whitepapers/tvm#3-1-3-the-level-of-a-cell) $c$.
...
3. If $h=1$, the serialization is continued by $l+1$ 32-byte [higher hashes](/ecosystem/whitepapers/tvm#3-1-6-the-higher-hashes-of-a-cell) of $c$: $\text{Hash}_1(c)$, ..., $\text{Hash}_{l+1}(c)=\text{Hash}_\infty(c)$.
