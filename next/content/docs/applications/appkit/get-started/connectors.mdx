...
Drop `<TonConnectButton />` somewhere in the tree to open the wallet picker. The button assumes the React providers are already mounted — see [Install AppKit → Wrap the application](/applications/appkit/get-started/installation/react-app#wrap-the-application).
