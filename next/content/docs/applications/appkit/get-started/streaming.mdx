...
A streaming provider has to be registered on the AppKit instance before any watcher will fire. The default `ApiClient` does not stream — you register a streaming provider separately on `appKit.streamingManager`. See [Use providers → How they are registered](/applications/appkit/howto/providers#how-they-are-registered) for the registration pattern, and use `hasStreamingProvider(appKit, network)` from `@ton/appkit` as the runtime check before subscribing.
...
- For task-level patterns that combine sends with settlement tracking, see [Use streaming](/applications/appkit/howto/streaming) in the How to section.
