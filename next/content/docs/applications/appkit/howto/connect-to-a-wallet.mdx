...
You need an `AppKit` instance with at least one connector and the React providers mounted. See [Add wallet connectors](/applications/appkit/get-started/connectors) for the instance and [Install AppKit → Wrap the application](/applications/appkit/get-started/installation/react-app#wrap-the-application) for the provider setup.
