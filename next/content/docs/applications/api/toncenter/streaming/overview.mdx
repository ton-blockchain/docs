---
id: ecosystem/api/toncenter/streaming/overview
...
Streaming API serves as a real-time, streaming version of the [indexed access layer (API v3)](/applications/api/toncenter/v3-overview). Use it when building wallets, explorers, monitoring systems, or automation tools.
...
>
  ...
  When application state or business logic depends on a complete event sequence, resynchronize with historical data by polling [API v3](/applications/api/toncenter/v3-overview) after reconnecting.
</Aside>
...
>
  The [API v2](/applications/api/toncenter/v2-overview) and [API v3](/applications/api/toncenter/v3-overview) include their major version numbers in their product names. For the Streaming API, `v2` means the current protocol version and doesn't refer to different APIs.
</Aside>
...
The [event types section](/applications/api/toncenter/streaming/reference#event-types) and [notification schemas section](/applications/api/toncenter/streaming/reference#notification-schemas) provide the exact payload structure for each event.
...
The [delivery semantics section](/applications/api/toncenter/streaming/reference#delivery-semantics) and [event invalidation section](/applications/api/toncenter/streaming/reference#event-invalidation) document the exact behavior for each event type.
...
<Columns cols={2}>
  <Card
    ...
    icon="server"
    href="/applications/api/toncenter/streaming/sse"
  >
  ...
  <Card
    title="WebSocket"
    icon="radio"
    href="/applications/api/toncenter/streaming/wss"
  >
...
- [Notification reference](/applications/api/toncenter/streaming/reference)
- [API key](/applications/api/toncenter/get-api-key)
