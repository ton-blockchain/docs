...
If the wallet cannot fetch the manifest, the connect flow returns `MANIFEST_NOT_FOUND_ERROR` (code 2) or `MANIFEST_CONTENT_ERROR` (code 3). See [Manifest 404 and CORS](/applications/ton-connect/troubleshooting#manifest-404-and-cors).

For the full field reference, see [Manifest](/applications/ton-connect/core-concepts#manifest).
...
Next.js serves the file at `https://yourapp.com/tonconnect-manifest.json`. Make sure your hosting layer does not add CORS or auth gates — see [Manifest 404 and CORS](/applications/ton-connect/troubleshooting#manifest-404-and-cors).
...
See [Connect a wallet](/applications/ton-connect/how-to/connect) for the proof-verification flow.
...
- Authenticate the user with `ton_proof` → [Connect a wallet](/applications/ton-connect/how-to/connect)
- Send a structured transaction → [Send a transaction](/applications/ton-connect/how-to/send-transaction)
- Sign data on the wallet → [Sign data](/applications/ton-connect/how-to/sign-data)
- Gasless transactions → [Sign and relay a message (gasless)](/applications/ton-connect/how-to/sign-message-gasless)
- One-tap connect-and-pay → [Connect-and-act in one tap](/applications/ton-connect/how-to/embedded-request)
- Handle wallet-initiated disconnects → [Disconnect a session](/applications/ton-connect/how-to/disconnect)
- Filter wallets by feature → [Filter wallets by required features](/applications/ton-connect/how-to/filter-wallets)
- WalletConnect support → [Add WalletConnect support](/applications/ton-connect/how-to/walletconnect-support)
