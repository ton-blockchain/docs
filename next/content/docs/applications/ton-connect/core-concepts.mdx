---
id: ecosystem/ton-connect/core-concepts
...
For cross-device continuity, use `ton_proof` with a server-issued token — the user reconnects on each device, the backend verifies the proof, and the token follows the user. See [Authenticate with `ton_proof`](/applications/ton-connect/how-to/connect#authenticate-with-ton_proof).

Spec reference: [Session specification](https://github.com/ton-blockchain/ton-connect/blob/main/spec/session.md). See also [Disconnect a session](/applications/ton-connect/how-to/disconnect).
...
1. **Feature handling.** When the dApp declares required capabilities via [`walletsRequiredFeatures`](/applications/ton-connect/how-to/filter-wallets), the UI checks each entry's `features` array. Once a session opens, the SDK checks the runtime `DeviceInfo.features`, which is authoritative.
...
1. Deploy an HTTP bridge according to the [bridge spec](https://github.com/ton-blockchain/ton-connect/blob/main/spec/bridge.md), expose a JS bridge, or both.
...
Spec reference: [Session specification](https://github.com/ton-blockchain/ton-connect/blob/main/spec/session.md), [`ton_proof` signature specification](https://github.com/ton-blockchain/ton-connect/blob/main/spec/connect.md#address-proof-signature-ton_proof), and the [Bridge specification](https://github.com/ton-blockchain/ton-connect/blob/main/spec/bridge.md).
