---
id: ecosystem/ton-pay/payment-integration/payments-tonconnect
title: "How to send payments using TON Connect"
sidebarTitle: "Payments using TON Connect"
description: ""
---
...
The manifest URL must be publicly accessible and served over HTTPS in production. Replace `<APP_URL>` with the public HTTPS origin that serves `tonconnect-manifest.json`. [The manifest file](/applications/ton-connect/core-concepts#manifest) provides application metadata required for wallet identification.
...
<ResponseField
  ...
  required
>
  Recipient wallet address in [user-friendly format](/blockchain-basics/primitives/addresses/formats).
</ResponseField>
...
- Ensure the sender address from TON Connect matches the format expected by the backend. Use the [user-friendly format](/blockchain-basics/primitives/addresses/formats) consistently.
...
When using TON Connect with server-side message building, [the optional API key can be included](/applications/ton-pay/payment-integration/payments-tonconnect#api-key-configuration) in the backend:
...
<CardGroup cols={2}>
  <Card
    ...
    icon="webhook"
    href="/applications/ton-pay/payment-integration/transfer"
  >
