export const FeePlayground = () => {
  ...
  const compute = (form) => {
    ...
    const storageFeeNano = Math.ceil(
      ((accountBits * bit_ps + accountCells * cell_ps) * timeDelta) / bit16,
    );
  ...
  return (
    <form
      ref={init}
      onInput={(e) => compute(e.currentTarget)}
      className="not-prose my-4 p-4 border rounded-xl dark:border-white/20 border-black/10"
    >
        ...
        <div className="space-y-2">
          ...
          <label className="block text-xs">
            ...
            <input
              name="import_bits"
              defaultValue={528}
              className="mt-1 w-full border rounded-md px-2 py-1 bg-transparent"
              type="number"
            />
          </label>
          <label className="block text-xs">
      ...
      <div className="mt-4 text-sm grid grid-cols-1 md:grid-cols-2 gap-2">
        <div className="space-y-1">
          <div>
            Fwd. fee:{' '}
            <strong>
              <span data-out="fwd"></span> TON
            </strong>
          </div>
          <div>
            Gas fee:{' '}
            <strong>
              <span data-out="gas"></span> TON
            </strong>
          </div>
          <div>
            Storage fee:{' '}
            <strong>
              <span data-out="storage"></span> TON
            </strong>
          </div>
          <div>
            Action fee:{' '}
            <strong>
              <span data-out="action"></span> TON
            </strong>
          </div>
        </div>
        <div className="space-y-1">
          <div>
            Import fee:{' '}
            <strong>
              <span data-out="import"></span> TON
            </strong>
          </div>
          <div>
            Total fee:{' '}
            <strong>
              <span data-out="total"></span> TON
            </strong>
          </div>
        </div>
      </div>
