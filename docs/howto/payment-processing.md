...
To generate link to transaction in explorer, service need to get lt (logic time), transaction hash and account address (account address for which lt and txhash was retrieved via getTransactions method). ton.cx and explorer.ton.org may then show page for that tx in the following format:
