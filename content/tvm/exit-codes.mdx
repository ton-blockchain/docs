...
The action phase returns exit code `46` when a change-library action attempts to add a private library or when a regular account attempts any library change, including deployment of new libraries. Only [special masterchain accounts](/foundations/config#param-31-fee-exempt-contracts) can add or remove public libraries.
