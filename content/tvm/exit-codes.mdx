...
| [46](#46-library-change-action-is-prohibited)             | [Action phase][a]                   | Library change action is prohibited.                                                                   |
...
<Callout type="note">
  Read more about expecting specific exit codes: [Explore TVM logs](/contracts/blueprint/debug#explore-tvm-logs).
</Callout>
...
When the account lacks a requested [extra-currency](/foundations/extra-currencies) amount to send in an outbound internal message, an error with exit code 38 is thrown: `Not enough extra currencies`.
...
### 46: Library change action is prohibited

The action phase returns exit code `46` when a change-library action attempts to add a private library or when a regular account attempts any library change, including deployment of new libraries. Only [special masterchain accounts](/foundations/config#31) can add or remove public libraries.

### 50: Account state size exceeded limits

If the account state (contract storage, essentially) exceeds any of the limits specified in the [blockchain config param 43](/foundations/config#43) by the end of the [action phase](#action-phase), an error with exit code 50 is thrown: `Account state size exceeded limits`.
...
[tlb]: /foundations/tlb/overview
[tvm]: /tvm/overview
[bp]: /contracts/blueprint/overview
