...
Every internal message should have some Gram attached to it so that it can pay for the cost of handling it. External messages cannot have Gram attached to them because they spend some of their time outside the blockchain, and only the blockchain can prove the transfer of Gram is correct.
...
Usually, a transfer message is sent without an internal message in its payload and only instructs the wallet to transfer some Gram to a destination address. This is the reason this type of smart contract is called a wallet.
