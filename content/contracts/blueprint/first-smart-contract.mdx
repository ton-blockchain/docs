...
- Installed [wallet service](/onboarding/wallet-apps/web) with [Gram on testnet](/onboarding/wallet-apps/get-coins)
...
<Steps>
  <Step>
    ...
    Use the [Blueprint](/contracts/blueprint/overview) development toolkit for smart contracts. Start a new project with:
...
The TON ecosystem provides editor plugins with syntax support for IDEs and code editors. View them [here](/contracts/ide/vscode).
...
<Steps>
  <Step>
    ...
    - The method `sendDeploy` sends the first message with `stateInit`, the structure holding the contract's initial code and data, which triggers deployment. In practice, this can be an empty message with Gram attached.
  </Step>
  ...
  <Step>
    ...
    <Callout
      ...
      title="Funds at risk"
    >
      On-chain deployments spend Gram and are irreversible. Verify the network before executing. Use testnet for practice; use mainnet only for actual deployment.
    </Callout>

    Run the script with:

    ```bash
    npx blueprint run deployFirstContract --testnet --tonconnect --tonscan
    ```

    For flags and options, see the [Blueprint deployment guide](/contracts/blueprint/deploy).
  </Step>
  ...
  <Step>
    ...
    <Callout
      ...
      type="note"
    >
      If a wallet is not installed, check the [wallet section](/onboarding/wallet-apps/web) to select and install a wallet before deploying the contract. Make sure the wallet is funded with [Gram on the testnet](/onboarding/wallet-apps/get-coins).
    </Callout>
    ...
    You can view it at https://<EXPLORER_NAME>/kQBz-OQQ0Olnd4IPdLGZCqHkpuAO3zdPqAy92y6G-UUpiC_o
    ```

    The link opens the contract on a [blockchain explorer](/onboarding/explorers/overview) showing transactions, messages and [account states](/foundations/status).
...
<Steps>
  ...
  <Step>
    ...
    export async function run(provider: NetworkProvider) {
      ...
      // `toNano('0.05')` converts 0.05 GRAM to nanograms
    ...
    ```bash
    npx blueprint run sendIncrease --testnet --tonconnect --tonscan
    ```
    ...
    You can view it at https://<EXPLORER_URL>/transaction/fe6380dc2e4fab5c2caf41164d204e2f41bebe7a3ad2cb258803759be41b5734
    ```
  ...
  <Step>
    ...
    ```bash
    npx blueprint run sendReset --testnet --tonconnect --tonscan
    ```
