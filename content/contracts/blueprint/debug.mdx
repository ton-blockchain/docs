...
Of all [verbosity levels](/contracts/blueprint/testing/reference#verbosity-2), the two are the most useful:
...
## Debug with TVM TxTracer

Even when a contract executes successfully (exit code = `0`) with no errors, the actions may not produce the expected on-chain result. [TVM TxTracer](/tvm/tools/txtracer) replays the transaction and displays VM-level execution in detail.
...
1. Obtain the transaction hash from a [blockchain explorer](/onboarding/explorers).
1. Open [TVM TxTracer](/tvm/tools/txtracer) and enter the transaction hash.
1. Review the execution:
   - Inspect **Logs** for executed instructions and exceptions.
