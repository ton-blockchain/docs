---
title: "Testing smart contracts with Blueprint"
...
| ------------- | ------------------ | ---------------------------------- |
| Speed         | Instant execution  | \~1 second finality                |
| Cost          | Free               | Requires real Gram                 |
...
Use `printTransactionFees()` to print fee totals for every transaction in a trace. The [testing utility reference](/contracts/blueprint/testing/reference#printtransactionfees) defines its signature and parameters.

### Inspect transaction fees

Access compute and action fees through a generic transaction description:

```typescript
// Not runnable: use a transaction result from a Sandbox test.
const transaction = result.transactions[1];

if (transaction === undefined || transaction.description.type !== 'generic') {
    throw new Error('Expected a generic transaction');
}

const computeFees = transaction.description.computePhase.type === 'vm'
    ? transaction.description.computePhase.gasFees
    : undefined;
const actionFees = transaction.description.actionPhase?.totalActionFees;

if (computeFees === undefined || actionFees === undefined) {
    throw new Error('Expected compute and action fees');
}

expect(computeFees + actionFees).toBeLessThan(toNano('1'));
```
...
The [transaction matcher field reference](/contracts/blueprint/testing/reference#flattransaction) defines `destroyed`, `deploy`, `oldStatus`, and `endStatus`.

Transaction matchers expose compute-phase `exitCode` and action-phase `actionResultCode`. The [TVM exit code reference](/tvm/exit-codes#table-of-exit-codes) defines each code and the phase that emits it.
...
- [Debug Guide](/contracts/blueprint/debug) — Advanced debugging techniques
