...
- `params` — optional treasury parameters
  ...
  - `balance` — initial balance of the treasury (default: 1\_000\_000 GRAM if omitted)
...
Returns the current GRAM balance of the treasury.

**Parameters:**

- `provider` — `ContractProvider`

**Returns:** promise resolving to balance in nanograms
...
console.log(`Treasury balance: ${balance} nanograms`);
```
...
```typescript
// Create treasury with default 1M GRAM balance
const treasury = await blockchain.treasury('user');
...
- `value` — message value in nanograms
...
const conditionalMatch: FlatTransactionComparable = {
    value: (v) => v !== undefined && v >= toNano('0.1'), // At least 0.1 GRAM
    exitCode: (code) => code === 0 || code === undefined, // Success or no code
    totalFees: (fees) => fees !== undefined && fees < toNano('0.01') // Less than 0.01 GRAM fees
};
