...
| `response_destination` | `MsgAddress`        | address where to send a response with confirmation of a successful transfer and the rest of the incoming message Gram |
| `custom_payload`       | `Maybe ^Cell`       | optional custom data (which is used by either sender or receiver jetton wallet for inner logic)                       |
| `forward_ton_amount`   | `VarUInteger 16`    | the amount of nanograms to be sent to the destination address                                                         |
