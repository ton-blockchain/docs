...
also have an _owner (admin)_ who, by sending suitable messages to them, [mints new tokens](/contracts/standard/tokens/jettons/mint), closes the minting, changes metadata, or transfers the
admin rights to another contract.

Jetton master contracts also receive notifications about the [burning of tokens](/contracts/standard/tokens/jettons/burn).
When they receive such a notification, they must adjust the total supply accordingly.

### Jetton wallet

_Jetton wallet_ smart contracts are used to [transfer](/contracts/standard/tokens/jettons/transfer), receive, and [burn](/contracts/standard/tokens/jettons/burn) jettons. Each jetton wallet contract stores
...
They are designed to store only the native TON Blockchain currency: Gram (or GRAM).
...
<Image
  src="/images/jettons/transfer_light.png"
  darkSrc="/images/jettons/transfer_dark.png"
  alt="Jetton transfer"
/>
...
| `response_destination` | MsgAddress        | The wallet address used to return remaining Gram through the excesses message.                                                                                         |
...
| `forward_ton_amount`   | VarUInteger 16    | Gram to forward to the recipient. Must `> 0` to send a transfer notification with `forward_payload` and must be less than the amount attached to this message.         |
...
| `response_destination` | Maybe MsgAddress  | The optional wallet address used to return remaining Gram through the excesses message.                                                                                |
| `forward_ton_amount`   | VarUInteger 16    | Gram to forward to the recipient. Must `> 0` to send a transfer notification with `forward_payload` and must be less than the amount attached to this message.         |
| `forward_payload`      | Either Cell ^Cell | Optional data. If the first `32` bits equal `0x00000000`, it is a text comment.                                                                                        |
...
`Actor B Jetton wallet -> actor C smart contract`. Excess message body. This is sent only if there are remaining Gram after paying the fees. Contains the following data:
...
<Image
  src="/images/jettons/mint_light.png"
  darkSrc="/images/jettons/mint_dark.png"
  alt="Jettons minting"
/>
...
<Image
  src="/images/jettons/burn_light.png"
  darkSrc="/images/jettons/burn_dark.png"
  alt="Jettons burning"
/>
...
- [getting wallet balance](/contracts/standard/tokens/jettons/wallet-data);
- [transfer jettons](/contracts/standard/tokens/jettons/transfer);
- indexing available jettons.

### Applications

The most popular user wallet applications use their own SQL-like database. When the user receives a new jetton for the first time,
...
Various web services can also be used to obtain information about the types of user jettons and their amounts.
