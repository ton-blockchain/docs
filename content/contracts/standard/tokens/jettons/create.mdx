...
Alternatively, enter the `JETTON_MINTER_ADDRESS` value to any of the [TON explorers](/onboarding/explorers/overview), such as [Tonscan for Testnet](https://testnet.tonscan.org/).
...
Use the following steps **only** when WSL is **not** available. Deployment scripts require Node.js LTS version 22 or later.
...
Add the `vendor` folder and other sensitive files to `.gitignore`:
...
Download the Windows Tolk compiler `tolk.exe` and Fift interpreter `fift.exe` from the [latest TON release](https://github.com/ton-blockchain/ton/releases/latest), then place them in the `vendor` directory.

Check that PowerShell can run them:

```powershell
.\vendor\tolk.exe --version
.\vendor\fift.exe -V
```

Download and unarchive the `smartcont_lib.zip` from the [latest TON release](https://github.com/ton-blockchain/ton/releases/latest) in the `vendor` directory.

Set `TOLK_STDLIB` to the Tolk standard library directory:

```powershell
$env:TOLK_STDLIB = "$PWD\vendor\smartcont\tolk-stdlib"
```

Set `FIFTPATH` to the Fift library directory:

```powershell
$env:FIFTPATH = "$PWD\vendor\smartcont\lib"
```

### 3. Compile the jetton wallet

Compile `JettonWallet.tolk` into `.fif` first:

```powershell
.\vendor\tolk.exe `
  --path-mapping "@contracts=$PWD\contracts" `
  -o build\JettonWallet.fif `
  contracts\JettonWallet.tolk
```

Compile `JettonWallet.fif` into [BoC](/foundations/serialization/boc):

```powershell
.\vendor\fift.exe build\JettonWallet.fif
```

Check the produced artifact:

```powershell
Test-Path build\JettonWallet.compiled.boc
```
...
Convert the compiled jetton wallet BoC file to base64:

```powershell
$walletCodeBase64 = [Convert]::ToBase64String([IO.File]::ReadAllBytes("build\JettonWallet.compiled.boc"))
$walletCodeBase64 | Set-Content build\JettonWallet.compiled.boc.base64
```

### 4. Add the wallet code helper

Create `gen\JettonWallet.code.tolk`:

```powershell
$walletCodeBase64 = Get-Content build\JettonWallet.compiled.boc.base64
...
### 5. Compile the jetton minter

Compile `JettonMinter.tolk` into `.fif` first:

```powershell
.\vendor\tolk.exe `
  --path-mapping "@contracts=$PWD\contracts" `
  -o build\JettonMinter.fif `
  contracts\JettonMinter.tolk
```

Compile `JettonMinter.fif` into BoC:

```powershell
.\vendor\fift.exe build\JettonMinter.fif
```

Check the produced artifact:

```powershell
Test-Path build\JettonMinter.compiled.boc
```
...
async function main() {
  ...
  const code = Cell.fromBoc(readFileSync("build/JettonMinter.compiled.boc"))[0];
...
### 7. Deploy and mint from the terminal

Run the deployment script to deploy the jetton in the testnet:
