...
>
  ...
  Mitigation: Always retrieve and apply the correct `decimals` value. Test on testnet first. Read [decimals parameter](/contracts/standard/tokens/metadata#decimals) for details.
</Callout>

To attach a comment, the message has to encode it in `forward_payload` field, and `forward_ton_amount` is some amount of Gram attached to let the receiving wallet process the message.
...
A single manual transfer can be done with a web service — for example, [TON Minter](https://minter.ton.org/).

A programmatic transfer is usually done with an SDK that handles low-level message serialization details, like [`assets-sdk`](https://github.com/ton-community/assets-sdk). The provided example uses TON Center API that might [require a key](/api/get-api-key). Also, you'll need a mnemonic of a wallet that will pay for the transfer.
