...
1. Read the NFT item index from the item contract via [get\_nft\_data()](/contracts/standard/tokens/nft/api#get_nft_data).
1. Query the collection with [get\_nft\_address\_by\_index(index)](/contracts/standard/tokens/nft/api#get_nft_address_by_index).
1. If the returned address equals the NFT item address, the item belongs to the collection.

### High level

There is an [API method](/api/v3/nfts/get-nft-items?playground=open) that performs this verification off-chain with a single request. Provide the addresses of the NFT item and NFT collection:
