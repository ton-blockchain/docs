...
NFT transfer is an operation specified in [TEP 0062](/contracts/standard/tokens/nft/how-it-works#transfer-nft-item). Its implementation must comply with this standard, and NFT item contracts must support the logic described there. The NFT transfer message is also specified and must comply with the corresponding [TL-B scheme](/contracts/standard/tokens/nft/how-it-works#transfer-nft-item). This scheme involves, among other things,

- `forward_amount`: the amount of nanograms to be sent to the destination address;
...
Use a wallet app that supports NFTs and follow its guide. Alternatively, perform the transfer manually. The example below illustrates a programmatic transfer:
