...
<Image
  src="/images/nft/collection_light.png"
  darkSrc="/images/nft/collection_dark.png"
  alt="NFT collection"
/>
...
Following TON's [contract sharding](/contracts/techniques/contract-sharding) approach, each NFT item is its own smart contract account. It provides the collection address, index, current owner, and individual metadata. On a valid transfer from the current owner, it updates the owner, optionally notifies the new owner, and returns excess Gram to the specified address.

<Callout type="caution">
  Not every NFT that stores a collection address actually belongs to that collection. [Verify](/contracts/standard/tokens/nft/verify) that the collection returns the item's address for the item's index.
</Callout>

## Transfer NFT item

The current owner sends a transfer message to the NFT item contract. The item updates its owner field and, optionally, sends a notification and/or excess Gram to the specified addresses.
