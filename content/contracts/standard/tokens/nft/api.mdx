...
| `response_destination` | `MsgAddress`        | optional address to receive excess Gram, usually the sender's address |
| `custom_payload`       | `Maybe ^Cell`       | optional custom data                                                  |
| `forward_amount`       | `VarUInteger 16`    | the amount of Gram to be sent to the new owner                        |
...
<Callout type="caution">Verify that item belongs to collection via [get method in collection](/contracts/standard/tokens/nft/api#get_nft_address_by_index), see more [here](/contracts/standard/tokens/nft/verify)</Callout>
...
<Callout type="caution">Should be rejected if sender address is not the owner's address.</Callout>
...
<Callout type="caution">Should be rejected if sender address is not the owner's address. Sets the owner's address and authority to null, and sends an excesses message with the contract's balance.</Callout>
...
<Callout type="caution">Should be rejected if sender address is not the authority's address or if already revoked. Sets `revoked_at` to current unix time.</Callout>
...
Same as [NFT standard](/contracts/standard/tokens/nft/api#get_nft_data). No arguments. Outputs:
...
<Callout type="note">This method is mandatory for SBT. If there is no authority it should return `addr_none` (2 zero bits).</Callout>
