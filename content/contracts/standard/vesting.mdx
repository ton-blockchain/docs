...
- Recipients can return coins from the pool (exchange tokens back to Gram) during vesting
...
1. Find the jetton wallet address:
   ...
   <Image
     src="/images/vesting/tstakers_trace.png"
     alt="Tonstakers deposit trace"
   />

   <Image
     src="/images/vesting/tstakers_jwallet.png"
...
[Bemo](https://bemo.finance) is a liquid staking protocol in TON. When staking Gram, the protocol mints `bmTON` tokens that represent the staked position and automatically accumulate rewards.
...
- Staking and unstaking Gram is possible during vesting
...
1. Stake GRAMs by sending an empty message to the Financial contract:

   - Destination: Financial contract address above
   - Amount: desired stake amount (e.g., 100 GRAM)
...
1. Find the `bmTON` jetton wallet address:
   ...
   <Image
     src="/images/vesting/bemo_trace.png"
     alt="Bemo staking trace"
   />

   <Image
     src="/images/vesting/bemo_jwallet.png"
...
1. To withdraw (unstake), send a burn message to the `bmTON` jetton wallet:
   ...
   After burning `bmTON`, GRAM is returned to the vesting contract after a cooldown period. Check the current cooldown on [Bemo documentation](https://docs.bemo.fi).
...
[block explorer]: /onboarding/explorers/overview
