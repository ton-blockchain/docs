...
Gasless transactions execute wallet operations when the wallet owner does not hold Gram to pay [network fees](/foundations/fees). A relayer service and its on-chain contract submit transactions on behalf of users and attach Gram to cover gas costs.
...
- External messages: arrive from outside the blockchain. When a wallet contract receives an external message, it pays gas from its own Gram balance.
- Internal messages: arrive from other contracts. Gas is paid from Gram attached to the message.

Gasless transactions use internal messages: the relayer's contract sends an internal message with attached Gram to the wallet contract. The wallet executes actions without spending its own balance.
...
- Attaches sufficient Gram for gas
- Includes the user's signed payload as the message body
- Spends the relayer's Gram, not the user's
...
- Executes signed actions using attached Gram
...
- For high-value operations, use direct external messages when Gram is available
- Monitor for suspicious delays or failures

### Operational risks

- Insufficient gas: if the relayer attaches too little Gram, execution fails. The wallet's sequence number does not increment, so the user can retry with the same signed payload.
...
Users can switch to alternative relayers or use external messages once Gram is available.

## Example: jetton payment

Alice wants to send 100 USDT to Bob but has zero Gram. The relayer charges 0.5 USDT.
...
1. Sends internal message to Alice's wallet with 0.1 Gram attached
...
1. Executes both USDT transfers using attached 0.1 Gram for gas
1. Increments sequence number to 43

Result:

- Alice: 0 Gram (unchanged), -100.5 USDT
- Bob: +100 USDT
- Relayer: -0.1 Gram, +0.5 USDT

The relayer accumulates USDT fees and exchanges them for Gram periodically.
...
[TON Connect](/applications/ton-connect/overview) incompatibility: currently, TON Connect does not support gasless. The `sendTransaction` method does not return signed payloads to dApps — it submits transactions directly. Gasless requires obtaining signed payloads and forwarding them to relayers off-chain.
...
- Attach sufficient Gram to cover gas for wallet execution and outgoing actions
