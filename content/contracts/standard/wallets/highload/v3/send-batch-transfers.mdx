...
<Callout type="danger">
  **Funds at risk:** This guide sends real GRAM. Test on testnet first. Double-check recipient addresses — blockchain transactions cannot be reversed.
</Callout>
...
- Completed [wallet creation](/contracts/standard/wallets/highload/v3/create) with funded balance and saved configuration in `.wallet.json`
...
<Callout type="tip">
  **Batch limit:** You can send up to **254 messages** per transaction. This limit exists because one action slot is reserved for [`set_code` protection](/contracts/standard/wallets/highload/v3/specification#protection-against-set_code).
</Callout>
...
- See [Query ID structure](/contracts/standard/wallets/highload/v3/specification#query-id-structure) for details
...
- See [Timestamp validation](/contracts/standard/wallets/highload/v3/specification#timestamp-validation) for why this is necessary
...
- At current gas prices, this equals \~0.0007024 GRAM
...
See [Message sending flow](/contracts/standard/wallets/highload/v3/specification#message-sending-flow) for the complete validation sequence.

## Next steps

You can send multiple batches in parallel, each with a unique `query_id`.

To verify that your batch was fully processed, see [How to verify message is processed](/contracts/standard/wallets/highload/v3/verify-is-processed).
