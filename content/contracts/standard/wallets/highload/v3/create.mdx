...
<Callout type="caution">
  **Cannot be changed later:** `timeout` is part of the contract's [initial configuration](/contracts/standard/wallets/highload/v3/specification#storage-structure). Changing it would change the wallet address entirely.
</Callout>

See [Timeout constraints](/contracts/standard/wallets/highload/v3/specification#timeout-constraints) in the specification for details.
...
See [Storage structure](/contracts/standard/wallets/highload/v3/specification#storage-structure) in the specification for details.
...
<Callout type="danger">
  **Funds at risk:** You will send GRAM to this address. Test on testnet first. Verify the wallet address carefully — blockchain transactions cannot be reversed.
</Callout>

**Required before deployment:** Send GRAM to your wallet address (from Step 6) to prepare it for deployment.
...
Send GRAM using a faucet (testnet) or from another wallet (mainnet). After funding, the account transitions to `uninit` status — it has a balance and can accept external messages, but no code or data yet.
