...
Contracts that receive the initial external message are referred to as _receiver_ contracts. Other contracts in the same system are called _internal_ contracts. This terminology is local to this article.
...
In the system, the typical trace looks like this, with transactions moving from left to right:
...
<Callout type="note">
  The following explanation uses an abstract contract system and is not connected to any existing project. It is primarily applicable to systems that follow a carry-value pattern, where Gram is passed between contracts as part of message processing.
</Callout>

There is _no separate_ message balance and contract balance. After the message is received, the coins from the message are stored in the contract balance, and the contract is then executed. [Sending message modes](/foundations/messages/modes) and [reserve actions](/foundations/actions/reserve) help properly divide the contract balance during the action phase.

The following diagrams illustrate the possible value flow.
...
B,Storage fee (B),5
B,Compute fee (B),10
B,Forward fee (B->User),15
B,Remaining value on B,70
```

### Receiver requirements

Receiver contracts must verify that the attached Gram is sufficient to cover fees for all contracts in the subsequent trace. If an entry contract accepts an external message, it must guarantee that the message will not later fail due to insufficient attached Gram. "Accept" doesn't mean the call to `acceptExternalMessage()`, but semantic acceptance, i.e., no throw and no asset returns.

The reason for this requirement is that reverting the contract system state is usually not possible, because the Gram is already spent.

When a contract system's correctness depends on successful execution of the remaining transaction trace, it must guarantee that an incoming message carries enough attached Gram to cover all fees. This article describes how to compute those fees.
...
There are two kinds of values: gas units and Gram. The price of contract execution is fixed in gas units. However, the price of the gas itself is determined by the [blockchain configuration](/foundations/config).

Convert gas to Gram on-chain using blockchain configuration:
...
- `lumpPrice` is the fixed value [from config](/foundations/config#param-24-and-25-message-price) paid once for the message.
...
If intermediate `(cells, bits)` values are not required, use `msg.estimateFeeWithoutSending()`, allowing TVM to perform the same calculations as `calculateForwardFee()`. In TVM, this is implemented as [`SENDMSG`](/tvm/instructions#fb08-sendmsg) with the [`+1024` mode](/foundations/messages/modes) set, and consumes approximately the same amount of gas.

<Callout type="tip">
  To obtain an estimate and also send the message, use the `msg.sendAndEstimateFee()` function.
</Callout>
...
Always keep a minimum balance on all contracts in the system. Storage fees get deducted from this reserve. The reserve gets replenished with each external interaction. Do not hardcode Gram values for fees. Instead, hardcode the maximum possible contract size in cells and bits.
...
// NOTE: This contract requires code allowing the owner to withdraw Gram from it.
```
...
fun onInternalMessage(in: InMessage) {
    ...
    val otherFees = 0;
    // `ERR_NOT_ENOUGH_TON` means "not enough Gram attached".
...
It is impossible to know in advance what the storage fee due will be on the contract, so a threshold must be selected depending on the network configuration. It is a good practice to use [`freeze_due_limit`](/foundations/config#param-20-and-21-gas-prices) as the threshold. Otherwise, the contract likely is already frozen and a transaction chain is likely to fail anyway.

<Callout type="tip">
  Use [TON Unfreezer](https://unfreezer.ton.org/) to revive frozen contracts.
</Callout>
...
This confirms that all incoming value was consumed or forwarded, with none left behind. It helps identify any bugs that cause accumulation of Gram on any contract.
...
fun onInternalMessage(in: InMessage) {
    ...
    // IMPORTANT: each of these messages is less than or equal to `SwapRequest`.
    ...
    // `ERR_INSUFFICIENT_TON_ATTACHED` means "insufficient Gram attached".
    ...
    // contract if it is not supposed to hold incoming Gram.
