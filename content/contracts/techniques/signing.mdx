...
Cryptographic signatures are the foundation of access control. Contracts verify signatures on-chain and implement their own authorization policies for [wallet operations](/contracts/standard/wallets/how-it-works#how-ownership-verification-works), server-authorized actions, [gasless transactions](/contracts/standard/wallets/v5#gasless-transactions), multisig wallets, and delegation.

## Ed25519 in TON

TON uses [Ed25519](https://en.wikipedia.org/wiki/EdDSA#Ed25519) as the [standard](/foundations/whitepapers/tblkch#a-3-10-cryptographic-ed25519-signatures) signature scheme. All [wallets](/contracts/standard/wallets/history) (v1–v5) and [highload wallets](/contracts/standard/wallets/highload/overview) rely on Ed25519.
...
Standard wallet contracts are [described in more detail](/contracts/standard/wallets/history) in the wallets section.
...
1. Service sends internal message to user's wallet and pays gas in GRAM.
...
- Who pays gas: service (in GRAM), gets compensated in jettons

This pattern enables users to pay gas in jettons instead of GRAM.
...
const messageBodyInline = beginCell()
    ...
    .storeSlice(signedData.beginParse())  // Signed data as slice
    .endCell();
