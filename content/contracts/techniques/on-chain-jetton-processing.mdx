...
<Callout type="note">
  Understand [the jetton architecture](/contracts/standard/tokens/jettons/overview) before reading this article.
</Callout>
...
If the jetton type is known in advance, [calculate the corresponding jetton wallet address off-chain](/contracts/standard/tokens/jettons/find) and set it via a message after deployment. It is impossible to hardcode the jetton wallet address in the contract’s [`StateInit`](/foundations/messages/deploy) because the wallet address depends on the contract address and creates a circular dependency. This is the most efficient approach, because it only requires one address comparison.
...
<Callout type="tip">
  Most popular DEXes use automatic wallet discovery.
</Callout>
...
<Callout type="note">
  Jettons that do not implement the TEP-89 messages for computing a wallet address on-chain are rare. Some DEXes reject them, while other platforms allow them only after manual approval.
</Callout>
...
Minter address and `StateInit` will match if the jetton minter was not deployed with [a vanity contract](/contracts/techniques/vanity). The logic for the contract that handles `JettonNotify` messages is thus:
