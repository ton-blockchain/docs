...
- `amount` — the amount of nanograms to send
...
**Returns:** a URL deep link that can be opened in GRAM wallets

**Example:**

```typescript
const link = tonDeepLink(myAddress, 10_000_000n); // 0.01 GRAM
// "ton://transfer/..."
```
...
- `explorer` — the desired explorer (`tonscan`, `toncx`, `dton`)
...
export async function run(provider: NetworkProvider) {
  ...
  console.log(`Contract balance: ${fromNano(state.balance)} GRAM`);
}
```
...
export async function run(provider: NetworkProvider) {
  const ui = provider.ui();
  const confirmed = await ui.prompt('Deploy to mainnet? This will cost real GRAM');
  if (confirmed) {
