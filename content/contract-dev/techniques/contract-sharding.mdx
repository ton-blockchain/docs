...
Some protocols need to store a lot of information in contracts, for example, token contracts with many users. In TON, there is a [limit `max_acc_state_bits`](/foundations/limits#message-and-transaction-limites) on how much can be stored in a single contract. The solution is to split the data across multiple contracts.
