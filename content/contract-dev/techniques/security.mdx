...
Improper handling of signed integers can allow attackers to exploit overflow or underflow conditions.
...
```tolk
fun transferVotingPower(mutate votes: cell, from: slice, to: slice, amount: uint /* prefer unsigned */): void {
    var fromVotes = getVotingPower(votes, from);
    var toVotes = getVotingPower(votes, to);

    // In case `amount` needs to be a signed integer, add:
