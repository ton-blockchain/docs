...
Read more: [Randomness in TON](/contracts/techniques/random)

## Oracles in TON

Since the TON execution model is asynchronous, the classic ways to interact with oracles, e.g., get methods during a transaction, [cannot be applied here](/from-ethereum). The best pattern to retrieve data from an oracle on TON is the Request-Response pattern - you send an internal message to the oracle contract and verify the response, getting the needed data.
...
<Image
  src="/images/oracles/oracle-flow.svg"
  alt="oracle-flow"
/>
