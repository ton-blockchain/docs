...
Use wallet-specific links to create simple jetton and Gram transfers. The syntax is usually similar to `ton://transfer` links, but always check each wallet’s documentation.
...
Find more wallet-specific features in the documentation: [MyTonWallet docs at `help.mytonwallet.io`](https://help.mytonwallet.io/intro/key-features/deeplinks-in-mytonwallet).

## Explorers links

To generate a transaction link, the service must obtain the account address, logical time (lt), and transaction hash by calling the [`getTransactions`](https://toncenter.com/api/v2/#/transactions/get_transactions_getTransactions_get) method. Using these values, [Tonscan](https://tonscan.org) and [Gram Explorer](https://explorer.toncoin.org/) render the transaction page according to the following formats respectively:

- [Tonscan links](#tonscan)
- [Gram Explorer links](#gram-explorer)
...
```bash title="Tonscan Testnet link example"
...
```bash title="Mainnet link example"
...
```bash title="Testnet link example"
