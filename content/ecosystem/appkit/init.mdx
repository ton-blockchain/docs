...
The [basic setup](#initialization) allows for direct API requests but is insufficient for connecting GRAM wallets via the TON Connect protocol. For that, configure a connector and host a public [app manifest file](/ecosystem/ton-connect/manifest#app-manifest) adjusted to the needs of a given dApp.
...
<CodeGroup>
  ...
  import {
    AppKit,
    AppKitProvider,
    // Enables GRAM wallet connections
...
The [setup with connectors](#connectors) connects to GRAM wallets via services such as [Tonkeeper](/ecosystem/wallet-apps/tonkeeper). To enable advanced DeFi operations like asset swaps or staking, or to set up a [streaming API](/ecosystem/api/toncenter/streaming/overview) service, configure providers.
...
- [TON Center Streaming API v2](/ecosystem/api/toncenter/streaming/overview). Has no additional dependencies.
...
<CodeGroup>
  ...
  const kit = new AppKit({
    ...
    connectors: [
      // Enables connections with GRAM wallet services, such as Tonkeeper or MyTonWallet.
      createTonConnectConnector({
        tonConnectOptions: {
          // Public link to the application manifest JSON file.
          // For local development and testing, use the one from a demo dApp:
...
To add a [streaming API](/ecosystem/api/toncenter/streaming/overview) provider, register it via `kit.streamingManager`:
...
Lower-level TON Connect libraries, such as `@tonconnect/ui-react` and `@tonconnect/ui`, only give access to wallet services via the TON Connect protocol and basic facilities to query connected GRAM wallets with direct API calls.
