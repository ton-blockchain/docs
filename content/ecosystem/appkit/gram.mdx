---
title: "How to work with Gram using AppKit"
...
Balances are returned as strings representing fractional Gram units. For example, `'0.1'` is a balance of 0.1 Gram or 100000000 nanograms. Decimal precision of Gram is 9 digits, so the smallest possible balance is `'0.000000001'` Gram or 1 nanograms.
...
<CodeGroup>
  ...
  export const BalanceCard = () => {
    ...
    return (
      <p>GRAM wallet balance in fractional Gram: {balance ?? '0'}</p>
    );
  };
  ```
  ...
  if (balance) {
    console.log('GRAM wallet balance in fractional Gram:', balance);
  }
  ```
</CodeGroup>
...
<CodeGroup>
  ...
  // Pre-built UI component for sending Gram transactions by clicking a button.
  ...
  export const SendToncoin = () => {
    ...
    // For example, '0.1' or '1' Gram.
    const gram = '<FRACTIONAL_AMOUNT>';

    return (
      <SendTonButton
        // Where to send Gram to
        ...
        // Defaults to Send X GRAM, where X is the amount
        text="Send GRAM"
  ...
  export const SendToncoin = () => {
    ...
    const handleTransfer = () => {
      transfer({
        // Where to send Gram to.
        // For example: 'UQ...'
        recipientAddress: '<GRAM_WALLET_ADDRESS>',
    ...
    return (
      <div>
        <button onClick={handleTransfer} disabled={isPending}>
          {isPending ? 'Transferring...' : 'Transfer GRAM'}
        </button>
