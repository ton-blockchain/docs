...
For example, USDT on TON is implemented as a jetton, and its minter contract address is `EQCxE6mUtQJKFnGfaROTKOt1lZbDiiX1kCixRv7Nw2Id_sDs`. By providing this address and the recipient's GRAM wallet contract address, AppKit knows which tokens to send and to whom.
...
<CodeGroup>
  ...
  export const JettonWalletAddressCard = ({ jettonAddress }) => {
    ...
    const {
      ...
      // GRAM wallet address of the jetton holder
      ownerAddress: ownerAddress ?? '<GRAM_WALLET_ADDRESS>',
  ...
  async function fetchJettonWalletAddress(
    ...
    const address = await getJettonWalletAddress(kit, {
      jettonAddress,
      // GRAM wallet address of the jetton holder
      ownerAddress: selectedWallet?.getAddress() ?? '<GRAM_WALLET_ADDRESS>',
    });
...
Unlike Gram, the balance units and decimal places vary between jettons — use the `decimals` field from the [jetton's metadata](#metadata) to interpret raw amounts correctly.
...
Check the balance of a specific jetton for the connected GRAM wallet or an arbitrary address:
...
Retrieve every jetton held by the connected GRAM wallet or an arbitrary address:
...
<CodeGroup>
  ...
  export const JettonListByAddress = () => {
    ...
    const {
      ...
      // GRAM wallet address of the jetton holder
      address: address ?? '<GRAM_WALLET_ADDRESS>',
    });
  ...
  async function fetchJettonsByAddress(
    ...
    const response = await getJettonsByAddress(kit, {
      address: selectedWallet?.getAddress() ?? '<GRAM_WALLET_ADDRESS>',
    });
...
<CodeGroup>
  ...
  export const JettonBalanceCard = ({ jettonAddress }) => {
    ...
    useWatchJettonsByAddress({
      address: ownerAddress ?? '<GRAM_WALLET_ADDRESS>',
    });
...
Before making a transfer, make sure there is enough Gram in the balance to cover the [fees](/foundations/fees).
...
<CodeGroup>
  ...
  export const SendJetton = () => {
    // For example: 'UQ...'
    const recipientAddress = '<GRAM_WALLET_ADDRESS>';
    ...
    return (
      <SendJettonButton
        ...
        jetton={{
          ...
          // Jetton decimals to calculate raw unit amounts
          // For example, USDT defaults to 6, while Gram to 9:
          decimals: 6,
        }}
