---
title: "How to stake Gram using AppKit"
sidebarTitle: "Stake Gram"
---
...
AppKit supports on-chain liquid Gram staking through [pluggable staking providers](#available-providers). For most user-facing apps, the flow is:
...
<CodeGroup>
  ...
  export const StakingQuoteCard = () => {
    ...
    const { data: quote, isLoading, error } = useStakingQuote({
      ...
      // Staking user's GRAM wallet address
      userAddress: address ?? '<GRAM_WALLET_ADDRESS>',

      // Fractional Gram amount string.
      // For example, '0.1' or '10' Gram.
      amount: '1',
    });
    ...
    return (
      <div>
        ...
        <p>Stake amount: {quote?.amountIn} Gram</p>
        <p>Estimated output: {quote?.amountOut} Gram</p>
      </div>
    );
  };
  ```
  ...
  async function getStakeQuote(
    ...
    console.log('Stake amount:', quote.amountIn, 'Gram');
    console.log('Estimated output:', quote.amountOut, 'Gram');
...
<CodeGroup>
  ...
  export const StakeButton = () => {
    ...
    return (
      ...
      >
        {isBuilding || isSending ? 'Submitting...' : 'Stake 1 GRAM'}
      </button>
    );
  };
  ```
