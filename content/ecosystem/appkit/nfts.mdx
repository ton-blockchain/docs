...
Retrieve NFTs held by the connected GRAM wallet or an arbitrary address. If there are many held NFTs, provide the `limit` option to retrieve fewer items and the `offset` option to paginate the item lists.
...
<CodeGroup>
  ...
  export const NftListByAddress = () => {
    ...
    const {
      ...
      // GRAM wallet address of the NFT holder
  ...
  export const SendNft = ({ recipientAddress, nftAddress }) => {
    ...
    const handleTransfer = () => {
      transfer({
        ...
        // (optional) Additional Gram sent to recipient.
