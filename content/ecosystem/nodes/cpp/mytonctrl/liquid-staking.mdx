...
- Signs the `top-up.boc` script with the validator wallet and sends `<amount-gram>` TON to `<controller-addr>`.
...
- Triggers a withdrawal for `[amount-gram]` TON; when omitted, withdraws almost the full balance (balance minus \~10.1 GRAM to keep the contract alive).
