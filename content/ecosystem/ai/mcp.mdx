...
- Balance queries: check Gram and jetton (token) balances, view transaction history
- Transfers: send Gram, jettons, and NFTs
...
<Steps>
  <Step>
    ...
    1. On the dashboard, the user should connect their regular mainnet or testnet GRAM wallet: it will be the owner of an agentic wallet.
...
The default GRAM wallet version in the single-wallet mode is [`v5r1`][wallets-comparison]. Override it with `WALLET_VERSION`:
...
Serverless mode operates in [single-wallet mode](#single-wallet-mode) only and always uses the [`v5r1` GRAM wallet version][wallets-comparison]. It does not use the wallet registry, nor does it expose wallet management or onboarding tools.
...
- Balances and history:
  - `get_balance` - returns the Gram balance of the current wallet.
  - `get_balance_by_address` - returns the Gram balance for any address.
  ...
  - `get_transactions` - returns recent transaction history: Gram transfers, jetton transfers, swaps.
...
- Transfers:
  - `send_ton` - sends Gram to an address. Amounts are in human-readable format. For example, `"1.5"` for 1.5 Gram. Returns a `normalizedHash`.
  ...
  - `send_raw_transaction` - sends a raw transaction with full control over messages. Amounts are in nanograms. Supports multiple messages. Returns a `normalizedHash`.
- Swaps:
  - `get_swap_quote` - returns a quote for a token swap. The response includes transaction parameters ready for `send_raw_transaction`. Use `"GRAM"` for native Gram or a jetton minter (master) contract address.
- NFTs:
...
<ParamField
  ...
  default="v5r1"
>
  GRAM wallet version to use in the single-wallet mode: [`v5r1`, `v4r2`][wallets-comparison], or [`agentic`][wallets].
