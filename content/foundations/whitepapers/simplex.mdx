---
title: "Catchain 2.0: Simplex Consensus in TON"
description: "Whitepaper by the TON Core team"
---

**Authors**: TON Core team <br />
**Date**: March 10, 2026 <br />
<Icon icon="file-pdf" size={16} />: [Original whitepaper site](https://ton-blockchain.github.io/simplex-docs/)
...
_Byzantine_. Define the quorum threshold $q = \lfloor 2W/3 \rfloor + 1$.
...
1. _Genesis validity._ $\mathsf{ValidSeq}(\varnothing) = \mathsf{true}$.
1. _Extendability._ For every valid sequence $(d_1,\dots,d_m)$, there exists some $d$ such that $\mathsf{ValidSeq}(d_1,\dots,d_m,d)$ holds.

A sequence $(d_1,\dots,d_m)$ is called _valid_ if $\mathsf{ValidSeq}(d_1,\dots,d_m) = \mathsf{true}$.

Consensus does not depend on the internal structure of payloads beyond these properties.

### 1.3 Protocol Objects

**Slots.** _Slot number_ is a discrete index $s \in \mathbb{N}_0$. Slots are grouped into _leader windows_ of size $L$. Window $k$ covers slots $[kL, (k+1)L)$ with an associated leader $v_{(k \bmod n)+1}$.

**Candidates.** A _candidate_ is a tuple $(s, d, s_p, h_p, \sigma)$ where
...
**Chains.** A _chain_ ending at $(s_m, h_m)$ is a sequence of candidates where each candidate's
...
The _slot set_ of a chain is $\{s_1, \dots, s_m\}$.

The associated sequence $(d_1,\dots,d_m)$ is called the _chain state_.

A candidate $(s,h)$ is called _valid_ if there exists a chain ending at $(s, h)$ with a state that is valid.

**Votes.** At any point, a validator may cast a _vote_ signed by its signing key certifying some statement $S$. Votes can be sent over the network.

**Certificates.** A _certificate_ for a statement $S$ is a set of votes for $S$ from distinct validators with total
weight $\ge q$.

We say that $S$ is _reached_ in the network if a certificate for $S$ can be produced by an oracle that knows all cast votes. We say that $S$ is _observed_ on validator $v$ when validator $v$ receives enough votes to
...
- $\mathsf{Notar}(s,h)$. Validator $v$ has not cast a vote for $\mathsf{Notar}(s, h')$ for $h \ne h'$, and there exists a candidate $(s,d,s_p,h_p,\sigma)$ identified by $(s,h)$ such that:
  ...
  1. for each slot $s_p < s' < s$, the statement $\mathsf{Skip}(s')$ is reached,
  1. the candidate $(s,h)$ is valid.
...
_Proof._ Suppose both are reached. Each certificate has total signing weight $\ge q$. Since
...
_Proof._ If $\mathsf{Notar}(s,h_1)$ and $\mathsf{Notar}(s,h_2)$ are both reached with
...
_Proof._ The certificate for $\mathsf{Final}(s,h)$ has honest signing weight $> 0$ (since
...
_Proof._ Suppose $\mathsf{Final}(s,h_1)$ and $\mathsf{Final}(s,h_2)$ are both reached with
...
_Proof._ By induction on chain length. For the head, $\mathsf{Notar}(s,h)$ is reached by
...
_Proof._ If $s_a = s_b$, then by Lemma 2.4, $h_a = h_b$.
...
$\mathsf{Final}(s^*, h^*)$, or $(-1, \varnothing)$ if no finalization has been observed. The _output
log_ of $v$ is the chain state of the chain ending at $(s^*, h^*)$.

**Corollary 2.7 (Consistency).** Any two honest validators' output logs are such that one is a
prefix of the other.

_Proof._ Let $(s_a^*, h_a^*)$ and $(s_b^*, h_b^*)$ be the latest finalized identifiers observed by
...
**Rule 1 (Frontier tracking).** A slot $s$ is _cleared_ for $v$ if $v$ has observed
...
When $F_v$ crosses a window boundary $kL$, window $k$ becomes _active_ for $v$.
...
state of the chain at $(s,h)$ (or simply, _resolve the state at $(s,h)$_) by following
...
1. $s_p < kL$,
1. $\mathsf{Notar}(s_p,h_p)$ is reached,
1. $\mathsf{Skip}(s')$ is reached for every $s_p < s' < kL$.
...
1. $\mathsf{Notar}(s_p,h_p)$ is reached.
1. For every slot $s'$ with $s_p < s' < s$, $\mathsf{Skip}(s')$ is reached.
1. Candidate $(s,h)$ is valid.
...
1. $\mathsf{Notar}(s,h)$ is reached,
1. $v$ has not voted $\mathsf{Skip}(s)$.
...
1. the certificate for $\mathsf{Final}(s,\cdot)$ with largest slot $s$ that $v$ has
   observed (unless $s=-1$),
1. all certificates $v$ holds for slots $> s$,
1. all votes cast by $v$ for slots $> s$.
...
- **Good phase** ($t \ge T_\mathsf{GST}$). Each message sent by an honest validator to
  ...
  with probability $1-r$ and lost otherwise. Here $r \in [0,1)$ is the _drop rate_.
...
_Proof._ Under $E_0$, no honest validator ever observes a new finalized block after time
...
_Proof._ Proceed by induction on $s > s_f$.
...
1. No honest validator ever observes any $\mathsf{Notar}(s,\cdot)$.
...
_Proof._ Consider one missing candidate on the chain. Because the chain is certified
...
_Proof._ Fix such a window $k$, and let $u$ be its honest leader. Let $\tau_k := T_0 \alpha^{k-k_f-1}$, the lower bound from Rule 6 for skip timeouts in window $k$.
...
1. the candidate reaches a quorum-weight honest set $H_k$,
1. the leader's proof of eligibility reaches every validator in $H_k$ via standstill
   broadcasts,
1. every validator in $H_k$ resolves the base state quickly,
1. notarization and finalization votes are exchanged quickly enough.
...
1. $c_k$ delivers the resulting notarization certificate to every validator in $H_k$
   within $\delta$;
1. every validator in $H_k$ delivers its finalization vote to $c_k$ within $\delta$.
...
1. the candidate reaches an honest set $H_k$ of total weight at least $q$ within
   $\Delta_\mathrm{bcast}$;
1. by time $\tau_k/2$, every validator in $H_k$ has received all certificates in $P_k$;
1. by time $\tau_k/2 + \tau_k/4$, every validator in $H_k$ has resolved the base state;
1. $E_\mathrm{vote}$ occurs.
...
_Proof._ Let $E_0$ be the event that no slot larger than $s_f$ is ever finalized.
...
_Proof._ Define a sequence of random slots $(S_j)_{j \ge 0}$ recursively by $S_0 := -1$, and for each $j \ge 0$, let $S_{j+1}$ be the smallest slot strictly larger than $S_j$
...
1. A masterchain leader generates empty candidates when the next block to produce would be more
