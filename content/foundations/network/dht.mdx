...
The [Distributed Hash Table (DHT)](https://en.wikipedia.org/wiki/Distributed_hash_table) in TON is a Kademlia-like distributed key-value database used to discover nodes in the network. Any network participant can operate a DHT node, generate keys, and store data.
