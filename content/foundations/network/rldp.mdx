...
The Reliable Large Datagram Protocol (RLDP) runs on top of [ADNL UDP](/foundations/network/adnl-udp) and transfers data that does not fit in one ADNL datagram. It uses Forward Error Correction (FEC) to recover from packet loss. RLDP2 also uses acknowledgment packets, loss detection, pacing, and congestion control.

<Callout type="note">
  The [sub-second node architecture](/ecosystem/subsecond) uses [QUIC](https://en.wikipedia.org/wiki/QUIC) for internal node communication. In v2026.06, the C++ `rldp-http-proxy` uses RLDP2 for TON Sites.
</Callout>
...
The C++ RLDP2 connection defaults to a 7,680-byte inbound limit. A caller can configure a larger peer MTU, and `send_query_ex` registers `max_answer_size` as the limit for the answer transfer.

## Transfer mechanism

1. The sender generates a random `transfer_id`.
1. The sender splits the payload into parts and creates a RaptorQ encoder for each part. The C++ RLDP2 implementation uses parts of up to 2,000,000 bytes and 768-byte symbols.
...
1. The sender uses this feedback to estimate round-trip time, bandwidth, and loss. It paces symbols and adjusts its congestion window with a BBR-style controller.
...
The TL schema defines RoundRobin, Online, and RaptorQ FEC types. The C++ RLDP2 implementation accepts only [RaptorQ](https://www.qualcomm.com/media/documents/files/raptorq-technical-overview.pdf); its FEC parser rejects the other constructors.
...
[RaptorQ encoder setup in the C++ RLDP2 implementation](https://github.com/ton-blockchain/ton/blob/v2026.06/rldp2/OutboundTransfer.cpp#L27-L40).
...
The v2026.06 C++ proxy uses bit 0 (value 1) in the `http.proxy.getCapabilities` and `http.proxy.capabilities` exchange to advertise RLDP2 support. It requests payload chunks of up to 2,095,104 bytes and caps larger `max_chunk_size` values at that size.

## TON Site request example

This example demonstrates requesting `foundation.ton`, assuming the ADNL address has been resolved via DNS and a connection has been established over [ADNL UDP](/foundations/network/adnl-udp). The v2026.06 C++ proxy requires the peer to advertise RLDP2 support.
...
1. The sender uses the 768-byte symbol size used by the C++ RLDP2 implementation.
...
- If `no_payload` is `false`, the client fetches the body separately:
  ...
  1. The client sends an `http.getNextPayloadPart` request with incrementing `seqno`. The C++ proxy requests at most 2,095,104 bytes per chunk.
