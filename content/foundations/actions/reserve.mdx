...
_Reserving coins_ is one of the actions that a smart contract can perform during the [action phase](/foundations/phases#action-phase). In TVM it is implemented via the [`RAWRESERVE`](/tvm/instructions#fb02-rawreserve) instruction that takes from the stack an `amount` of Gram (in nanograms) to reserve and an integer `mode` specifying a way of the reservation. The reservation action is queued to the _output action list_ of a smart contract, which contains other actions such as _message sends_.

The `RAWRESERVE` instruction is equivalent to creating an outbound message carrying the specified `amount` of nanograms to oneself. But unlike regular sending of a message, the reservation action does not create a new message and does not incur any forward fees. Its primary goal is to limit the amount of Gram that can be spent by subsequent actions.
...
| :--------: | ------------------ | ----------------------------------------------------- |
|     `0`    | `ReserveExact`     | Reserves exactly the specified `amount` of nanograms. |
|     `1`    | `ReserveAllExcept` | Reserves all but the specified `amount` of nanograms. |
|     `2`    | `ReserveAtMost`    | Reserves at most the specified `amount` of nanograms. |
...
- `amount` – the amount of Gram passed to the `RAWRESERVE` instruction.
...
- `original_balance` = `2` GRAM;
- `remaining_balance` = `3` GRAM.
...
1. `mode` has flag `ReserveAddOriginalBalance`? Yes.
   ...
   - Thus, `reserve` = `original_balance - reserve` = `2 - 0.1` = `1.9` GRAM.
1. `mode` has flag `ReserveAtMost`? No.
1. `mode` has flag `ReserveAllExcept`? Yes.
   - Thus, `reserve` = `remaining_balance - reserve` = `3 - 1.9` = `1.1` GRAM.
1. `remaining_balance` = `remaining_balance - reserve` = `3 - 1.1` = `1.9` GRAM.
...
- If after step 4, `reserve` is greater than `remaining_balance`, then the error [`37`](/tvm/exit-codes#37%3A-not-enough-grams) ("Not enough GRAMs") is thrown.
