...
Validators classify an account as special when it resides in the masterchain and its address appears in this parameter. The [configuration smart contract](#param-0-config-address) and the [elector smart contract](#param-1-elector-address) are also special, even when their addresses do not appear in parameter `31`.
...
- `max_vm_data_depth`: Maximum [cell depth](#standard-cell-representation-and-its-hash) in messages and account state.
