...
Validators enforce TL-B validity only for configuration parameters with non-negative indices. Values with negative indices are **not** validated against a specific `ConfigParam i` type. These values are also referred to as negative parameters or _extension slots_.
...
| `-123`                  | [Telegram wallet contract][tgw] bytecode used by the Telegram's native Gram wallet.                            |
...
[tgw]: https://github.com/ton-blockchain/tg-wallet-contract

<Callout type="note">
  Not every slot appears on every network or at every block. Inspect active slots in [mainnet](https://actonscan.com/config?network=mainnet) and [testnet](https://actonscan.com/config?network=testnet).
</Callout>
