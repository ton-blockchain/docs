...
The parameter values in the [current configuration](https://tonscan.org/config), and the method of writing them into [cells](/foundations/serialization/boc) are outlined in the [`block.tlb`](https://github.com/ton-blockchain/ton/blob/master/crypto/block/block.tlb) file in [TL-B](/languages/tl-b/overview) format.
