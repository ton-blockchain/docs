...
- `library`: the account's initial library dictionary. Since [global TVM version](/foundations/config#8) 15, this field must be empty when deploying a new account. Otherwise, deployment would fail unless the [contract is special](/foundations/config#31).
...
The largest value for `fixed_prefix_length` is set in the [blockchain config param 43](/foundations/config#43), and as of writing this, it is `8`, i.e., a discrepancy in only eight bits of the address can be ignored.
