...
- `library`: the account's initial library dictionary. Since [global TVM version 15](/foundations/config#param-8-network-version), this field must be empty when deploying a new account. Otherwise, deployment would fail unless the [contract is special](/foundations/config#param-31-fee-exempt-contracts).
...
The largest value for `fixed_prefix_length` is set in the blockchain config ([param 43](/foundations/config#param-43-account-and-message-limits)), and at the moment is `8`, i.e. discrepancy in only eight bits of address can be ignored.
