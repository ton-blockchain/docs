---
title: "Messages and transactions overview"
...
- When an incoming external message is processed, a transaction is created only if the contract [accepts the message](/tvm/accept#external-message-outcomes).
...
The `total_fees` field stores the total [fees](/foundations/fees) paid by the transaction. Its type supports [extra currencies](/foundations/extra-currencies), but blockchain fees are charged only in Gram.
