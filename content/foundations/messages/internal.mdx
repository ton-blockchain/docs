...
- `value` — amount of Gram and extra-currency attached to the message.
...
- there is enough Gram to send it left from handling the failed message,
...
- The `bounce` flag is set to `false` to prevent infinite loop of bouncing messages until no Gram is left to spend.
