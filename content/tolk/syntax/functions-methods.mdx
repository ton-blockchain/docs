...
The same convention has long been allowed for [`void`-typed struct fields](/tolk/types/void-never#void-typed-fields).
...
To allow modifications, declare [`mutate self`](/tolk/syntax/mutability) explicitly.
...
The [compiler automatically inlines](/tolk/features/compiler-optimizations#auto-inline-functions) small functions and functions called once. An early `return` does not block inlining when control flow leaves a single path into the code below. For example:
...
- `@inline` forces a function to be inlined in-place. If impossible, compilation fails. The compiler already inlines small functions and functions called once, so the annotation is for the remaining cases.
...
- `@pure` is valid only on an [`asm` function](/tolk/features/asm-functions). Pure calls may be reordered.
...
Tolk supports declaring low-level [assembler functions](/tolk/features/asm-functions) with embedded Fift code:
...
Tolk supports first-class functions: [they can be passed as callbacks](/tolk/types/callables).
...
Lambdas can capture outer variables by value, becoming [closures](/tolk/types/callables#closures-lambdas-with-captures).
