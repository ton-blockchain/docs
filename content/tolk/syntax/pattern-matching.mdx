...
Pattern matching on union types is used in [message handling](/tolk/features/message-handling).
...
`else` is not allowed for unions, but is permitted for a [`lazy` match](/tolk/features/lazy-loading#lazy-matching).
...
Pattern matching on [enums](/tolk/types/enums) requires exhaustive coverage of all cases:
