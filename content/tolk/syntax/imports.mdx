...
<Callout type="tip">
  When selecting items from auto-completion, [IDE](/contracts/ide/vscode) inserts imports automatically. This works in both JetBrains-based and VSCode-based IDEs.
</Callout>
...
1. [Prefer methods to global-scope functions](/tolk/idioms-conventions#prefer-methods-to-functions).

## Repeated symbols across contracts

When developing multiple contracts, each contract has its own file and compilation target. Place a [`contract` declaration](/tolk/features/contract-abi) in every entrypoint file:
