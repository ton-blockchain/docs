...
- Use `match` expression for [pattern matching](/tolk/syntax/pattern-matching).
...
A condition must be a [`bool`](/tolk/types/booleans). Integers are not implicitly converted — write the comparison explicitly:
...
The same rule applies to `while`, `do while`, ternary, [`assert`](/tolk/syntax/exceptions#assert-statement), and the unary `!` operator. There is no gas overhead — the compiler strips the `!= 0` comparison off whenever it would translate to a redundant TVM instruction.
...
`assert` throws an [exceptions](/tolk/syntax/exceptions) if a condition is false.
...
`match` is used to perform different actions for different values of a variable. A common use case is routing values of a [union type](/tolk/types/unions):
...
`break` leaves the nearest `while`, `do-while`, or `repeat`. `continue` skips the rest of the current iteration and jumps to the loop condition. For `repeat`, `continue` starts the next iteration when one remains.
...
In nested loops, `break` and `continue` apply to the inner loop.

Inside `try/catch` these keywords are not supported.

`break` and `return` cannot appear in the same loop (they both are `RETALT` instruction).
