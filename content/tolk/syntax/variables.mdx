...
To make updates to `userId` visible in `demo`, declare the parameter with [`mutate`](/tolk/syntax/mutability#mutate-for-a-parameter): `mutate userId`.
...
To calculate CRC32 and similar values at compile time, use [`"...".crc32()`](/tolk/types/strings#compile-time-methods) and similar compile-time methods.

To group integer constants, use [enums](/tolk/types/enums).
...
In Tolk, avoid globals when possible. Use [auto-serialization](/tolk/features/auto-serialization) and [`lazy` loading](/tolk/features/lazy-loading).
