...
An exception can carry an argument of type [`unknown`](/tolk/types/unknown):
...
The `condition` must be a [`bool`](/tolk/types/booleans). Integers are not implicitly converted: use `assert (someInt != 0)` instead of `assert (someInt)`.
...
Additionally, an [out-of-gas exception](/tvm/exit-codes#14%3A-out-of-gas-error) may occur at **any** point, and it cannot be caught using a [`catch`](#try-catch-statement) block. It can only be prevented by writing [exhaustive testing suites](/contracts/blueprint/testing/overview).
