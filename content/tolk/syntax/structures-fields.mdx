...
Tolk supports [structures](/tolk/types/structures) — similar to TypeScript classes.
...
[Methods](/tolk/syntax/functions-methods) are declared separately as extension functions:
...
[Generics](/tolk/types/generics) exist only at the type level and incur no runtime cost.
...
The syntax `struct (PREFIX) Name { ... }` specifies a [serialization prefix for the structure](/tolk/types/overall-serialization#structures). For messages, 32-bit prefixes are typically called opcodes.
