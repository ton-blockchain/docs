...
Methods for [structures](/tolk/types/structures) are declared in the same way:
...
[Auto-serialization](/tolk/features/auto-serialization) with `fromSlice` follows the same mutability rules. Passing an argument without `mutate` never modifies the original variable.
