...
1. Review the list of [differences](/tolk/from-func/tolk-vs-func#key-differences).
1. See the [Tolk contract examples](/tolk/examples) page.
1. Use the [FunC-to-Tolk converter](/tolk/from-func/converter) to migrate existing projects.

## Gas benchmarks

The [`tolk-bench` repository](https://github.com/ton-blockchain/tolk-bench) compares FunC and Tolk across several TEP contracts. See the [Tolk contract examples](/tolk/examples) page for selected cases.
...
- Both languages are supported by [IDE plugins](/contracts/ide/vscode).
- Both languages are available in [Blueprint](/contracts/blueprint/overview) and other client-side tooling. However, best support for Tolk is provided in the [Acton](/contract-dev/acton) development environment.
...
- [Tolk resembles](/tolk/basic-syntax) TypeScript, Rust, and Kotlin.
...
- Tolk uses named [structures](/tolk/syntax/structures-fields) with the same runtime efficiency.
...
- Tolk derives [serialization](/tolk/features/auto-serialization) from `struct` using `toCell` and `fromCell`.
...
All [integer types](/tolk/types/numbers) such as `int8`, `uint64`, and `coins` are TVM integers.

### Lazy loading

- FunC requires manual control over preloads and skips for optimization.
- Tolk uses the [`lazy` keyword](/tolk/features/lazy-loading) to load only accessed fields.
...
- Tolk provides a [`bool` type](/tolk/types/booleans) and logical operators `&&`, `||`, and `!`. Conditions of `if`, `while`, ternary, and `assert` must be `bool` — write `if (someInt != 0)` instead of `if (someInt)`.
...
- Tolk provides a [`address` type](/tolk/types/address) with methods and the `==` operator.
...
- Tolk provides [nullable types](/tolk/types/nullable) `T?`, null safety, and smart casts.
...
- Tolk provides [a type system](/tolk/types/list-of-types), including unions, generics, and enums.
...
- Tolk provides both [functions and methods](/tolk/syntax/functions-methods), applicable to structures and primitives.
...
- Tolk provides native [maps](/tolk/types/maps), for example `m.set(1, 10)`.
...
- Tolk provides [`onInternalMessage(in)`](/tolk/features/message-handling) and use `in.senderAddress`, etc.
...
- Tolk routes messages using union types and [pattern matching](/tolk/syntax/pattern-matching).
...
- Tolk handles empty or unknown messages using `else` in a [`lazy` matching](/tolk/features/lazy-loading#lazy-matching).
...
- Tolk provides [`createMessage`](/tolk/features/message-sending), which automatically chooses between inline body and body reference.
...
- Tolk allows only [alphanumeric identifiers](/tolk/syntax/variables), for example  `2+2` is `4`.
...
In FunC, `inline` modifier operates at the Fift level and may introduce extra stack permutations. In Tolk, inlining is performed at the compiler level and is combined with [constant folding](/tolk/features/compiler-optimizations).
...
There are [differences between standard libraries](/tolk/from-func/stdlib-comparison). For example, functions from `stdlib.fc` started having descriptive names:
...
Although Tolk is a high-level language, it exposes [low-level capabilities](/tolk/features/asm-functions). Code may still be written in a FunC-style with manual builders and slices, and TVM instructions are supported.
...
- Tolk emits an [ABI JSON](/tolk/features/contract-abi#contract-abi-export) directly from sources whenever a file declares a `contract { ... }`. The same metadata drives auto-generated TypeScript wrappers, source maps, and a step-by-step debugger that works on fully-optimized bytecode.
