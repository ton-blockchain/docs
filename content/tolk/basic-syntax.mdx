...
[Imports](/tolk/syntax/imports) must appear at the top of the file:
...
A [struct](/tolk/syntax/structures-fields) `Point` holding two 8-bit integers:
...
- Fields can use any [types](/tolk/types/list-of-types): numeric, cell, union, and others.
- Fields can define default values: `x: int8 = 0`.
- Fields can be `private` and `readonly`.
- Structs can be generic: `struct Wrapper<T> { ... }`.

If all fields are serializable, a struct can be [automatically serialized](/tolk/features/auto-serialization):
...
A [function](/tolk/syntax/functions-methods) that returns the sum of two integers:
...
A function declared as `fun <receiver>.name(...)` is a [method](/tolk/syntax/functions-methods).
...
Within functions, [variables](/tolk/syntax/variables) are declared with `val` or `var` keywords. The `val` keyword declares an immutable variable that can be assigned only once:
...
To group integer constants, [enums](/tolk/types/enums) are useful.

## Value semantics

Tolk follows value semantics: assignments create independent copies, and function calls do not [mutate](/tolk/syntax/mutability) arguments unless explicitly specified.
...
In [conditions](/tolk/syntax/conditions-loops), `if` is a statement. `else if` and `else` blocks are optional.
...
[Union types](/tolk/types/unions) allow a variable to hold one of possible types. They are typically handled by `match`:
...
Union types are commonly used when [handling incoming messages](/tolk/features/message-handling).

## While loop

Tolk does not have a `for` loop; use a [`while` loop](/tolk/syntax/conditions-loops) for repeated execution. [`break` and `continue`](/tolk/syntax/conditions-loops#break-and-continue) are supported.
...
The `try-catch` statement is supported for [exceptions](/tolk/syntax/exceptions), although it is not commonly used in contracts.
...
[Arrays](/tolk/types/tuples) are dynamically sized containers created with `[...]`:
...
To iterate, [maps](/tolk/types/maps) can be used:
...
To [construct and send a message](/tolk/features/message-sending), a message body is typically represented by a structure. For example, `RequestedInfo`:
...
[Contract getters](/tolk/features/contract-getters) or get-methods are declared with `get fun`:
