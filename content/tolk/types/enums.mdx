...
Members of one enum must have distinct values.
...
[`match`](/tolk/syntax/pattern-matching) on enums requires coverage of all cases:
...
Every enum is [backed by TVM](/tolk/types/overall-tvm-stack) `INT` and [serialized](/tolk/types/overall-serialization) as `(u)intN`, where `N` is:
