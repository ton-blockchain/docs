...
When [serialized](/tolk/types/overall-serialization), values of `address` type occupy 267 bits:
...
Compare addresses using `==` or `!=`. Internally, an [address is represented as a raw `slice` without references](/tolk/types/overall-tvm-stack), so `==` and `!=` test for bits equality.
