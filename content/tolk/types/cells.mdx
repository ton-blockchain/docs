...
Tolk provides low-level functions for constructing and parsing cells manually, as well as [automatic serialization](/tolk/features/auto-serialization) of structures to and from cells.
...
The `slice` type cannot be serialized, but the `bitsN` types, such as `bits32` or `bits256`, can. At runtime, `bitsN` is a regular slice, just how `int32` are regular TVM integers.
...
fun demo() {
    ...
    rawBits.loadAddress();                 // error
    (rawBits as slice).loadAddress();      // ok
}
```
...
<Callout type="note">
  Tolk has a dedicated [`string`](/tolk/types/strings) type backed by snake-encoded cells. The examples below show how to embed constant slices with low-level binary data.
</Callout>
...
Structures are [auto-serializable](/tolk/features/auto-serialization) to and from cells when their fields are well-typed. Prefer using structures over manual work with builders and slices.
...
<Callout type="note">
  Notice that `T.fromSlice(s)` does not mutate a slice `s`, while `s.loadAny<T>()` does. This is due to how Tolk handles [mutability](/tolk/syntax/mutability): all `s.loadXYZ()` methods mutate a slice, but functions in the form `doSomething(s)` do not.
</Callout>
...
Structure fields of `cell` and `Cell<T>` types which are [serialized](/tolk/types/overall-serialization) as references to corresponding [TVM](/tolk/types/overall-tvm-stack) cells, with nullable types `?` serialized as 0 when the value is `null` and 1, followed by the cell reference otherwise.
