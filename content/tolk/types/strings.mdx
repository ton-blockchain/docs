...
Tolk provides a dedicated `string` type. At the TVM level, a string is a [cell](/tolk/types/cells) that stores data in snake format: a chain of cells where each cell holds a portion of the string and a reference to the next.
...
Per the [token metadata standard](/contracts/standard/tokens/metadata), strings returned from get-methods must contain prefixes for off-chain explorers and APIs:
