...
Methods declared on the alias apply only to that alias, not to its underlying type.
...
Once a variable is checked, the compiler narrows its type. For instance, if a variable [could be `null`](/tolk/types/nullable) before the check, and the condition is confirmed to be true, the variable's type will not allow `null` in the true block anymore.
...
Given a [union type](/tolk/types/unions) `T1 | T2 | ...`, the `is` operator performs a runtime type test, narrowing the type.
