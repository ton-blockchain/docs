...
## Setup code before a lazy load

A wrapper for `lazy` may contain some statements before a final `return`:

```tolk
fun Storage.load() {
    if (readRevision() != LAST_REVISION) {
        migrateStorage();
    }
    return Storage.fromCell(contract.getData());
}

get fun new_field(): uint32 {
    val storage = lazy Storage.load();
    return storage.new_field;
}
```

Here `if` always runs, `new_field` is loaded from the cell, fields before/after are skipped.
