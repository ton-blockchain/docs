...
Contract storage is a [`struct`](/tolk/types/structures), serialized into persistent blockchain data. Tolk does not enforce any specific storage layout.
...
- `fun Storage.f(self)` defines an instance [method](/tolk/syntax/functions-methods).
- `T.fromCell()` deserializes a cell into `T`, and [`obj.toCell()`](/tolk/features/auto-serialization#unpackoptions-and-packoptions) packs it back into a cell.
- The [`lazy`](/tolk/features/lazy-loading) operator does the parsing on demand.
- [`contract.getData()`](/tolk/features/standard-library) fetches persistent data.
...
This behavior is not related to [nullable types](/tolk/types/nullable). Nullable values such as `int8?` or `cell?` serialize with an explicit null marker 0 bit. In this case, fields are absent entirely, and no extra bits appear in serialization.
...
When this pattern is used, declare both shapes in the [`contract` declaration](/tolk/features/contract-abi) so client tools can compute the deployment address correctly:
...
Different shapes with missing fields can also be expressed using [generics](/tolk/types/generics) and the [void type](/tolk/types/void-never).
