...
A `struct` can define [a serialization](/tolk/types/overall-serialization) prefix. 32-bit prefixes are commonly used as opcodes for incoming and outgoing messages:
...
The Tolk compiler reports an error if a serializable struct may exceed 1023 bits. This can happen because many types have variable size. For example, `int8?` can be 1 or 9 bits, `coins` can range from 4 to 124 bits, etc. Consider a struct:
...
A common mistake is using `int`. [It cannot be serialized](/tolk/types/numbers#stack-layout-and-serialization); instead, use `int32`, `uint64`, etc.
...
Auto-serialization can be applied when [sending messages](/tolk/features/message-sending) to other contracts:
...
Tolk provides a special keyword [`lazy`](/tolk/features/lazy-loading) for use with auto-deserialization.
