...
For unions, [createMessage](/tolk/features/message-sending) is lightweight. The compiler generates all `IF-ELSE` and `STU`, but during compile-time analysis, these instructions resolve to constants because all types are known at compile time. The resulting code flattens into `PUSHINT` and `STSLICECONST`.
...
- `@inline` forces inlining. If inlining is impossible, compilation fails.
...
- it participates in a recursive call chain, such as `f -> g -> f`;
- it is used as a value rather than called, for example `val callback = f`;
- a `return` sits where the compiler cannot route it into the caller, e.g., inside a loop or inside `try`/`catch`.

`@inline` that cannot be applied is a compilation error. The message names the reason, for example "because a function recursively calls itself" or "because of `return` inside a loop".

```tolk
@inline
fun factorial(x: int): int {
    if (x <= 1) { return 1 }
    return x * factorial(x - 1);   // error: recursive
}
```

This function is inlined. The early `return` has one continuation, the rest of the body:
...
The [`lazy` keyword](/tolk/features/lazy-loading) loads only the required fields from a cell or slice:
...
To do it, change the evaluation order to minimize stack manipulations. The compiler keeps the order of statements written in source. It may reorder `@pure` assembler calls when it needs a particular stack order, and it folds constant expressions.
...
After the first block, the stack is `(v1 v2 v3)`. Since `v1` is used first, the stack must be rearranged with `SWAP`, `ROT`, `XCPU`, etc. Reordering assignments or usages — for example, moving `assert(v3)` up — pops the topmost element and can remove those shuffles. The compiler preserves the written order, because those calls may throw or modify state.
...
The Tolk compiler outputs the Fift assembler. Fift generates the bitcode. Projects built on [Blueprint](/contracts/blueprint/overview) use `tolk-js`, which invokes Tolk and then Fift.
...
- For [Acton](/contract-dev/acton) users, it is an intermediate result that can be accessed in the build directory.
