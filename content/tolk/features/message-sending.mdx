...
When handling a message, some values can be represented in multiple valid forms. [Union types](/tolk/types/unions) allow expressing these alternatives explicitly, so the same message-handling logic can accept and correctly process any of them.
...
This is implemented using [generics](/tolk/types/generics):
