...
Manual work with slices and builders is error-prone and tedious. By comparison, [auto-serialization](/tolk/features/auto-serialization) with structures helps express data with types and prevents many related bugs.
...
All data in TON is stored in cells. To express data relation clearly and to aid in [serialization](/tolk/features/auto-serialization#controlling-cell-references-typed-cells), use cells with well-typed contents: `Cell<T>`.
...
When reading data from cells, add the [`lazy` keyword](/tolk/features/lazy-loading):
...
Serialization may require custom rules which are not covered by existing types. Tolk allows defining custom [serialization rules for type aliases](/tolk/types/overall-serialization#type-aliases):
...
[Contract storage](/tolk/features/contract-storage) is a regular `struct`, serialized into persistent on-chain data.
...
By convention, every message in TON has an _opcode_: a unique 32-bit number. In Tolk, every [`struct`](/tolk/syntax/structures-fields) can have a serialization prefix of arbitrary length. Use 32-bit prefixes to express message opcodes.
...
When implementing [Jettons](/contracts/standard/tokens/jettons/overview), [NFTs](/contracts/standard/tokens/nft/overview), or other standard contracts, use predefined opcodes according to the specification. Otherwise, opcodes are ad hoc.

## Use unions to handle incoming messages

The suggested pattern:

1. Each [incoming message](/tolk/features/message-handling) is made a struct with an opcode.
1. Structs are combined into a union type.
1. Union is used to lazily load data from the message body slice.
1. Finally, result is [pattern matched](/tolk/syntax/pattern-matching) over union variants.
...
<Callout type="tip">
  See the [Tolk contract examples](/tolk/examples#jetton) page for selected contracts from the [`tolk-bench`](https://github.com/ton-blockchain/tolk-bench).
</Callout>

## Target certain shards when deploying sibling contracts

Specify the prefix length and the contract address to aim for the [same shard](/tolk/features/message-sending#shard-based-deployment). For example, sharded jetton wallet must be deployed to the same shard as the owner's wallet.
...
[External messages](/tolk/features/message-sending#universal-createexternallogmessage) with a special address `none` are used to emit events and logs to the outer world. Indexers catch such messages and provide a picture of on-chain activity.
...
When a [contract getter](/tolk/features/contract-getters) needs to return several values, introduce a structure. Avoid returning unnamed tensors like `(int, int, int)`. Field names provide clear metadata for client wrappers and human readers.
...
After parsing an incoming message, validate required fields with [`assert`](/tolk/syntax/exceptions#assert-statement):
...
- Have `MyContract.tolk` as an entrypoint, named after the contract in PascalCase. Place a [`contract` declaration](/tolk/features/contract-abi) at the top of the file and keep all `get fun` and entrypoint functions within it; use [imports](/tolk/syntax/imports) to bring in shared code.
...
Use [methods](/tolk/syntax/functions-methods) to avoid name collisions:
...
A nullable [address](/tolk/types/address) `address?` is a pattern for an optional address, sometimes called _"maybe address"_:
...
Several [compile-time methods](/tolk/types/strings#compile-time-methods) operate on constant [strings](/tolk/types/strings):
...
Tolk provides a dedicated [`string` type](/tolk/types/strings) backed by snake-encoded cells. Use the [`StringBuilder`](/tolk/types/strings#stringbuilder) for concatenation:
...
For fixed-size binary data, use [`bitsN` types](/tolk/types/cells#fixed-size-slices), for example `bits256` or `bits512`.

## Avoid micro-optimization

The [compiler applies many optimizations](/tolk/features/compiler-optimizations): it automatically inlines functions, reduces stack allocations, and handles the underlying work. Attempts to outsmart the compiler yield negligible effects, either positive or negative.
