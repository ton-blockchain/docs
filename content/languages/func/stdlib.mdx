...
- `int`: The remaining balance in nanograms.
- `cell`: A dictionary (with 32-bit keys) containing [extra-currency balances](/foundations/extra-currencies#representation).
...
The smart contract agrees to purchase gas to complete the transaction.
This function is required when processing external messages that do not carry any value and do not provide gas.

See the [`ACCEPT` instruction](/tvm/accept) page for more details.
...
If the `limit` is `2^63 − 1` or higher, calling `set_gas_limit` works the same as calling [`accept_message`](#accept_message).
...
This function calculates how much gas can be purchased for the `gram` nanograms and updates the gas limit `gl` accordingly,
...
that reserves a specific amount of nanograms from the account's remaining balance.
The behavior depends on the `mode` parameter:

- If `mode = 0`, exactly the `amount` of nanograms is reserved.
- If `mode = 2`, up to the `amount` of nanograms is reserved.
- If `mode = 1` or `mode = 3`, all but the `amount` of nanograms is reserved.

This process is equivalent to generating an outbound message that transfers the `amount` of nanograms or `b − amount` nanograms,
...
This dictionary, given as a `cell` or `null`, allows reserving currencies other than Gram.
...
Loads serialized amount of Gram, which is an unsigned integer up to `2^120 - 1`.
...
This is the standard way of storing Gram.
