...
FunC, or more specifically, the Fift assembler, reserves several function names with predefined [IDs](/languages/func/functions#method_id-specifier):
...
- `balance` is the smart contract balance in nanograms after adding the amount `msg_value` in the inbound message. It is an integer.
- `msg_value` is the amount in nanograms included in the inbound message. It is an integer.
...
- `balance` is the smart contract balance in nanograms. It is an integer.
