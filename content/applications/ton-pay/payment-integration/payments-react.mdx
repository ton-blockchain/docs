...
export function PaymentButton() {
  ...
  return <button onClick={handlePayment}>Pay 3.5 GRAM</button>;
}
```
...
export function ServerPaymentButton({
  ...
  return <button onClick={handlePayment}>Pay {amount} GRAM</button>;
}
```
...
<Callout
  ...
  title="Funds at risk"
>
  Running tests on mainnet can result in irreversible loss of real GRAM. Always use `chain: "testnet"` and testnet wallet addresses during development. Verify the network before switching to mainnet.
</Callout>

A complete description of testnet configuration, including obtaining testnet GRAM, testing jetton transfers, verifying transactions, and preparing for mainnet deployment, is available in the [Testnet configuration](/applications/ton-pay/payment-integration/transfer#testnet-configuration) section.
