...
<Steps>
  ...
  <Step>
    ...
    Create [`tonconnect-manifest.json`](/applications/ton-connect/core-concepts#manifest) in the app public directory:
...
[Use `useTonPay`](/applications/ton-pay/ui-integration/button-react#option-1:-using-usetonpay-hook-recommended) for a complete example.

<CodeGroup>
  ```tsx title="Long variant (default)"
  <TonPayButton
    variant="long"
    handlePay={handlePay}
  />
...
- Save `reference` and `bodyBase64Hash` to track payment status using [webhooks](/applications/ton-pay/webhooks).
