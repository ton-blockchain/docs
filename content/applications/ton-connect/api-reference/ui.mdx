...
> Generated from `@tonconnect/ui` v3.0.0.
...
Returns the normal response, or `EmbeddedSendTransactionResponse` when
`enableEmbeddedRequest` is `true`.
...
signData(
    ...
    | OptionalTraceable<EmbeddedSignDataResponse>
>;
```
...
Returns the normal response, or `EmbeddedSignDataResponse` when
`enableEmbeddedRequest` is `true`.
...
signMessage(
    ...
    | OptionalTraceable<EmbeddedSignMessageResponse>
>;
```
...
Returns the normal response, or `EmbeddedSignMessageResponse` when
`enableEmbeddedRequest` is `true`.
