...
For frequently asked questions about expected behavior rather than failures, see the [FAQ](/applications/ton-connect/faq).
...
**Action.** The SDK already retries: it reconnects the SSE channel with a fixed 2-second delay and repeats every `POST /message` every 5 seconds. Public action methods accept an `AbortSignal`; abort it when your give-up policy is reached. Surface a _Connection problem_ state in the UI and offer to reconnect. Each wallet entry in [`wallets-v2.json`](https://github.com/ton-connect/wallets-list) lists at most one SSE bridge URL — there is no fallback inside the entry, so a permanently broken bridge has to be fixed by the wallet provider.
...
Connect, action, and disconnect operations use a `traceId` (UUID, UUIDv7 by default) for correlation. Connect places it in the URL; actions and disconnect place it on the corresponding bridge request. Results from `sendTransaction`, `signMessage`, and `signData` expose `result.traceId`; `disconnect` returns `void`. Log the ID alongside your analytics and include it when reporting issues.
...
Use `curl` to identify the failing layer. For the manifest schema and fields, see [Manifest](/applications/ton-connect/core-concepts#manifest) and the [App manifest specification](https://github.com/ton-blockchain/ton-connect/blob/main/spec/manifest.md).
...
- [FAQ](/applications/ton-connect/faq) — frequently asked questions about behavior and design choices.
