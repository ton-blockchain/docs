---
title: "How to perform message lookup after sending a transaction with TON Connect"
sidebarTitle: "Message lookup"
---

<Callout type="caution">
  You should never use external message tracking for payment processing purposes. Check out [payment processing](/applications/payments/overview) for more details.
</Callout>

The process of seeking a transaction associated with an `external-in` message is called _message lookup_. It should only be used for better UX to display, for example, the progress of the operation and its result.
...
```ts
// Not runnable: add the preceding retry and normalization helpers.
...
```ts
// Not runnable: replace the sample BoC with a returned transaction BoC.
...
```typescript
// Not runnable: place this code inside a React component.
import { TonClient } from '@ton/ton';
...
- [How to send a transaction with TON Connect](/applications/ton-connect/how-to/send-transaction)
