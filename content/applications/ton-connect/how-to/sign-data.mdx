...
Use `cell` when a smart contract will verify the signature. The wallet hashes a TON cell, not a flat byte string; you also send the [TL-B](/blockchain-basics/languages/tl-b/overview) schema so the wallet can decode the cell on-screen.
...
1. Reject if `Timestamp` is older than your tolerance window — 15 minutes is a common bound.
