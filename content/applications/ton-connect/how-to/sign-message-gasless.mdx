---
title: "How to sign and relay a gasless message with TON Connect"
sidebarTitle: "Sign message (gasless)"
---

`signMessage` asks the wallet to sign an internal message **without broadcasting it**. The dApp receives a signed BoC and submits it through a relayer — typically to pay gas in a jetton instead of Gram. This flow is currently supported only for Wallet V5.
    ...
    messages: [
        {
            ...
            amount: '5000000',   // in nanogram (0.005 Gram)
...
The most common use is a gasless jetton transfer: the relayer takes its fee in the same jetton, so the user never holds Gram.

The flow:

1. Resolve the user's jetton wallet address on-chain.
1. Build the jetton transfer cell to pass to the estimator.
1. Call `gaslessEstimate` — the relayer returns the exact messages to sign (main transfer plus the relay fee).
1. Call `signMessage` with those messages — the wallet signs but does not broadcast.
1. Wrap the signed BoC in an external message and submit via `gaslessSend`.
...
REST endpoints are documented in the [gasless reference](https://docs.tonconsole.com/tonapi/rest-api/gasless). A parallel [cookbook recipe](https://docs.tonconsole.com/tonapi/cookbook/gasless-transfer) shows the same flow.
...
const BASE_JETTON_SEND_AMOUNT = toNano('0.05'); // attached Gram for the jetton transfer message; pass as string to avoid float rounding
...
```ts
import { TonConnectError, TonConnectUI, TonConnectUIError } from '@tonconnect/ui';
