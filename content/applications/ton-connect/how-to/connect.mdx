---
title: "How to connect a wallet with TON Connect"
...
The same option is accepted by `tonConnectUi.disconnect`, `sendTransaction`, `signMessage`, and `signData`.

<Callout type="note">
  The signing-action results expose `traceId`, while `disconnect` returns `void`.
</Callout>
...
verify  = domainSignVerify({ data: hash, signature, publicKey, domain })
```

The `domainSignVerify()` function performs Ed25519 verification with the signature domain for the selected network.

<Callout type="note">
  Field encodings are defined in the [`ton_proof` signature specification](https://github.com/ton-blockchain/ton-connect/blob/main/spec/connect.md#address-proof-signature-ton_proof).
</Callout>
...
interface VerifyTonProofInput {
    ...
    network: string;           // '-239', '-3', or another accepted global ID
    ...
    expectedPayload: string;   // unused nonce issued for this session
...
export async function verifyTonProof(
    ...
    if (proof.payload !== expectedPayload) throw new Error('wrong payload');
    if (network !== expectedNetwork) throw new Error('wrong network');
...
- [Get started](/applications/ton-connect/get-started) — React, Next.js, vanilla JS, and headless SDK quick-starts.
- [Sign data § Extracting the public key](/applications/ton-connect/how-to/sign-data#extracting-the-public-key) — the wallet public-key extraction the `ton_proof` verifier depends on.
...
- [Disconnect a session](/applications/ton-connect/how-to/disconnect)
- [Filter wallets by required features](/applications/ton-connect/how-to/filter-wallets)
- [TON Connect manifest](/applications/ton-connect/core-concepts#manifest)
