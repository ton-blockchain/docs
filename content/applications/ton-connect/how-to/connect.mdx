...
The same option is accepted by `tonConnectUi.disconnect`, `sendTransaction`, `signMessage`, and `signData`. The signing-action results expose `traceId`; `disconnect` returns `void`.
...
`domainSignVerify()` performs Ed25519 verification with the signature domain for the selected network.
