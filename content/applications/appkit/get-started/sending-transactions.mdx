---
title: "Send transactions with AppKit"
...
## Send Gram in one component

`<SendTonButton />` owns the whole lifecycle for a Gram transfer. It builds the request from `recipientAddress`, `amount` (decimal GRAM), and an optional `comment`; calls the wallet; and surfaces pending, error, and success state through a render-prop child. When you need to send something other than a plain Gram transfer — an NFT, a multi-message bundle, a swap-built transaction — use `<Send />` with a `request` callback that returns a `TransactionRequest` you assembled yourself.
...
| ----------------------------------- | ---------------------- | -------------------- | ----------------- |
| Send Gram                           | `<SendTonButton />`    | `useTransferTon`     | `transferTon`     |
...
See a [working example](https://github.com/ton-connect/kit/tree/main/apps/appkit-minter) of Gram, jetton, and NFT transfers built with these mutations — [try it live](https://appkit-minter.vercel.app/).
...
For task-level walkthroughs, see:

- [How to send Gram](/applications/appkit/howto/send-gram)
- [How to send jettons](/applications/appkit/howto/send-jettons)
- [How to manage NFTs](/applications/appkit/howto/nfts)
