---
title: "How to set up AppKit"
sidebarTitle: "Set up AppKit"
---
...
- **Asset support**: work with Gram, jettons (USDT and others), and NFTs.
...
Create the `AppKit` instance once with an `AppKitConfig`. All fields are optional; pass only the configuration your app needs. See [Manual setup → Add wallet connectors](/applications/appkit/get-started/connectors) for the constructor walkthrough.
...
`@ton/appkit-react` provides hooks for the main AppKit actions and watchers. Mount `AppKitProvider` once at the root inside a TanStack `QueryClientProvider`, then call hooks anywhere below it. See [Install AppKit → Wrap the application](/applications/appkit/get-started/installation/react-app#wrap-the-application) for the full mount.
