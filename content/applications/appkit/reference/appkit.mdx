---
title: "`@ton/appkit` reference"
sidebarTitle: "`@ton/appkit`"
---
...
Read the Gram balance of the currently selected wallet, returning `null` when no wallet is selected (use [`getBalanceByAddress`](#getbalancebyaddress) for an arbitrary address).
...
Returns: [`GetBalanceReturnType`](#getbalancereturntype) — Balance in Gram as a human-readable decimal string, or `null` when no wallet is selected.
...
Read the Gram balance of an arbitrary address — useful for wallets that aren't selected in AppKit (use [`getBalance`](#getbalance) for the selected wallet).
...
| `options.network`   | [`Network`](#network)                                                          | Network to read the balance from. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |

Returns: [`GetBalanceByAddressReturnType`](#getbalancebyaddressreturntype) — Balance in Gram as a human-readable decimal string.
...
Subscribe to Gram balance updates for an arbitrary address — useful for monitoring wallets that aren't selected in AppKit (use [`watchBalance`](#watchbalance) for the selected wallet). Requires a streaming provider registered for the network — call throws when none is configured. Use [`hasStreamingProvider`](#hasstreamingprovider) to check first.
...
| `options.address`\*  | <code><a href="#userfriendlyaddress">UserFriendlyAddress</a> \| Address</code> | Wallet address — pass a [`UserFriendlyAddress`](#userfriendlyaddress) string or an `Address` instance from `@ton/core`.                                        |
| `options.network`    | [`Network`](#network)                                                          | Network to watch on. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
| `options.onChange`\* | <code>(update: <a href="#balanceupdate">BalanceUpdate</a>) => void</code>      | Callback fired on every balance update from the streaming provider.                                                                                            |
...
| `options.network`\* | [`Network`](#network)                         | Network whose configured [`ApiClient`](#apiclient) should be returned. |

Returns: [`GetApiClientReturnType`](#getapiclientreturntype) — The configured [`ApiClient`](#apiclient) for the requested network.
...
Returns: [`ConnectReturnType`](#connectreturntype) — Resolves once the connector's connect flow completes. If a wallet was successfully connected, it becomes available via [`getSelectedWallet`](#getselectedwallet).
...
| `config.metadata`          | [`ConnectorMetadata`](#connectormetadata)                 | Display metadata override. Merged on top of TON Connect's default name and icon.                                                                                                                            |
...
Returns: [`ConnectorFactory`](#connectorfactory) — Factory function consumed by AppKit at registration time.
...
Returns: [`DisconnectReturnType`](#disconnectreturntype) — Resolves once the connector tears down its session.
...
Create a crypto on-ramp deposit from a quote previously obtained via [`getCryptoOnrampQuote`](#getcryptoonrampquote) — the returned [`CryptoOnrampDeposit`](#cryptoonrampdeposit) carries the address and amount the user must send on the source chain.
...
Returns: [`CreateCryptoOnrampDepositReturnType`](#createcryptoonrampdepositreturntype) — Deposit details the UI should show to the user (address, amount, optional `memo`/`expiresAt`).
...
| Parameter | Type                                                  | Description                                                                                                          |
| --------- | ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `config`  | [`LayerswapProviderConfig`](#layerswapproviderconfig) | Optional [`LayerswapProviderConfig`](#layerswapproviderconfig). Defaults are filled in for any field left undefined. |
...
| Parameter  | Type                                              | Description                                                                      |
| ---------- | ------------------------------------------------- | -------------------------------------------------------------------------------- |
| `config`\* | [`DecentProviderConfig`](#swapsxyzproviderconfig) | Configuration carrying the required `apiKey` plus optional URL/sender overrides. |

Returns: <code>ProviderFactory\<<a href="#swapsxyzcryptoonrampprovider">DecentCryptoOnrampProvider</a>></code>.

#### `getCryptoOnrampProvider`

Get a registered crypto on-ramp provider by ID, or the default provider when no ID is given. Throws when no provider matches — or when no ID is passed and no default has been registered.
...
Returns: [`GetCryptoOnrampProviderReturnType`](#getcryptoonrampproviderreturntype) — The matching [`CryptoOnrampProviderInterface`](#cryptoonrampproviderinterface).
...
List every crypto on-ramp provider registered on this AppKit instance — both those passed via [`AppKitConfig`](#appkitconfig)'s `providers` and those added later through [`registerProvider`](#registerprovider).

| Parameter  | Type                | Description       |
| ---------- | ------------------- | ----------------- |
| `appKit`\* | [`AppKit`](#appkit) | Runtime instance. |

Returns: [`GetCryptoOnrampProvidersReturnType`](#getcryptoonrampprovidersreturntype) — Array of registered [`CryptoOnrampProviderInterface`](#cryptoonrampproviderinterface)s.
...
Quote a crypto-to-TON on-ramp — given a source asset/chain and target TON asset, returns the rate, expected amount, and provider metadata needed to call [`createCryptoOnrampDeposit`](#createcryptoonrampdeposit).
...
Returns: [`GetCryptoOnrampQuoteReturnType`](#getcryptoonrampquotereturntype) — Quote with pricing details and the provider metadata required to create a deposit.

#### `getCryptoOnrampStatus`

Read the current status of a crypto on-ramp deposit by ID — typically polled until the result is `'success'` or `'failed'`.
...
Returns: [`GetCryptoOnrampStatusReturnType`](#getcryptoonrampstatusreturntype) — Current [`CryptoOnrampStatus`](#cryptoonrampstatus) of the deposit.

#### `watchCryptoOnrampProviders`

Subscribe to crypto on-ramp provider lifecycle — fires `onChange` whenever a new provider is registered or the default crypto on-ramp provider switches.

| Parameter               | Type                                                                            | Description                                                                                                     |
| ----------------------- | ------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| `appKit`\*              | [`AppKit`](#appkit)                                                             | Runtime instance.                                                                                               |
| `parameters`\*          | [`WatchCryptoOnrampProvidersParameters`](#watchcryptoonrampprovidersparameters) | Update callback.                                                                                                |
| `parameters.onChange`\* | `() => void`                                                                    | Callback fired whenever a crypto on-ramp provider is registered or the default crypto on-ramp provider changes. |

Returns: [`WatchCryptoOnrampProvidersReturnType`](#watchcryptoonrampprovidersreturntype) — Unsubscribe function — call it to stop receiving updates.

### DeFi

#### `registerProvider`

Register a DeFi / on-ramp provider at runtime — equivalent to passing it via [`AppKitConfig`](#appkitconfig)'s `providers` at construction, but available after AppKit is up. AppKit emits `provider:registered`, picked up by domain-specific subscribers like [`watchSwapProviders`](#watchswapproviders) and [`watchCryptoOnrampProviders`](#watchcryptoonrampproviders).
...
| `parameters.jettonAddress`\*    | [`UserFriendlyAddress`](#userfriendlyaddress)                                             | Jetton master contract address being transferred.                                         |
| `parameters.recipientAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress)                                             | Recipient who should receive the jettons.                                                 |
...
Returns: [`CreateTransferJettonTransactionReturnType`](#createtransferjettontransactionreturntype) — Transaction request ready to pass to `sendTransaction`.
...
| `options.jettonAddress`\*  | [`UserFriendlyAddress`](#userfriendlyaddress)         | Jetton master contract address (the token, not the user's wallet for it).                                                                                                   |
| `options.ownerAddress`\*   | [`UserFriendlyAddress`](#userfriendlyaddress)         | Owner of the jetton wallet — typically the connected user's address.                                                                                                        |
...
Returns: [`GetJettonBalanceReturnType`](#getjettonbalancereturntype) — Balance as a human-readable decimal string in the jetton's units.
...
| `options.address`\* | [`UserFriendlyAddress`](#userfriendlyaddress)   | Jetton master contract address whose metadata is being fetched.                                                                                             |
| `options.network`   | [`Network`](#network)                           | Network to query. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |

Returns: [`GetJettonInfoReturnType`](#getjettoninforeturntype) — Jetton metadata, or `null` if the indexer has no record.
...
| `options.jettonAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress)                     | Jetton master contract address.                                                                                                                             |
| `options.ownerAddress`\*  | [`UserFriendlyAddress`](#userfriendlyaddress)                     | Owner whose jetton wallet should be derived.                                                                                                                |
| `options.network`         | [`Network`](#network)                                             | Network to query. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |

Returns: [`GetJettonWalletAddressReturnType`](#getjettonwalletaddressreturntype) — User-friendly address of the owner's jetton wallet.
...
| `options.network` | [`Network`](#network)                     | Network to read jettons from. Defaults to the selected wallet's network.  |
...
Returns: [`GetJettonsReturnType`](#getjettonsreturntype) — Jettons response for the selected wallet, or `null` when none is selected.
...
| `options.network`   | [`Network`](#network)                                                          | Network to read the jettons from. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
| `options.limit`     | `number`                                                                       | Maximum number of jettons to return.                                                                                                                                        |
| `options.offset`    | `number`                                                                       | Number of jettons to skip before returning results — used for pagination.                                                                                                   |

Returns: [`GetJettonsByAddressReturnType`](#getjettonsbyaddressreturntype) — Jettons response with formatted balances.
...
Returns: [`TransferJettonReturnType`](#transferjettonreturntype) — Wallet response carrying the BoC of the sent transaction.
...
Returns: [`WatchJettonsReturnType`](#watchjettonsreturntype) — Unsubscribe function — call it to stop receiving updates.
...
Returns: [`WatchJettonsByAddressReturnType`](#watchjettonsbyaddressreturntype) — Unsubscribe function — call it to stop receiving updates.
...
| `parameters.nftAddress`\*       | [`UserFriendlyAddress`](#userfriendlyaddress)                                       | NFT contract address to transfer.                                                                         |
| `parameters.recipientAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress)                                       | New owner address.                                                                                        |
| `parameters.amount`             | `string`                                                                            | Amount of GRAM to attach to the transfer for gas. Defaults to AppKit's NFT gas-fee constant when omitted. |
| `parameters.comment`            | `string`                                                                            | Optional human-readable comment attached to the transfer.                                                 |

Returns: [`CreateTransferNftTransactionReturnType`](#createtransfernfttransactionreturntype) — Transaction request ready to pass to `sendTransaction`.
...
Returns: [`GetNftReturnType`](#getnftreturntype) — NFT data, or `null` if the indexer has no record.
...
| `options.network` | [`Network`](#network)               | Network to read NFTs from. Defaults to the selected wallet's network.  |
...
Returns: [`GetNftsReturnType`](#getnftsreturntype) — NFTs response for the selected wallet, or `null` when none is selected.
...
| `options.address`\* | <code><a href="#userfriendlyaddress">UserFriendlyAddress</a> \| Address</code> | Owner address — pass a [`UserFriendlyAddress`](#userfriendlyaddress) string or an `Address` instance from `@ton/core`.                                               |
| `options.network`   | [`Network`](#network)                                                          | Network to read NFTs from. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
| `options.limit`     | `number`                                                                       | Maximum number of NFTs to return.                                                                                                                                    |
| `options.offset`    | `number`                                                                       | Number of NFTs to skip before returning results — used for pagination.                                                                                               |

Returns: [`GetNftsByAddressReturnType`](#getnftsbyaddressreturntype) — NFTs response with the owner's items.
...
Returns: [`TransferNftReturnType`](#transfernftreturntype) — Wallet response carrying the BoC of the sent transaction.
...
| `options.network` | [`Network`](#network)                             | Network to query. Defaults to mainnet when omitted. |

Returns: [`GetBlockNumberReturnType`](#getblocknumberreturntype) — Current masterchain seqno as a number.
...
Returns: [`GetDefaultNetworkReturnType`](#getdefaultnetworkreturntype) — The default [`Network`](#network), or `undefined` if none is set.
...
Returns: [`GetNetworkReturnType`](#getnetworkreturntype) — Network of the selected wallet, or `null` when none is selected.
...
Returns: [`GetNetworksReturnType`](#getnetworksreturntype) — Array of configured [`Network`](#network)s.
...
Returns: [`HasStreamingProviderReturnType`](#hasstreamingproviderreturntype) — `true` when a streaming provider is registered for that network.
...
Returns: [`SetDefaultNetworkReturnType`](#setdefaultnetworkreturntype).
...
Returns: [`WatchDefaultNetworkReturnType`](#watchdefaultnetworkreturntype) — Unsubscribe function — call it to stop receiving updates.
...
Returns: [`WatchNetworksReturnType`](#watchnetworksreturntype) — Unsubscribe function — call it to stop receiving updates.
...
| `parameters.bytes`\* | [`Base64String`](#base64string)                 | Binary blob the user is asked to sign, encoded as Base64.                                                                                                   |
| `parameters.network` | [`Network`](#network)                           | Network to issue the sign request against. Defaults to AppKit's configured default network. When none is set, the wallet falls back to its current network. |

Returns: [`SignBinaryReturnType`](#signbinaryreturntype) — Signature and signed payload, as returned by the wallet.
...
| `parameters.cell`\*   | [`Base64String`](#base64string)             | TON cell content encoded as Base64 (BoC).                                                                                                                   |
| `parameters.schema`\* | `string`                                    | TL-B-style schema describing the cell layout so the wallet can render the payload to the user.                                                              |
| `parameters.network`  | [`Network`](#network)                       | Network to issue the sign request against. Defaults to AppKit's configured default network. When none is set, the wallet falls back to its current network. |

Returns: [`SignCellReturnType`](#signcellreturntype) — Signature and signed payload, as returned by the wallet.
...
Returns: [`SignTextReturnType`](#signtextreturntype) — Signature and signed payload, as returned by the wallet.
...
| `options.quote`\*         | [`StakingQuote`](#stakingquote)                                 | The staking quote based on which the transaction is built.                              |
| `options.userAddress`\*   | [`UserFriendlyAddress`](#userfriendlyaddress)                   | Address of the user performing the staking.                                             |
| `options.providerOptions` | `TProviderOptions = unknown`                                    | Provider-specific options.                                                              |
| `options.providerId`      | `string`                                                        | Provider to stake/unstake through. Defaults to the registered default staking provider. |

Returns: [`BuildStakeTransactionReturnType`](#buildstaketransactionreturntype) — Transaction request ready to pass to `sendTransaction`.
...
| Parameter | Type                                                    | Description                                                                                                            |
| --------- | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `config`  | [`TonStakersProviderConfig`](#tonstakersproviderconfig) | Optional [`TonStakersProviderConfig`](#tonstakersproviderconfig). Defaults are filled in for any field left undefined. |
...
| `options.userAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress)         | Owner whose staked balance should be read.                                                                                                                  |
| `options.network`       | [`Network`](#network)                                 | Network to query. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
| `options.providerId`    | `string`                                              | Provider to query. Defaults to the registered default staking provider.                                                                                     |

Returns: [`GetStakedBalanceReturnType`](#getstakedbalancereturntype) — Staked-balance breakdown ([`StakingBalance`](#stakingbalance)) for the user on the resolved network/provider.
...
Returns: [`GetStakingManagerReturnType`](#getstakingmanagerreturntype) — The [`StakingManager`](#stakingmanager) bound to this AppKit instance.
...
Returns: [`GetStakingProviderReturnType`](#getstakingproviderreturntype) — The matching staking provider instance.
...
| `options.network`    | [`Network`](#network)                                             | Network whose staking pool should be inspected. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
| `options.providerId` | `string`                                                          | Provider to query. Defaults to the registered default staking provider.                                                                                                                   |

Returns: [`GetStakingProviderInfoReturnType`](#getstakingproviderinforeturntype) — Live staking-provider info for the resolved network.
...
| `options.network`    | [`Network`](#network)                                                     | Network whose provider metadata should be read. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
| `options.providerId` | `string`                                                                  | Provider to query. Defaults to the registered default staking provider.                                                                                                                   |

Returns: [`GetStakingProviderMetadataReturnType`](#getstakingprovidermetadatareturntype) — Static [`StakingProviderMetadata`](#stakingprovidermetadata) for the resolved provider.
...
Returns: [`GetStakingProvidersReturnType`](#getstakingprovidersreturntype) — Array of registered staking providers.
...
| `options.direction`\*     | [`StakingQuoteDirection`](#stakingquotedirection)   | Direction of the quote (stake or unstake)                                                                                                                                                                                                                               |
| `options.amount`\*        | `string`                                            | Amount of tokens to stake or unstake.                                                                                                                                                                                                                                   |
| `options.userAddress`     | [`UserFriendlyAddress`](#userfriendlyaddress)       | Address of the user.                                                                                                                                                                                                                                                    |
| `options.network`         | [`Network`](#network)                               | Network on which the staking will be executed.                                                                                                                                                                                                                          |
| `options.unstakeMode`     | [`UnstakeModes`](#unstakemodes)                     | Unstake-timing mode the quote should target — see [`UnstakeMode`](#unstakemode) for the supported flavours (`'INSTANT'`, `'WHEN_AVAILABLE'`, `'ROUND_END'`). Only meaningful when `direction === 'unstake'` and the provider lists the mode in `supportedUnstakeModes`. |
| `options.isReversed`      | `boolean`                                           | If true, for unstake requests the amount is specified in the staking coin (e.g. GRAM) instead of the Liquid Staking Token (e.g. tsTON).                                                                                                                                 |
| `options.providerOptions` | `TProviderOptions = unknown`                        | Provider-specific options.                                                                                                                                                                                                                                              |
| `options.providerId`      | `string`                                            | Provider to quote against. Defaults to the registered default staking provider.                                                                                                                                                                                         |

Returns: [`GetStakingQuoteReturnType`](#getstakingquotereturntype) — Quote with pricing details and provider metadata.
...
Returns: [`SetDefaultStakingProviderReturnType`](#setdefaultstakingproviderreturntype).
...
Returns: [`WatchStakingProvidersReturnType`](#watchstakingprovidersreturntype) — Unsubscribe function — call it to stop receiving updates.
...
| `options.quote`\*            | [`SwapQuote`](#swapquote)                                     | The swap quote based on which the transaction is built.                            |
| `options.userAddress`\*      | [`UserFriendlyAddress`](#userfriendlyaddress)                 | Address of the user performing the swap.                                           |
| `options.destinationAddress` | [`UserFriendlyAddress`](#userfriendlyaddress)                 | Address to receive the swapped tokens (defaults to userAddress)                    |
...
Returns: [`BuildSwapTransactionReturnType`](#buildswaptransactionreturntype) — Transaction request ready to pass to `sendTransaction`.
...
| Parameter | Type                                                    | Description                                                                                                            |
| --------- | ------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| `config`  | [`DeDustSwapProviderConfig`](#dedustswapproviderconfig) | Optional [`DeDustSwapProviderConfig`](#dedustswapproviderconfig). Defaults are filled in for any field left undefined. |
...
| Parameter | Type                                                        | Description                                                                                                                |
| --------- | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `config`  | [`OmnistonSwapProviderConfig`](#omnistonswapproviderconfig) | Optional [`OmnistonSwapProviderConfig`](#omnistonswapproviderconfig). Defaults are filled in for any field left undefined. |
...
Returns: [`GetSwapManagerReturnType`](#getswapmanagerreturntype) — The [`SwapManager`](#swapmanager) bound to this AppKit instance.
...
Returns: [`GetSwapProviderReturnType`](#getswapproviderreturntype) — The matching swap provider instance.
...
Returns: [`GetSwapProvidersReturnType`](#getswapprovidersreturntype) — Array of registered swap providers.
...
| `options.from`\*              | [`SwapToken`](#swaptoken)                     | Token to swap from.                                                                             |
| `options.to`\*                | [`SwapToken`](#swaptoken)                     | Token to swap to.                                                                               |
| `options.network`\*           | [`Network`](#network)                         | Network on which the swap will be executed.                                                     |
| `options.slippageBps`         | `number`                                      | Slippage tolerance in basis points (1 `bp` = 0.01%)                                             |
...
Returns: [`GetSwapQuoteReturnType`](#getswapquotereturntype) — Quote with pricing details and the provider metadata required to build a transaction.
...
const quote = await getSwapQuote(appKit, {
    ...
    amount: '1000000000', // nanograms as string
    network: Network.mainnet(),
});
...
Returns: [`SetDefaultSwapProviderReturnType`](#setdefaultswapproviderreturntype).
...
Returns: [`WatchSwapProvidersReturnType`](#watchswapprovidersreturntype) — Unsubscribe function — call it to stop receiving updates.
...
Build a GRAM transfer [`TransactionRequest`](#transactionrequest) for the selected wallet without sending it — useful when the UI needs to inspect or batch transactions before signing. Throws `Error('Wallet not connected')` if no wallet is currently selected.
...
| `parameters.recipientAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress)                                       | Recipient address.                                                                                    |
| `parameters.amount`\*           | `string`                                                                            | Amount in Gram as a human-readable decimal string (e.g., `"1.5"`). Converted to nanograms internally. |
| `parameters.comment`            | `string`                                                                            | Human-readable text comment. Converted to a Base64 payload when no `payload` is supplied.             |
| `parameters.payload`            | [`Base64String`](#base64string)                                                     | Raw Base64 message payload — wins over `comment` when both are set.                                   |
| `parameters.stateInit`          | [`Base64String`](#base64string)                                                     | Initial state for deploying a new contract, encoded as Base64.                                        |

Returns: [`CreateTransferTonTransactionReturnType`](#createtransfertontransactionreturntype) — Transaction request ready to pass to `sendTransaction`.
...
const tx = createTransferTonTransaction(appKit, {
    recipientAddress: 'EQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAM9c',
    amount: '0.1', // 0.1 Gram (human-readable format)
...
Returns: [`GetTransactionStatusReturnType`](#gettransactionstatusreturntype) — Status response with current state, completed/total message counts and trace details.
...
Returns: [`SendTransactionReturnType`](#sendtransactionreturntype) — Wallet response carrying the BoC of the sent transaction.
...
const result = await sendTransaction(appKit, {
    messages: [
        {
            address: 'EQAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAM9c',
            amount: '100000000', // 0.1 Gram in nanograms (raw format)
        },
    ],
});
...
Build and send a GRAM transfer from the selected wallet in one step (use [`createTransferTonTransaction`](#createtransfertontransaction) + [`sendTransaction`](#sendtransaction) if you need to inspect the transaction first). Throws `Error('Wallet not connected')` if no wallet is currently selected.
...
Returns: [`TransferTonReturnType`](#transfertonreturntype) — Wallet response carrying the BoC of the sent transaction.
...
Returns: [`WatchTransactionsReturnType`](#watchtransactionsreturntype) — Unsubscribe function — call it to stop receiving updates.
...
Returns: [`WatchTransactionsByAddressReturnType`](#watchtransactionsbyaddressreturntype) — Unsubscribe function — call it to stop receiving updates.
...
Returns: [`GetConnectedWalletsReturnType`](#getconnectedwalletsreturntype) — Read-only array of [`WalletInterface`](#walletinterface)s.
...
Returns: [`GetSelectedWalletReturnType`](#getselectedwalletreturntype) — Selected [`WalletInterface`](#walletinterface), or `null` when none is selected.
...
Returns: [`SetSelectedWalletIdReturnType`](#setselectedwalletidreturntype).
...
Returns: [`WatchConnectedWalletsReturnType`](#watchconnectedwalletsreturntype) — Unsubscribe function — call it to stop receiving updates.
...
Returns: [`WatchSelectedWalletReturnType`](#watchselectedwalletreturntype) — Unsubscribe function — call it to stop receiving updates.
...
[`ApiClient`](#apiclient) implementation backed by the TON Center API.

Constructor: `new ApiClientToncenter(config)`

| Parameter                   | Type                                                    | Description                                                                                                                                                 |
| --------------------------- | ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `config`                    | [`ApiClientToncenterConfig`](#apiclienttoncenterconfig) | TON Center client config — endpoint URL, API key and optional DNS resolver override. Defaults to mainnet/testnet TON Center URLs based on `config.network`. |
...
| `config.network`            | [`Network`](#network)                                   | [`Network`](#network) the client should target. Determines the default `endpoint` when one isn't supplied.                                                  |
...
| `config.networks`       | [`NetworkAdapters`](#networkadapters)                        | Map of chain ID to [`NetworkConfig`](#networkconfig) (which holds the api-client setup). If omitted, AppKit defaults to mainnet only. |
| `config.connectors`     | <code><a href="#connectorinput">ConnectorInput</a>\[]</code> | Wallet connectors registered at startup.                                                                                              |
| `config.defaultNetwork` | [`Network`](#network)                                        | Default network.                                                                                                                      |
...
Abstract base class implemented by crypto on-ramp providers, either built-in or a custom integration. Apps don't use it directly — they consume providers through [`CryptoOnrampManager`](#cryptoonrampmanager) and the `getCryptoOnramp*` / [`createCryptoOnrampDeposit`](#createcryptoonrampdeposit) actions.

Constructor: `new CryptoOnrampProvider()`

#### `LayerswapCryptoOnrampProvider`

[`CryptoOnrampProvider`](#cryptoonrampprovider) implementation. Prefer the [`createLayerswapProvider`](#createlayerswapprovider) factory over instantiating this class directly — the factory returns a `ProviderInput` ready to pass to [`AppKitConfig`](#appkitconfig)'s `providers` or [`registerProvider`](#registerprovider).
...
| `config.apiUrl` | `string`                                              | Override the base API URL. Defaults to `https://api.layerswap.io/api/v2` (see [respective API V2](https://api.layerswap.io/swagger/index.html)). |

#### `DecentCryptoOnrampProvider`

[`CryptoOnrampProvider`](#cryptoonrampprovider) implementation. Prefer the [`createDecentProvider`](#createswapsxyzprovider) factory over instantiating this class directly — the factory returns a `ProviderInput` ready to pass to [`AppKitConfig`](#appkitconfig)'s `providers` or [`registerProvider`](#registerprovider).

Constructor: `new DecentCryptoOnrampProvider(config)`

| Parameter                    | Type                                                                  | Description                                                                                                                                                                |
| ---------------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `config`\*                   | [`DecentProviderConfig`](#swapsxyzproviderconfig)                     | Configuration carrying the required `apiKey` plus optional URL/sender overrides.                                                                                           |
| `config.apiKey`\*            | `string`                                                              | API key issued by Decent (passed as `x-api-key`)                                                                                                                           |
...
| `config.supportedChains`     | `Record<string, DecentChainConfig>`                                   | Full CAIP-2 source-chain map. Replaces the built-in map when provided.                                                                                                     |
| `config.supportedCurrencies` | [`CryptoOnrampSupportedCurrencies`](#cryptoonrampsupportedcurrencies) | Curated source and destination currency lists. Replaces the built-in lists when provided.                                                                                  |

### DeFi

#### `DefiError`

Base error thrown across all DeFi domains (swap, staking, on-ramp, crypto on-ramp). Subclassed by [`SwapError`](#swaperror), [`StakingError`](#stakingerror), [`CryptoOnrampError`](#cryptoonramperror) and the internal on-ramp error — catch the base when you don't care which domain produced the failure.
...
Abstract base class implemented by staking providers, either built-in or a custom integration. Apps don't use it directly — they consume providers through [`StakingManager`](#stakingmanager) and the `getStaking*` / `buildStakeTransaction` actions.
...
[`StakingProvider`](#stakingprovider) implementation. The constructor is private — always go through the [`createTonstakersProvider`](#createtonstakersprovider) factory and pass the result to [`AppKitConfig`](#appkitconfig)'s `providers` or [`registerProvider`](#registerprovider).
...
[`SwapProvider`](#swapprovider) implementation. Prefer the [`createDeDustProvider`](#creatededustprovider) factory over instantiating this class directly — the factory returns a `ProviderInput` ready to pass to [`AppKitConfig`](#appkitconfig)'s `providers` or [`registerProvider`](#registerprovider).
...
[`SwapProvider`](#swapprovider) implementation. Prefer the [`createOmnistonProvider`](#createomnistonprovider) factory over instantiating this class directly — the factory returns a `ProviderInput` ready to pass to [`AppKitConfig`](#appkitconfig)'s `providers` or [`registerProvider`](#registerprovider).
...
| ---------------------------- | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `config`                     | [`OmnistonSwapProviderConfig`](#omnistonswapproviderconfig) | Optional [`OmnistonSwapProviderConfig`](#omnistonswapproviderconfig). Defaults are filled in for any field left undefined. |
| `config.apiUrl`              | `string`                                                    | Optional URL for the given API                                                                                             |
| `config.defaultSlippageBps`  | `number`                                                    | Default slippage tolerance in basis points (1 `bp` = 0.01%)                                                                |
...
Abstract base class implemented by swap providers, either built-in or a custom integration. Apps don't use it directly — they consume providers through [`SwapManager`](#swapmanager) and the `getSwap*` / `buildSwapTransaction` actions.
...
[`WalletInterface`](#walletinterface) implementation backed by a TON Connect wallet. Prefer the [`createTonConnectConnector`](#createtonconnectconnector) factory over instantiating this class directly — the connector builds the adapter for every wallet it tracks and apps interact with it through standard AppKit actions ([`sendTransaction`](#sendtransaction), [`signText`](#signtext)/[`signBinary`](#signbinary)/[`signCell`](#signcell)). On-chain reads (balance, jettons, NFTs) live on separate actions ([`getBalance`](#getbalance), [`getJettons`](#getjettons), [`getNfts`](#getnfts)) and don't go through this adapter.

Constructor: `new TonConnectWalletAdapter(config)`

| Parameter                   | Type                                                              | Description                                                                                |
| --------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `config`\*                  | [`TonConnectWalletAdapterConfig`](#tonconnectwalletadapterconfig) | TON Connect wallet + UI instance and the ID of the connector that produced them.           |
| `config.connectorId`\*      | `string`                                                          | ID of the connector that produced this wallet — surfaced as `WalletInterface.connectorId`. |
| `config.tonConnectWallet`\* | `TonConnectWallet`                                                | Underlying TON Connect wallet object.                                                      |
| `config.tonConnectUI`\*     | `TonConnectUI`                                                    | TON Connect UI instance used to drive `sendTransaction` and `signData` calls.              |
...
Update payload delivered to [`watchBalance`](#watchbalance) / [`watchBalanceByAddress`](#watchbalancebyaddress) subscribers when the watched address's GRAM balance changes.
...
| `status`\*     | [`StreamingUpdateStatus`](#streamingupdatestatus) | Finality stage of the update — see [`StreamingUpdateStatus`](#streamingupdatestatus). |
...
| `network`   | [`Network`](#network)                                                          | Network to read the balance from. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
...
| ------------ | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `address`\*  | <code><a href="#userfriendlyaddress">UserFriendlyAddress</a> \| Address</code> | Wallet address — pass a [`UserFriendlyAddress`](#userfriendlyaddress) string or an `Address` instance from `@ton/core`.                                        |
| `network`    | [`Network`](#network)                                                          | Network to watch on. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
...
| Field          | Type                                          | Description                                                  |
| -------------- | --------------------------------------------- | ------------------------------------------------------------ |
| `address`      | [`UserFriendlyAddress`](#userfriendlyaddress) | The human-readable representation of the blockchain address. |
...
| `getBalance`\*             | <code>(address: <a href="#userfriendlyaddress">UserFriendlyAddress</a>, seqno?: number) => Promise\<<a href="#tokenamount">TokenAmount</a>></code>                   | Read the GRAM balance of an address in raw nanograms.                                                                                      |
...
Configuration accepted by [`NetworkConfig`](#networkconfig)'s `apiClient` — picks an [`ApiClient`](#apiclient) implementation (TON Center) and supplies its endpoint URL plus optional API key.
...
| `network`            | [`Network`](#network) | [`Network`](#network) the client should target. Determines the default `endpoint` when one isn't supplied.       |
...
| Field       | Type                  | Description                                                            |
| ----------- | --------------------- | ---------------------------------------------------------------------- |
| `network`\* | [`Network`](#network) | Network whose configured [`ApiClient`](#apiclient) should be returned. |
...
Display metadata for a token (Gram, jetton, or NFT) — name, symbol, image and animation as reported by the indexer. Surfaced as [`Jetton`](#jetton)'s `info` and [`NFT`](#nft)'s `info`.
...
| `symbol`      | `string`         | Ticker symbol of the token (e.g., "GRAM", "USDT") |
...
Wallet connector contract — the protocol-specific bridge (TON Connect, embedded wallet, …) AppKit drives once you register it via [`AppKitConfig`](#appkitconfig)'s `connectors`.
...
| `metadata`          | [`ConnectorMetadata`](#connectormetadata) | Display metadata override. Merged on top of TonConnect's default name and icon.                                                                                                                             |
...
| Field            | Type                                                         | Description                                                                                                                           |
| ---------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| `networks`       | [`NetworkAdapters`](#networkadapters)                        | Map of chain ID to [`NetworkConfig`](#networkconfig) (which holds the api-client setup). If omitted, AppKit defaults to mainnet only. |
| `connectors`     | <code><a href="#connectorinput">ConnectorInput</a>\[]</code> | Wallet connectors registered at startup.                                                                                              |
| `defaultNetwork` | [`Network`](#network)                                        | Default network.                                                                                                                      |
...
| Field                         | Type                                                                         | Description                                                                                                                                      |
| ----------------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| `connector:added`\*           | [`ConnectorAddedPayload`](#connectoraddedpayload)                            | Fired by [`addConnector`](#addconnector) when a connector is registered with AppKit.                                                             |
| `connector:removed`\*         | [`ConnectorRemovedPayload`](#connectorremovedpayload)                        | Fired when a connector is unregistered from AppKit — typically by calling the unregister function returned from [`addConnector`](#addconnector). |
| `connector:wallets-updated`\* | [`ConnectorWalletsUpdatedPayload`](#connectorwalletsupdatedpayload)          | Fired by a connector whenever its connected-wallets list changes (connect, disconnect, or account switch inside the wallet).                     |
...
| `networks:default-changed`\*  | [`DefaultNetworkChangedPayload`](#defaultnetworkchangedpayload)              | Fired by [`setDefaultNetwork`](#setdefaultnetwork) — carries the new default network, or `undefined` when the constraint was cleared.            |
| `streaming:balance-update`\*  | [`BalanceUpdate`](#balanceupdate)                                            | Fired by a streaming provider when a watched address's GRAM balance changes.                                                                     |
| `streaming:transactions`\*    | [`TransactionsUpdate`](#transactionsupdate)                                  | Fired by a streaming provider when new transactions land for a watched address.                                                                  |
| `streaming:jettons-update`\*  | [`JettonUpdate`](#jettonupdate)                                              | Fired by a streaming provider when a watched address's jetton holdings change.                                                                   |
| `provider:registered`\*       | [`BaseProviderUpdate`](#baseproviderupdate)                                  | Fired by a DeFi manager when a provider is registered through it (carries the provider's ID and kind).                                           |
| `provider:default-changed`\*  | [`BaseProviderUpdate`](#baseproviderupdate)                                  | Fired by a DeFi manager when its default provider is set or cleared (carries the new default's ID and kind).                                     |
...
| Field         | Type                      | Description                                                         |
| ------------- | ------------------------- | ------------------------------------------------------------------- |
| `connector`\* | [`Connector`](#connector) | The [`Connector`](#connector) that was just registered with AppKit. |
...
| Field         | Type                      | Description                                                                                               |
| ------------- | ------------------------- | --------------------------------------------------------------------------------------------------------- |
| `connector`\* | [`Connector`](#connector) | The [`Connector`](#connector) that was just unregistered from AppKit (already torn down via `destroy()`). |
...
Deposit details returned by a crypto on-ramp provider.

The user must send `amount` of `sourceCurrencyAddress` to `address` on `sourceChain`
to complete the on-ramp. The provider then delivers the target crypto to the user's TON address.
...
Parameters for creating a crypto on-ramp deposit.
...
Interface that all crypto on-ramp providers must implement

| Field                    | Type                                                                                                                                                                            | Description                                                                                                                                                                                   |
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `type`\*                 | `'crypto-onramp'`                                                                                                                                                               | Provider kind discriminator pinned to `'crypto-onramp'` so [`registerProvider`](#registerprovider) routes it to the crypto on-ramp manager.                                                   |
...
| `getQuote`\*             | <code>(params: <a href="#cryptoonrampquoteparams">CryptoOnrampQuoteParams</a>\<TQuoteOptions>) => Promise\<<a href="#cryptoonrampquote">CryptoOnrampQuote</a>></code>           | Get a quote for on-ramping from another crypto asset into a TON asset                                                                                                                         |
...
Static metadata for a crypto on-ramp provider.
...
Crypto on-ramp quote response with pricing information.
...
| `providerId`\*            | `string`              | Identifier of the crypto on-ramp provider.                                      |
...
Parameters for requesting a crypto-to-crypto on-ramp quote.

The target network is always TON, so only the source side is parameterised.

| Field                     | Type                         | Description                                                                                                                                                                         |
| ------------------------- | ---------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `amount`\*                | `string`                     | Amount to on-ramp (either source or target crypto, depending on isSourceAmount)                                                                                                     |
| `sourceCurrencyAddress`\* | `string`                     | Source crypto currency address (contract address or `0x0...` for native)                                                                                                            |
...
Final state of a crypto on-ramp deposit returned by [`getCryptoOnrampStatus`](#getcryptoonrampstatus) — `'success'` (delivered to the recipient), `'pending'` (still in flight), or `'failed'` (provider could not complete the deposit).
...
Configuration accepted by [`createDecentProvider`](#createswapsxyzprovider).

| Field                 | Type                                                                  | Description                                                                                                                                                                |
| --------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apiKey`\*            | `string`                                                              | API key issued by Decent (passed as `x-api-key`)                                                                                                                           |
...
| `supportedChains`     | `Record<string, DecentChainConfig>`                                   | Full CAIP-2 source-chain map. Replaces the built-in map when provided.                                                                                                     |
| `supportedCurrencies` | [`CryptoOnrampSupportedCurrencies`](#cryptoonrampsupportedcurrencies) | Curated source and destination currency lists. Replaces the built-in lists when provided.                                                                                  |

#### `DecentQuoteMetadata`

Provider-specific metadata returned on a [`CryptoOnrampQuote`](#cryptoonrampquote)'s `metadata` from Decent — carries the resolved action and bridge route that [`createCryptoOnrampDeposit`](#createcryptoonrampdeposit) needs.

| Field        | Type                      | Description                                                                                        |
| ------------ | ------------------------- | -------------------------------------------------------------------------------------------------- |
| `sender`\*   | `string`                  | EVM sender address Decent was quoted for. Required when later calling `createDeposit`.             |
| `response`\* | `DecentGetActionResponse` | Raw `getAction` response cached at quote time so `createDeposit` doesn't need an extra round-trip. |

#### `DecentQuoteOptions`

Decent-specific options forwarded through `providerOptions` on [`CryptoOnrampQuoteParams`](#cryptoonrampquoteparams).
...
| Field        | Type         | Description                                                                                                     |
| ------------ | ------------ | --------------------------------------------------------------------------------------------------------------- |
| `onChange`\* | `() => void` | Callback fired whenever a crypto on-ramp provider is registered or the default crypto on-ramp provider changes. |
...
Shape every DeFi domain manager (swap, staking, on-ramp, crypto on-ramp) satisfies — provider registration, default-provider selection and lookups. Mostly relevant when authoring a new domain manager.
...
Base interface implemented by every DeFi provider (swap, staking, on-ramp, crypto on-ramp) — exposes `providerId`, `type` and `getSupportedNetworks`. Domain-specific provider interfaces extend this with quote/build/status methods.

| Field                    | Type                                                 | Description                                                                                                                                                                                   |
| ------------------------ | ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `type`\*                 | [`DefiProviderType`](#defiprovidertype)              | Provider kind discriminator narrowed to the DeFi-domain literals so the right manager picks it up at registration time.                                                                       |
| `getSupportedNetworks`\* | <code>() => <a href="#network">Network</a>\[]</code> | Networks this provider can operate on. Consumers should check before calling provider methods. Implementations may return a static list or compute it dynamically (e.g. from runtime config). |
...
Either a ready-made DeFi/on-ramp provider instance or a factory that produces one — the value accepted by [`AppKitConfig`](#appkitconfig)'s `providers` and [`registerProvider`](#registerprovider).
...
| Field                | Type                                          | Description                                                                               |
| -------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------- |
| `jettonAddress`\*    | [`UserFriendlyAddress`](#userfriendlyaddress) | Jetton master contract address being transferred.                                         |
| `recipientAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress) | Recipient who should receive the jettons.                                                 |
...
| Field              | Type                                          | Description                                                                                                                                                                 |
| ------------------ | --------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `jettonAddress`\*  | [`UserFriendlyAddress`](#userfriendlyaddress) | Jetton master contract address (the token, not the user's wallet for it).                                                                                                   |
| `ownerAddress`\*   | [`UserFriendlyAddress`](#userfriendlyaddress) | Owner of the jetton wallet — typically the connected user's address.                                                                                                        |
...
| Field       | Type                                          | Description                                                                                                                                                 |
| ----------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `address`\* | [`UserFriendlyAddress`](#userfriendlyaddress) | Jetton master contract address whose metadata is being fetched.                                                                                             |
| `network`   | [`Network`](#network)                         | Network to query. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
...
| Field             | Type                                          | Description                                                                                                                                                 |
| ----------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `jettonAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress) | Jetton master contract address.                                                                                                                             |
| `ownerAddress`\*  | [`UserFriendlyAddress`](#userfriendlyaddress) | Owner whose jetton wallet should be derived.                                                                                                                |
| `network`         | [`Network`](#network)                         | Network to query. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
...
| `network`   | [`Network`](#network)                                                          | Network to read the jettons from. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
...
| Field     | Type                  | Description                                                               |
| --------- | --------------------- | ------------------------------------------------------------------------- |
| `network` | [`Network`](#network) | Network to read jettons from. Defaults to the selected wallet's network.  |
| `limit`   | `number`              | Maximum number of jettons to return.                                      |
| `offset`  | `number`              | Number of jettons to skip before returning results — used for pagination. |
...
| Field             | Type                                          | Description                                                                                           |
| ----------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `address`\*       | [`UserFriendlyAddress`](#userfriendlyaddress) | The jetton master contract address (the token itself).                                                |
| `walletAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress) | The owner's jetton-wallet contract address — the per-owner contract that actually holds this balance. |
| `balance`\*       | [`TokenAmount`](#tokenamount)                 | The current jetton balance.                                                                           |
| `info`\*          | [`TokenInfo`](#tokeninfo)                     | Information about the token.                                                                          |
...
| `verification`  | [`JettonVerification`](#jettonverification) | Verification status reported by the indexer.                                |
...
| `masterAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress)     | The master jetton contract address.                                                   |
| `walletAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress)     | The jetton wallet contract address.                                                   |
| `ownerAddress`\*  | [`UserFriendlyAddress`](#userfriendlyaddress)     | The owner of the jetton wallet.                                                       |
| `rawBalance`\*    | [`TokenAmount`](#tokenamount)                     | Balance in raw smallest units (e.g. nano)                                             |
...
| Field           | Type                                         | Description           |
| --------------- | -------------------------------------------- | --------------------- |
| `addressBook`\* | [`AddressBook`](#addressbook)                | Address book mapping. |
...
| Field                | Type                                          | Description                                                                                               |
| -------------------- | --------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `nftAddress`\*       | [`UserFriendlyAddress`](#userfriendlyaddress) | NFT contract address to transfer.                                                                         |
| `recipientAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress) | New owner address.                                                                                        |
| `amount`             | `string`                                      | Amount of GRAM to attach to the transfer for gas. Defaults to AppKit's NFT gas-fee constant when omitted. |
| `comment`            | `string`                                      | Optional human-readable comment attached to the transfer.                                                 |
...
| ----------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `address`\* | <code><a href="#userfriendlyaddress">UserFriendlyAddress</a> \| Address</code> | Owner address — pass a [`UserFriendlyAddress`](#userfriendlyaddress) string or an `Address` instance from `@ton/core`.                                               |
| `network`   | [`Network`](#network)                                                          | Network to read NFTs from. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
...
| Field     | Type                  | Description                                                            |
| --------- | --------------------- | ---------------------------------------------------------------------- |
| `network` | [`Network`](#network) | Network to read NFTs from. Defaults to the selected wallet's network.  |
| `limit`   | `number`              | Maximum number of NFTs to return.                                      |
| `offset`  | `number`              | Number of NFTs to skip before returning results — used for pagination. |
...
| Field                    | Type                                                     | Description                                                                    |
| ------------------------ | -------------------------------------------------------- | ------------------------------------------------------------------------------ |
| `address`\*              | [`UserFriendlyAddress`](#userfriendlyaddress)            | Contract address of the NFT item.                                              |
| `index`                  | `string`                                                 | Index of the item within its collection.                                       |
| `info`                   | [`TokenInfo`](#tokeninfo)                                | Display information about the NFT (name, description, images)                  |
| `attributes`             | <code><a href="#nftattribute">NFTAttribute</a>\[]</code> | Custom attributes/traits of the NFT (e.g., rarity, properties)                 |
| `collection`             | [`NFTCollection`](#nftcollection)                        | Information about the collection this item belongs to.                         |
| `auctionContractAddress` | [`UserFriendlyAddress`](#userfriendlyaddress)            | Address of the auction contract, if the NFT is being auctioned.                |
| `codeHash`               | [`Hex`](#hex)                                            | Hash of the NFT smart contract code.                                           |
| `dataHash`               | [`Hex`](#hex)                                            | Hash of the NFT's on-chain data.                                               |
...
| `ownerAddress`           | [`UserFriendlyAddress`](#userfriendlyaddress)            | Current owner address of the NFT.                                              |
| `realOwnerAddress`       | [`UserFriendlyAddress`](#userfriendlyaddress)            | Real owner address when NFT is on sale (sale contract becomes temporary owner) |
| `saleContractAddress`    | [`UserFriendlyAddress`](#userfriendlyaddress)            | Address of the sale contract, if the NFT is listed for sale.                   |
...
| Field           | Type                                          | Description                                                       |
| --------------- | --------------------------------------------- | ----------------------------------------------------------------- |
| `address`\*     | [`UserFriendlyAddress`](#userfriendlyaddress) | The blockchain address of the NFT collection contract.            |
...
| `codeHash`      | [`Hex`](#hex)                                 | The hash of the collection's smart contract code.                 |
| `dataHash`      | [`Hex`](#hex)                                 | The hash of the collection's data in the blockchain.              |
| `ownerAddress`  | [`UserFriendlyAddress`](#userfriendlyaddress) | The blockchain address of the collection owner.                   |
...
| Field         | Type                                   | Description                               |
| ------------- | -------------------------------------- | ----------------------------------------- |
| `addressBook` | [`AddressBook`](#addressbook)          | Address book entries related to the NFTs. |
...
| Field     | Type                  | Description                                         |
| --------- | --------------------- | --------------------------------------------------- |
| `network` | [`Network`](#network) | Network to query. Defaults to mainnet when omitted. |
...
| `networks`       | [`NetworkAdapters`](#networkadapters)                                     | Network configuration.                                                                                  |
...
Decimal string carrying a token amount. Preserves precision and avoids floating-point rounding (e.g., `"1.5"` GRAM, or raw nano units depending on the API).
...
| Field     | Type                            | Description                                                                                                                                                 |
| --------- | ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `bytes`\* | [`Base64String`](#base64string) | Binary blob the user is asked to sign, encoded as Base64.                                                                                                   |
| `network` | [`Network`](#network)           | Network to issue the sign request against. Defaults to AppKit's configured default network. When none is set, the wallet falls back to its current network. |
...
| Field      | Type                            | Description                                                                                                                                                 |
| ---------- | ------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cell`\*   | [`Base64String`](#base64string) | TON cell content encoded as Base64 (BoC).                                                                                                                   |
...
| Field       | Type                            | Description                           |
| ----------- | ------------------------------- | ------------------------------------- |
| `content`\* | [`Base64String`](#base64string) | Raw binary content encoded as Base64. |
...
| ----------- | ------------------------------- | ---------------------------------------------------------------------------------------------- |
| `schema`\*  | `string`                        | TL-B-style schema describing the cell layout so the wallet can render the payload to the user. |
| `content`\* | [`Base64String`](#base64string) | Cell content encoded as Base64.                                                                |
...
| `data`\*  | [`SignData`](#signdata) | Payload the user is asked to sign.                                                   |
...
| Field           | Type                                          | Description                                                                                                                                                 |
| --------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `userAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress) | Owner whose staked balance should be read.                                                                                                                  |
| `network`       | [`Network`](#network)                         | Network to query. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
...
| Field        | Type                  | Description                                                                                                                                                                               |
| ------------ | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `network`    | [`Network`](#network) | Network whose staking pool should be inspected. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
| `providerId` | `string`              | Provider to query. Defaults to the registered default staking provider.                                                                                                                   |
...
| Field        | Type                  | Description                                                                                                                                                                               |
| ------------ | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `network`    | [`Network`](#network) | Network whose provider metadata should be read. Defaults to the selected wallet's network. If no wallet is selected, falls back to AppKit's default network, or mainnet when none is set. |
| `providerId` | `string`              | Provider to query. Defaults to the registered default staking provider.                                                                                                                   |
...
| `unstakeMode`     | [`UnstakeModes`](#unstakemodes)                   | Unstake-timing mode the quote should target — see [`UnstakeMode`](#unstakemode) for the supported flavours (`'INSTANT'`, `'WHEN_AVAILABLE'`, `'ROUND_END'`). Only meaningful when `direction === 'unstake'` and the provider lists the mode in `supportedUnstakeModes`. |
...
| Field                          | Type                          | Description                                                                                                                        |
| ------------------------------ | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `rawStakedBalance`\*           | [`TokenAmount`](#tokenamount) | Amount currently staked, in raw smallest units of the stake token (e.g., nanograms).                                               |
...
| `rawInstantUnstakeAvailable`\* | [`TokenAmount`](#tokenamount) | Amount available for instant unstake, in raw smallest units of the stake token (e.g., nanograms).                                  |
...
| `rawInstantUnstakeAvailable` | [`TokenAmount`](#tokenamount) | Amount available for instant unstake right now, in raw smallest units of the stake token (e.g., nanograms).                                       |
...
| `exchangeRate`               | `string`                      | Exchange rate between stakeToken and receiveToken (e.g. 1 Gram = 0.95 tsTON). Undefined when there is no receiveToken (direct/custodial staking). |
...
| `stakeToken`            | [`StakingTokenInfo`](#stakingtokeninfo)                  | Override the [`StakingTokenInfo`](#stakingtokeninfo) of the token the user sends when staking.                                  |
| `receiveToken`          | [`StakingTokenInfo`](#stakingtokeninfo)                  | Override the [`StakingTokenInfo`](#stakingtokeninfo) of the token the user receives when staking (e.g. liquid-staking receipt). |
| `contractAddress`       | [`UserFriendlyAddress`](#userfriendlyaddress)            | Override the provider contract address (user-friendly format).                                                                  |
...
| `supportsReversedQuote` | `boolean`                                                | Override whether the provider supports reversed-amount quotes (e.g., specifying GRAM instead of tsTON on an unstake quote).     |
...
| `network`\*      | [`Network`](#network)                             | Network on which the staking will be executed.                                                                                |
| `providerId`\*   | `string`                                          | Identifier of the staking provider.                                                                                           |
| `unstakeMode`    | [`UnstakeModes`](#unstakemodes)                   | Unstake-timing mode the quote was produced for. Only meaningful when `direction === 'unstake'` — for `'stake'` it is ignored. |
...
Display metadata for a staking-pool token — `ticker`, `decimals` and `address` (or `'gram'` for native GRAM). Carried on [`StakingProviderMetadata`](#stakingprovidermetadata)'s `stakeToken` and `receiveToken` so the UI can render the pool's input/output assets.

| Field        | Type     | Description                                                                             |
| ------------ | -------- | --------------------------------------------------------------------------------------- |
| `ticker`\*   | `string` | Ticker symbol (e.g., `"GRAM"`, `"tsTON"`).                                              |
| `decimals`\* | `number` | Number of decimal places used to format raw amounts.                                    |
| `address`\*  | `string` | `'gram'` for native GRAM, otherwise the token contract address in user-friendly format. |
...
| `rawFromAmount`\*  | [`TokenAmount`](#tokenamount) | Amount of `fromToken` to sell, in raw smallest units (e.g., nanograms).                                                                      |
| `rawToAmount`\*    | [`TokenAmount`](#tokenamount) | Amount of `toToken` to buy, in raw smallest units (e.g., nanograms).                                                                         |
...
| `rawMinReceived`\* | [`TokenAmount`](#tokenamount) | Minimum amount of `toToken` to receive after slippage, in raw smallest units (e.g., nanograms).                                              |
...
| Field        | Type     | Description                                                      |
| ------------ | -------- | ---------------------------------------------------------------- |
| `address`\*  | `string` | Token contract address. `'gram'` is used for native GRAM on TON. |
| `decimals`\* | `number` | Number of decimal places used to format raw amounts.             |
...
| Field                | Type                                          | Description                                                                                           |
| -------------------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `recipientAddress`\* | [`UserFriendlyAddress`](#userfriendlyaddress) | Recipient address.                                                                                    |
...
| `payload`            | [`Base64String`](#base64string)               | Raw Base64 message payload — wins over `comment` when both are set.                                   |
| `stateInit`          | [`Base64String`](#base64string)               | Initial state for deploying a new contract, encoded as Base64.                                        |
...
| Field              | Type                            | Description                                 |
| ------------------ | ------------------------------- | ------------------------------------------- |
| `boc`\*            | [`Base64String`](#base64string) | BoC of the sent transaction.                |
| `normalizedBoc`\*  | [`Base64String`](#base64string) | Normalized BoC of the external-in message.  |
...
| Field                            | Type                                          | Description                                                       |
| -------------------------------- | --------------------------------------------- | ----------------------------------------------------------------- |
| `account`\*                      | [`UserFriendlyAddress`](#userfriendlyaddress) | Account of the transaction.                                       |
...
| `hash`\*                         | [`Hex`](#hex)                                 | Hash of the transaction.                                          |
...
| `traceExternalHash`\*            | [`Hex`](#hex)                                 | External hash of the trace.                                       |
...
| `totalFees`                      | [`TokenAmount`](#tokenamount)                 | Total fees of the transaction.                                    |
| `totalFeesExtraCurrencies`       | [`ExtraCurrencies`](#extracurrencies)         | Extra currencies in the total fees.                               |
...
| `amount`\*      | [`TokenAmount`](#tokenamount)         | Amount to transfer, in nanograms (1 Gram = 10⁹ nanograms).             |
| `extraCurrency` | [`ExtraCurrencies`](#extracurrencies) | Additional currencies to include in the transfer.                      |
...
| `address`\*      | [`UserFriendlyAddress`](#userfriendlyaddress)          | Account address the transactions belong to (the watched address).                                                                        |
| `transactions`\* | <code><a href="#transaction">Transaction</a>\[]</code> | Transactions that landed for `address` in this update.                                                                                   |
| `traceHash`\*    | [`Hex`](#hex)                                          | Hash of the trace that produced these transactions — the root external-message hash that spawned them.                                   |
| `addressBook`    | [`AddressBook`](#addressbook)                          | Pre-resolved address-book entries for raw addresses referenced inside `transactions`, so the UI can render labels without extra lookups. |
...
Configuration accepted by [`TonConnectWalletAdapter`](#tonconnectwalletadapter) when wrapping a TON Connect wallet for AppKit.

| Field                | Type               | Description                                                                                |
| -------------------- | ------------------ | ------------------------------------------------------------------------------------------ |
| `connectorId`\*      | `string`           | ID of the connector that produced this wallet — surfaced as `WalletInterface.connectorId`. |
| `tonConnectWallet`\* | `TonConnectWallet` | Underlying TON Connect wallet object.                                                      |
| `tonConnectUI`\*     | `TonConnectUI`     | TON Connect UI instance used to drive `sendTransaction` and `signData` calls.              |
...
Default ID assigned to the `TonConnect` connector when none is supplied to [`createTonConnectConnector`](#createtonconnectconnector). Pass this to [`connect`](#connect) / [`disconnect`](#disconnect) to drive the TON Connect flow without hard-coding the literal.
