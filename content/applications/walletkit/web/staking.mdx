...
WalletKit ships a bundled provider — **Tonstakers**, but you can configure a custom one. Extend the `StakingProvider` class. Implement `getQuote()`, `buildStakeTransaction()`, `getStakedBalance()`, `getStakingProviderInfo()`, and `getSupportedUnstakeModes()`.
...
| `getSupportedUnstakeModes(providerId?)`                       | Read unstake modes supported by the provider.          |
...
```ts title="TypeScript"
import { UnstakeMode } from '@ton/walletkit/staking/tonstakers';
