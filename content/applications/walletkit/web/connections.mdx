...
<Callout>
  [Initialize the WalletKit](/applications/walletkit/web/init) and [set up at least one GRAM wallet](/applications/walletkit/web/wallets) before using examples on this page.
</Callout>
...
When a user wants to connect a GRAM wallet from the dApp, the dApp fires the `connect` request over the bridge. The wallet service then handles it with the `onConnectRequest` method of the WalletKit.
...
When there are several GRAM wallets added, ask the user to select one before approving the connection request.
...
kit.onConnectRequest(async (event) => {
  try {
    ...
    // Selecting the 1st GRAM wallet by default
...
When a user disconnects a wallet service and its GRAM wallet from the dApp, the dApp fires the `disconnect` request over the bridge. The wallet service then handles it with the `onDisconnect` method of the WalletKit.

```ts title="TypeScript"
kit.onDisconnect(async (event) => {
  // Clean up any UI state related to this connection.
  console.log(`Disconnected from a dApp that used this GRAM wallet: ${event.walletAddress}`);
});
```
