...
<Callout>
  [Initialize the WalletKit](/applications/walletkit/web/init), [set up at least one GRAM wallet](/applications/walletkit/web/wallets) and [handle connection requests](/applications/walletkit/web/connections) before using examples on this page.
</Callout>
...
On TON, transactions are initiated by sending an external message to the GRAM wallet contract, which then processes it and sends internal messages as requested. To estimate money flows for planned transactions, WalletKit uses emulation via the configured API client, if supported.
...
If a dApp is connected to the wallet service, the former can request to sign data with the private key used by the selected GRAM wallet, which fires the `signData` request over the bridge. The wallet service then handles it with the `onSignDataRequest` method of the WalletKit.

The data to sign can be of several kinds: `text`, `binary`, or a raw [`cell`](/blockchain-basics/primitives/serialization/cells).
