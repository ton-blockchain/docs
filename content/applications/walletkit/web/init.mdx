...
1. Initialize the kit by creating an instance of the `TonWalletKit` object and passing it the `networks` parameter: a list of networks to operate on. For each network, specify the preferred API provider and its key obtained in [Prerequisites](#prerequisites):
   ...
   <Callout type="note">
     API providers allow WalletKit to read blockchain data — for example, to fetch account balances and query jettons and NFTs. API keys allow you to access to higher RPS limits.
   </Callout>
...
It's useful to add some of the optional settings right away. Add them to the same TonWalletKit call from [Step 2](3-initialize-the-kit):
