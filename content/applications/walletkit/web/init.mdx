---
title: "How to initialize the WalletKit on the Web platform"
...
See also: [TON Connect's wallet manifest](/applications/ton-connect/core-concepts#manifest).
...
>
  ...
  type Feature =
    | {
        ...
        // Depends on the GRAM wallet used, because different kinds can handle
  ...
  The `maxMessages` number depends on the GRAM wallet used, because every wallet has its own limit on the volume of messages.
...
>
  ...
  interface WalletInfo {
    ...
    /**
     * If the wallet handles JS Bridge connection, specifies the binding for the bridge object accessible through window. Example: the key "mytonwallet" means the bridge can be accessed as window.mytonwallet.
     */
    jsBridgeKey: string;
...
- [TON Connect's wallet manifest](/applications/ton-connect/core-concepts#manifest)
