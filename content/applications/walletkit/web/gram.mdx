...
Blockchain state changes constantly as new blocks are produced. This has implications for when and how to check GRAM wallet contract balances:
...
Notice that both cases require querying the blockchain data via the API client set during the [WalletKit initialization](/applications/walletkit/web/init#param-networks). Obtain and provide the key from the selected client to access higher requests-per-second limits.

### On-demand balance check

Use the `getBalance()` method to check the wallet contract balance in GRAM wallets managed by WalletKit. The balance is returned in nanograms, with 1 Gram equal to 10<sup>9</sup> nanograms.
