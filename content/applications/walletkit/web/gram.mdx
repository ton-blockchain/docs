---
title: "How to work with Gram using WalletKit on the Web platform"
sidebarTitle: "Work with Gram"
---
...
To work with Gram, the wallet service needs to handle [contract balances](#balances) and perform transfers initiated [from dApps](#transfers-from-dapps) and [from within the wallet service itself](#transfers-from-the-wallet-service).
...
Notice that both cases require querying the blockchain data via the API client set during the [WalletKit initialization](/applications/walletkit/web/configure#networks-required). Obtain and provide the key from the selected client to access higher requests-per-second limits.

### On-demand balance check

Use the `getBalance()` method to check the wallet contract balance in TON wallets managed by WalletKit. The balance is returned in nanograms, with 1 Gram equal to 10<sup>9</sup> nanograms.
...
async function getBalance(walletId: string): Promise<string | undefined> {
  ...
  // Query its balance in nanograms
...
When a connected dApp requests a Gram transfer, the wallet service follows this flow:
...
<Callout>
  Emulation uses the API client configured during [WalletKit initialization](/applications/walletkit/web/configure#networks-required).
</Callout>
