...
Streaming API serves as a real-time, streaming version of the [indexed access layer (API v3)](/api/v3/overview). Use it when building wallets, explorers, monitoring systems, or automation tools.
...
>
  ...
  When application state or business logic depends on a complete event sequence, resynchronize with historical data by polling [API v3](/api/v3/overview) after reconnecting.
</Callout>
...
>
  The [API v2](/api/v2/overview) and [API v3](/api/v3/overview) include their major version numbers in their product names. For the Streaming API, `v2` means the current protocol version and doesn't refer to different APIs.
</Callout>
...
The [event types section](/api/streaming/reference#event-types) and [notification schemas section](/api/streaming/reference#notification-schemas) provide the exact payload structure for each event.
...
The [delivery semantics section](/api/streaming/reference#delivery-semantics) and [event invalidation section](/api/streaming/reference#event-invalidation) document the exact behavior for each event type.
...
- [Notification reference](/api/streaming/reference)
- [API key](/api/get-api-key)
- [API authentication](/api/v3/authentication)
