...
[Server-Sent Events (SSE)](https://en.wikipedia.org/wiki/Server-sent_events) transport of the [Streaming API](/api/streaming/overview) uses a single `POST` request to establish the connection and specify the subscription. No further messages are sent by the client on this connection.
...
<ParamField
  ...
  required="true"
>
  One or more event types to receive: `transactions`, `actions`, `trace`, `trace_invalidated`, `account_state_change`, `jettons_change`. The [event types section](/api/streaming/reference#event-types) and [notification schemas section](/api/streaming/reference#notification-schemas) provide the exact payload structure for each event.
</ParamField>
...
<ParamField
  ...
  type="string[]"
>
  Optional action type filter. Applies only to `"actions"`. Refer to a list of [available action types in API v3](/api/v3/actions-and-traces/get-actions#parameter-action-type).
</ParamField>
...
If a client reconnects immediately after a disconnect, the previous connection may still be open for \~1 minute. The reconnect attempt receives a 429 error. Use exponential backoff or [an enterprise plan API key](/api/get-api-key).
...
- Skim the server [notification reference](/api/streaming/reference)
- [Get an API key](/api/get-api-key)
