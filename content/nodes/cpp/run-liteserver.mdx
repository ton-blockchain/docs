...
This guide describes how to set up a liteserver using [MyTonCtrl](/nodes/cpp/mytonctrl/overview).
...
<CodeGroup>
  ...
  ```bash title="Testnet"
  export ARCHIVE_TTL=2592000 STATE_TTL=86400 && sudo -v && nohup sudo bash install.sh -m liteserver -n testnet -d > mytonctrl_installation.log 2>&1 &
  ```
</CodeGroup>
...
Set up alerting in MyTonCtrl to get a notification of critical issues with the liteserver. For more information, see [MyTonCtrl private alerting bot](/nodes/cpp/mytonctrl/alerting).
...
Use [historical host metrics](/nodes/cpp/monitoring-host-performance) to correlate an incident with CPU, memory, storage, and network activity.
...
- [Run an archive liteserver node with MyTonCtrl](/nodes/cpp/run-archive-liteserver)
- [Run a validator node with MyTonCtrl](/nodes/cpp/run-validator)
- [Set up a node with MyTonCtrl](/nodes/cpp/setup-mytonctrl)
- [TON node types](/nodes/overview)
