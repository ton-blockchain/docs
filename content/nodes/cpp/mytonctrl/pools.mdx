...
```mytonctrl
new_pool <pool-name> <validator-reward-share-percent> <max-nominators-count> <min-validator-stake-gram> <min-nominator-stake-gram>
```
...
- `<min-validator-stake-gram>` and `<min-nominator-stake-gram>` define the minimum validator self-stake and delegator stake in GRAM (whole-gram values are expected; convert to nanograms before use if you need fractional amounts).
...
**Purpose:** Stake Gram from the validator wallet into the pool contract.

**Syntax**

```mytonctrl
deposit_to_pool <pool-addr> <amount-gram>
```

**Behavior**

- Builds a validator deposit query, signs it with the validator wallet, and sends it to `<pool-addr>`.
- `<amount-gram>` is a GRAM-denominated float or integer. Internally, the command converts it to the correct nanogram amount.
...
**Purpose:** Withdraw Gram from a nominator pool back to the validator wallet.

**Syntax**

```mytonctrl
withdraw_from_pool <pool-addr> <amount-gram>
```
...
- `<amount-gram>` is specified in GRAM.
...
**Purpose:** Withdraw Gram from a single-nominator pool back to the validator wallet or owner as configured.

**Syntax**

```mytonctrl
withdraw_from_single_pool <pool-addr> <amount-gram>
```

**Behavior**

- Issues an immediate withdrawal request against `<pool-addr>` for `<amount-gram>` GRAM.
