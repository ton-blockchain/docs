...
>
  Stake configuration directly controls how much GRAM is locked in the elector contract for a full validation round. Misconfiguration can lock the entire balance, leaving no funds for the next election. Review each setting carefully before applying changes on TON Mainnet.
</Callout>

MyTonCtrl automatically calculates the stake amount before submitting an election entry. The calculation balances operator preferences, staking mode, network-imposed limits, and fee reserves to produce a single integer value (in GRAM) sent to the [elector contract](/foundations/system).
...
| --------------------------------------------- | ---------------------- | ----------------------------------------------------------------------------------------------------------------- |
| Operator setting                              | `stake`                | Fixed stake amount (GRAM). When set, overrides percentage-based calculation.                                      |
| Operator setting                              | `stakePercent`         | Percentage of balance to stake. Defaults to `100` when unset.                                                     |
| Wallet state                                  | `balance`              | Current GRAM balance of the staking account (validator wallet, pool, or controller).                              |
...
| [On-chain config #17](/foundations/config#17) | `minStake`             | Network minimum stake. Submissions below this value are rejected by the elector.                                  |
| [On-chain config #17](/foundations/config#17) | `maxStake`             | Network maximum stake. Submissions above this value are clamped.                                                  |
...
Configure `stake` and `stakePercent` with the [`set`](/nodes/cpp/mytonctrl/core#set) command:
...
The 20 GRAM reserve covers the higher fee overhead of pool operations (deploy, process, and distribute messages).
...
The 50 GRAM reserve is the largest of all modes. Liquid staking controllers perform complex multi-step transactions (minting, burning, and balance adjustments), requiring a bigger fee cushion.
...
- **Existing validator** (active validators present):
  ...
  **Fee deduction at 100%:** when `stakePercent` is exactly `100`, MyTonCtrl subtracts 20 GRAM from the result to prevent draining the wallet completely. At lower percentages, the non-staked remainder covers fees implicitly.
...
### New validator with 800,000 GRAM balance
...
1. Result: **396,000 GRAM** is submitted. The remaining balance covers the next election.

### Returning validator with 1,600,000 GRAM balance
...
1. Result: **1,584,000 GRAM** is submitted.

### New validator with exactly 600,100 GRAM (near minimum)
...
1. `stakePercent == 100`: 20 GRAM fee deduction applies: `300,050 - 20 = 300,030`.
1. Result: **300,030 GRAM** is submitted.

### New validator with exactly 500,000 GRAM (halving would go below minimum)
...
1. `stakePercent == 100`: 20 GRAM fee deduction: `500,000 - 20 = 499,980`.
1. Result: **499,980 GRAM** is submitted.
...
### Nominator pool with 2,000,000 GRAM balance

Settings: standard nominator pool mode (not single nominator).

1. Pool mode detected (Priority 2).
1. `stake = 2,000,000 - 20 = 1,999,980`.
1. Result: **1,999,980 GRAM** is submitted.

## See also

- [Staking overview](/nodes/staking/overview) — comparison of staking solutions and minimum requirements.
- [Run a validator](/nodes/cpp/run-validator#step-4-set-optimal-stake-for-validator) — step-by-step guide for setting optimal stake.
- [MyTonCtrl settings](/nodes/cpp/mytonctrl/core#supported-settings) — full list of configurable parameters including `stake` and `stakePercent`.
- [Single nominator pools](/nodes/staking/single-nominator) — recommended staking contract for large holders.
