...
Follow the [MyTonCtrl single nominator pools guide](/nodes/cpp/mytonctrl/pools#single-nominator-pool-operations-single-nominator-mode).

## Nominator instructions

- To deposit, send an empty message with GRAM to the single nominator pool contract.
- To withdraw, send a message with 1 GRAM and the text comment "w" (withdraw) to the contract.

<Callout
  type="caution"
  title="Verify contract and message parameters"
>
  Depositing or withdrawing requires sending GRAM directly to a smart contract. Incorrect amounts, message comments, or contract addresses may result in irreversible fund loss.
...
- TON Center API v3 provides standard [nominator-pool endpoints](/api/v3/staking/get-nominator-pool), but does not provide a dedicated single-nominator endpoint.
