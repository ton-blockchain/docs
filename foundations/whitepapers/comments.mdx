...
Is not implemented. As of today, only the [Hypercube Routing (HR)](https://en.wikipedia.org/wiki/Hypercube_internetwork_topology) is implemented.
...
Product-ready only TON, `extra_currencies` is not fully implemented.
