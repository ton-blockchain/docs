...
<Aside>
  `ConfigParam 29` has evolved through several constructors. The latest, `consensus_config_v4#d9`, is defined in [`block.tlb`](https://github.com/ton-blockchain/ton/blob/v2026.04/crypto/block/block.tlb#L782). It extends `consensus_config_v3#d8` with a new `use_quic:Bool` (taking one bit from `flags`, which shrinks from 7 to 6 bits) and an additional `catchain_max_blocks_coeff:uint32` field at the end.
</Aside>
